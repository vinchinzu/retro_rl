"""Clear Scene3 (Battling Monks): P2-A scroll → P2-A Waldo.

Recipe (players=2, settle-gated scores, from Scene3.state HUD):
1. Hold P2-A ~300f (lands ~(160, 100)) → P1-A → +1000 (6450)
2. Hold P2-A ~200f (lands ~(206, 100)) → P1-A at ~(198, 100)
   → settled total ≥7850 (often ~8300–8350) → congrats

Scene3.state must be Battling Monks + HUD with a favorable layout
RNG. Rebuild from Scene2_Cleared: idle ~5f, then ~7× (A hold 6 +
idle 60). Default advance (0 idle) can land a soft layout where
P2-A seeks clocks/lookalikes instead of scroll.
"""
...
def clear_scene3(
    *,
    ...
    """Run the Scene3 clear recipe; return summary dict."""
    _configure_headless()
    out = RECORDINGS_DIR / "scene3_clear_run"
    ...
    try:
        ...
        if not (
            first_ok
            and first["score"] >= SCENE3_SCROLL_SCORE
            and first["found"] == 2
        ):
        ...
        if save_states:
            summary["after_find_state"] = str(
                save_state(env, GAME_DIR, GAME, "Scene3_AfterFind1000")
            )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--state", default="Scene3")
    parser.add_argument("--scroll-p2a", type=int, default=300)
    parser.add_argument("--scroll-x", type=int, default=160)
    parser.add_argument("--scroll-y", type=int, default=100)
    parser.add_argument("--waldo-p2a", type=int, default=200)
    parser.add_argument("--waldo-x", type=int, default=198)
    parser.add_argument("--waldo-y", type=int, default=100)
    parser.add_argument("--no-save", action="store_true")
    return parser


def main(argv: list[str] | None = None) -> int:
    """CLI for Scene3 clear recipe."""
    args = _build_parser().parse_args(argv)
    summary = clear_scene3(

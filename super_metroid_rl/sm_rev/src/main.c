...
int main(int argc, char** argv) {
  ...
  if (argc >= 2 && strcmp(argv[0], "--headless") == 0) {
    g_headless_frames = atoi(argv[1]);
    argc -= 2, argv += 2;
  }
  if (argc >= 2 && strcmp(argv[0], "--dump") == 0) {
    g_headless_dump = argv[1];
    argc -= 2, argv += 2;
  }
...
static void HandleInput(int keyCode, int keyMod, bool pressed) {
  if (HandleInputP2(keyCode, pressed))
    return;
...
static bool HandleInputP2(int keyCode, bool pressed) {
  int bit = -1;
  switch (keyCode) {
  case SDLK_j: bit = 6; break;
  case SDLK_l: bit = 7; break;
  case SDLK_i: bit = 4; break;
  case SDLK_k: bit = 5; break;
  case SDLK_n: bit = 0; break;
  case SDLK_u: bit = 1; break;
  case SDLK_RALT: bit = 2; break;
  case SDLK_BACKSPACE: bit = 3; break;
  case SDLK_m: bit = 8; break;
  case SDLK_o: bit = 9; break;
  case SDLK_COMMA: bit = 10; break;
  case SDLK_PERIOD: bit = 11; break;
  default:
    return false;
  }
  if (pressed)
    g_input2_state |= 1 << bit;
  else
    g_input2_state &= ~(1 << bit);
  return true;
}

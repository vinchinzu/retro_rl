# SF2 Turbo Multi-Character Training Guide
...
- Saves as: `models/sf2_multichar_ppo_final.zip`
- Checkpoints: `models/sf2_multichar_ppo_*_steps.zip` every 25k steps
...
```bash
./train_multi_opponent.sh --load models/sf2_multichar_ppo_final.zip
```
...
python ../fighters_common/train_ppo.py \
    ...
    --load models/sf2_multichar_ppo_final.zip \
    --steps 500000
```
...
```bash
cd models
tensorboard --logdir ppo_sf2_tensorboard/
...
- Wins ~50-70% across all 12 opponents
...
python train_multi_character.py \
    --load models/sf2_multichar_ppo_1000000_steps.zip \
...
```
models/
├── sf2_multichar_ppo_final.zip          # Final model after training
├── sf2_multichar_ppo_25000_steps.zip    # Checkpoint at 25k
├── sf2_multichar_ppo_50000_steps.zip    # Checkpoint at 50k
├── ...
└── ppo_sf2_tensorboard/                 # TensorBoard logs
```

**Note:** The old single-character model is:
- `sf2_ppo_final.zip` (1M steps, default character only)

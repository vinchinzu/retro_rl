# Agent Instructions — retro_harness.fighters

Shared fighting-game RL training stack under `retro_harness/fighters/`.
Used by MK1/MK2 and SF2/SSF2 workspaces under `snes/`.
...
```bash
# Run package tests (from monorepo root)
uv run pytest retro_harness/fighters/tests -q

# Train a game
uv run python retro_harness/fighters/train_ppo.py \
  --game sf2 --state Fight_StreetFighterIITurbo --steps 500000
uv run python retro_harness/fighters/train_ppo.py \
  --game mk1 --state Fight_MortalKombat --steps 500000 --n-envs 4

# Evaluate with rendering
uv run python retro_harness/fighters/train_ppo.py \
  --game sf2 --state Fight_StreetFighterIITurbo --eval \
  --load snes/street_fighter_ii/models/sf2_ppo_final.zip

# Create fight states
uv run python retro_harness/fighters/create_all_states.py
uv run python retro_harness/fighters/create_all_states.py --game sf2

# Train all games
./retro_harness/fighters/train_all.sh 500000
```
...
4. Place the game under `snes/<slug>/` (or `nes/`); `game_dir_name` is the slug
5. Test: `uv run pytest retro_harness/fighters/tests/test_env_load.py -q`

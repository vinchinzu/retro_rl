...
def contract_digest(kind: str, record: Mapping[str, Any]) -> str:
    return digest_record(kind, record)
...
class ObservationField:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "name", require_nonempty(self.name, "field name"))
        object.__setattr__(self, "dtype", require_nonempty(self.dtype, "field dtype"))
...
class ObservationContract:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "preprocessing", jsonable(dict(self.preprocessing), "preprocessing")
        )
...
class ActionEntry:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "action_id", require_nonempty(self.action_id, "action_id"))
...
class ActionContract:
    ...
    def __post_init__(self) -> None:
        buttons = tuple(
            require_nonempty(value, "controller button")
            for value in self.controller_buttons
        )
...
class RewardComponent:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "name", require_nonempty(self.name, "reward name"))
        ...
        object.__setattr__(
            self, "semantic", require_nonempty(self.semantic, "reward semantic")
        )
...
class RewardContract:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "aggregation",
            require_nonempty(self.aggregation, "aggregation"),
        )
        object.__setattr__(self, "version", require_nonempty(self.version, "version"))
...
class WrapperSpec:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "name", require_nonempty(self.name, "wrapper name"))
        object.__setattr__(self, "version", require_nonempty(self.version, "version"))
        object.__setattr__(self, "config", jsonable(dict(self.config), "wrapper config"))
...
class EnvironmentContract:
    ...
    def __post_init__(self) -> None:
        for name in (
            ...
            object.__setattr__(
                self, name, require_nonempty(getattr(self, name), name)
            )
        for name in ("action_space_size", "frame_skip", "players"):
            value = getattr(self, name)
            if isinstance(value, bool) or not isinstance(value, int) or value < 1:
                raise ContractError(f"{name} must be a positive integer")
        object.__setattr__(self, "metadata", jsonable(dict(self.metadata), "metadata"))

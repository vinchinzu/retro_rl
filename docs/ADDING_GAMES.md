...
Before scaffolding, apply the [roadmap prioritization heuristic](ROADMAP.md):
...
```text
snes/new_game/             # package import stays `import new_game`
├── game.py
├── roms/                  # gitignored
├── tests/
└── custom_integrations/
...
```text
nes/new_game/              # package import stays `import new_game`
...
- Use `retro_harness`: emulator/session lifecycle, actions, input scripts,
  states, recording, RAM schemas, task protocols, behavior trees, watchdogs,
  RAM discovery, cursor/combat policy, and segment completion helpers.
- Add `retro_harness.platformer`: platformer progress tracking, route evaluation,
  replay, and optimizers.
- Add `retro_harness.fighters`: fighting-game environments and training.
...
[scripted full-run process](FULL_RUN_PROCESS.md) and the
[M0–M8 maturity ladder](DEVELOPMENT_LADDER.md). Create the game-local
...
([benchmark spec](BENCHMARK_SPEC.md)). Resource assists should refill only

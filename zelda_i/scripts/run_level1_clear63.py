...
Examples::
    ...
    # Isolated from naturally-produced Level1North.state
    uv run python zelda_i/scripts/run_level1_clear63.py

    # Power-on → sword → first key → room 0x63 → clear, no state load
...
def run_once(
    *,
    ...
    clear_max_frames: int = CLEAR_63_MAX_FRAMES,
    tag: str = "level1_clear63",
    save_checkpoint: bool = False,
) -> dict:
    configure_headless()
    start_state = "NONE" if natural_entry else "Level1North"
    env = make_env(GAME, start_state, GAME_DIR, render_mode="rgb_array")
    first_key: Level1FirstKeyController | None = None
    north: Level1UnlockNorthController | None = None
    clear = Level1Clear63Controller()
    ...
    try:
        ...
        if prefix_ok:
            for _ in range(clear_max_frames):
                obs, *_ = env.step(clear.step(read_snapshot(env.get_ram())).action)
                if clear.success or clear.phase.name == "FAILED":
                    break

        ram = env.get_ram()
        snap = read_snapshot(ram)
        ok = prefix_ok and level1_room_63_cleared(ram)
        checkpoint = None
        if ok and save_checkpoint:
            checkpoint = str(save_state(env, GAME_DIR, GAME, "Level1Cleared63"))
        label = "natural" if natural_entry else "isolated"
        png = RECORDINGS_DIR / f"{tag}_{label}.png"
        save_rgb_png(obs, png)
        return {
            "ok": ok,
            "stage": "level1_room_63_cleared" if ok else "failed",
            "entry": entry,
            "prefix_ok": prefix_ok,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--natural-entry",
        action="store_true",
        help="Boot from power-on instead of loading Level1North.state",
    )
    ...
    parser.add_argument(
        "--save-state",
        action="store_true",
        help="Save successful endpoint as Level1Cleared63.state",
    )
    ...
    for i in range(args.trials):
        report = run_once(
            ...
            clear_max_frames=args.clear_max_frames,
            tag=f"level1_clear63_t{i}",
        ...
        print(
            ...
            f"clear_frames={report['clear']['frames']} "
            f"phase={report['clear']['phase']}"
        )

    label = "natural" if args.natural_entry else "isolated"
    out = RECORDINGS_DIR / f"level1_clear63_{label}.json"
    payload = {

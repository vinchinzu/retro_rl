"""Run through the first key and unlock Level 1's north entrance door.

Examples::

    # Isolated from naturally-produced Level1FirstKey.state
    uv run python zelda_i/scripts/run_level1_north.py

    # Power-on → sword → Level 1 first key → room 0x63, no state load
...
def run_once(
    *,
    ...
    tag: str = "level1_north",
    save_checkpoint: bool = False,
) -> dict:
    configure_headless()
    start_state = "NONE" if natural_entry else "Level1FirstKey"
    ...
    try:
        ...
        if prefix_ok:
            for _ in range(north_max_frames):
                obs, *_ = env.step(north.step(read_snapshot(env.get_ram())).action)
                if north.success or north.phase.name == "FAILED":
                    break

        ram = env.get_ram()
        snap = read_snapshot(ram)
        ok = prefix_ok and level1_north_room_success(ram)
        checkpoint = None
        if ok and save_checkpoint:
            checkpoint = str(save_state(env, GAME_DIR, GAME, "Level1North"))
        label = "natural" if natural_entry else "isolated"
        png = RECORDINGS_DIR / f"{tag}_{label}.png"
        save_rgb_png(obs, png)
        return {
            "ok": ok,
            "stage": "level1_north_room" if ok else "failed",
            "entry": entry,
            "prefix_ok": prefix_ok,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--natural-entry",
        action="store_true",
        help="Boot from power-on instead of loading Level1FirstKey.state",
    )
    ...
    parser.add_argument(
        "--save-state",
        action="store_true",
        help="Save successful endpoint as Level1North.state",
    )
    ...
    for i in range(args.trials):
        ...
        print(
            ...
            f"north_frames={report['north']['frames']} "
            f"phase={report['north']['phase']}"
        )

    label = "natural" if args.natural_entry else "isolated"
    out = RECORDINGS_DIR / f"level1_north_{label}.json"
    payload = {

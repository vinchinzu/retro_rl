...
Usage:
    from platformer_common.route import RouteConfig, evaluate_route, get_route
...
from platformer_common.level_config import get_level_config, LevelConfig
    ...
    if verbose:
        ...
        if failed:
            ...
            if f.status == "TRANSITION_FAILED":
                ...
                if f.config:
                    print(f"    uv run python -m platformer_common -l {f.config.level_id} play")
    ...
    *,
    ...
    for round_num in range(max_rounds):
        ...
        if f.status == "NO_RECORDING":
            ...
            print(f"  Record it: uv run python -m platformer_common -l {config.level_id} play")
            return result
        ...
        if not best_result.completed:
            ...
            print(f"    uv run python -m platformer_common -l {config.level_id} play --state {state_name}")
            return result

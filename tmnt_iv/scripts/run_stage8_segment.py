...
from tmnt_iv.paths import GAME, GAME_DIR, RECORDINGS_DIR, STAGE8_STATE
...
def _with_stage_progress(
    ...
    """Mark Stage 8 complete when the stage byte advances past 7."""
...
def _walk_until_enemies(
    ...
    *,
    ...
    """Idle/drift until a living enemy appears (Mode-7 auto-scroll)."""
    obs: Any = None
    for _ in range(max_frames):
        if state.living_enemies:
            break
        # RIGHT is a no-op at the right rail; idle lets depth scroll.
        step = env.step(idle_action())
        obs = step[0]
...
def run_stage8_segment(
    *,
    ...
    heal_low_hp: bool = True,
    state_name: str | None = None,
    out_dir: Path | None = None,
) -> dict[str, Any]:
    """Load Stage8, chain Mode-7 waves, write report + mid states."""
    configure_headless()
    available = get_available_states(GAME, GAME_DIR)
    preferred = (STAGE8_STATE, "Stage8_probe", "NONE")
    chosen = state_name or next(
        (name for name in preferred if name in available),
        "NONE",
    )
    out = out_dir or (RECORDINGS_DIR / "stage8_segment")
    ...
    try:
        ...
        final_state = state
        png = save_rgb_png(obs, out / "stage8_0000_start.png")
        ...
        for frame_i in range(1, max_frames + 1):
            ...
            if tracker.waves_cleared > last_cleared:
                ...
                png = save_rgb_png(
                    obs, out / f"stage8_{frame_i:04d}_{tag}.png"
                )
                screenshots.append(png.name)
                if (
                    save_wave_states
        ...
        if outcome is SegmentOutcome.SUCCESS and tracker.boss_reached:
            ...
            if save_wave_states and final_state.boss_active:
                path = save_state(env, GAME_DIR, GAME, "Boss8")
                saved_states.append(path.name)
            ...
            end_tag = "death"
        png = save_rgb_png(
            obs, out / f"stage8_{tracker.frames:04d}_{end_tag}.png"
        )
        screenshots.append(png.name)
        ...
        if (
            ...
            path = save_state(env, GAME_DIR, GAME, "Stage8_Clear")
            saved_states.append(path.name)
        ...
        report_path = write_json_report(
            out / "stage8_segment.json", report
        )

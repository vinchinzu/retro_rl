#!/usr/bin/env python3
"""
Train a generic fighter using all 16 SSF2 characters.
...
Phase 1: Character rotation (this script)
  - Randomly select from 16 character starting states
...
# All 16 SSF2 character starting states (12 original + 4 new challengers)
CHARACTER_STATES = [
    # Original 12
    "Fight_Ryu",
    "Fight_EHonda",
...
print("="*60)
print("SSF2 MULTI-CHARACTER TRAINING - PHASE 1")
print("="*60)
print(f"\nTraining with {len(CHARACTER_STATES)} characters:")
print("\nOriginal 12 World Warriors:")
for i, state in enumerate(CHARACTER_STATES[:12], 1):
    char_name = state.replace("Fight_", "")
    print(f"  {i:2d}. {char_name}")
print("\nNew Challengers:")
for i, state in enumerate(CHARACTER_STATES[12:], 13):
    char_name = state.replace("Fight_", "")
    print(f"  {i:2d}. {char_name}")
...
if __name__ == '__main__':
    ...
    print("Model will be saved to: models/ssf2_multichar_ppo_*.zip\n")

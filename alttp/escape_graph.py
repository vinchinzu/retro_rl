...
Courtyard pocket → main castle → Zelda remains ``verification="planned"``
until emulator evidence promotes those legs.
"""
...
def escape_route_graph() -> RouteGraph:
    ...
    nodes = (
        ...
        GraphNode(
            ...
            area="hyrule_castle",
            tags=frozenset({"indoors", "escape", "planned"}),
            meta=_room_meta(
                HYRULE_CASTLE_MAIN_HALL_ROOM,
    ...
    edges = (
        ...
        # --- planned: leave 0x55 and reach Zelda ---------------------------
        GraphEdge(
            ...
            meta={"note": "clear soldiers; collect small key in 0x55"},
        ),
        GraphEdge(
            ...
            meta={
                "note": "key/shutter path out of 0x55 (STATUS blocker)",
...
def escape_route_legs() -> tuple[RouteLeg, ...]:
    ...
    ``fighter_sword`` is acquired at the uncle leg; ``small_key`` after the
    south combat clear; ``zelda_follower`` when the cell rescue completes.
    """
    return (
        RouteLeg(

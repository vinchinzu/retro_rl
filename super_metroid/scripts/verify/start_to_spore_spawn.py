"""Independently verify the start-to-Spore-Spawn report and video."""
...
def verify(report_path: Path, video_path: Path) -> dict[str, object]:
    ...
    for observed in report["transitions"]:
        ...
        edge = START_TO_SPORE_SPAWN_GRAPH.edge_for(source, target)
    ...
    result = {
        ...
        "graph_id": START_TO_SPORE_SPAWN_GRAPH.graph_id,
...
def main() -> None:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=RECORDINGS_DIR / "start_to_spore_spawn.json",
    )
    parser.add_argument(
        "--video",
        type=Path,
        default=RECORDINGS_DIR / "start_to_spore_spawn.mp4",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=RECORDINGS_DIR / "start_to_spore_spawn.verify.json",
    )

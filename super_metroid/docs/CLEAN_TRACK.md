...
**Bronze / Resource-assisted** KPDR spine (current tip: Frog Save).
...
The assisted spine is past mid-game (Frog Save / K4.0). Early-game controllers
(Morph → two-Missile detour → Bombs → **Bomb Torizo** → Parlor) are mature
and hash-pinned. Clean is a **privilege-reduction** lane on that already-green
prefix — not a second full-route rewrite.

| Fact | Assisted (primary) | Clean (this track) |
|------|--------------------|--------------------|
| Intervention | Resource-assisted (energy + ammo) | **Clean** (no resource writes) |
| Program tip | Frog Save (`--to frog`) | ★ Target: Bomb Torizo exit (`--to bombs`) |
...
2. **Separate artifacts.** Clean reports/videos/checkpoints use a `_clean`
   stem (e.g. `start_to_bomb_torizo_clean.json`). Never overwrite
   `start_to_*.json` / `.mp4` that are assisted baselines.
3. **STATUS primary tip stays assisted.** Clean greens go under a **Clean
   track** section. Do not re-label Frog Save / Varia / Business as Clean.
...
5. **Clean failure ≠ assisted demotion.** A RED clean bombs run never unmarks
   assisted continuous greens or rolls back the Frog tip.
...
|------:|-----------|-----|------------------------|--------|
| 1 | Power-on → Morph (clean) | `--to morph --clean` | `start_to_morph_clean` | **GREEN** 27,074f |
| 2 | Power-on → **Bomb Torizo exit (clean)** | `--to bombs --clean` | `start_to_bomb_torizo_clean` | ★ product tip (missiles detour green; BT = existing model) |
| 3 | → Spore exit (clean) | `--to spore --clean` | `start_to_spore_spawn_clean` | parked until BT green |
...
Log: [`tasks/SM-CLEAN-BOMBS-residual.md`](tasks/SM-CLEAN-BOMBS-residual.md).
...
# → recordings/start_to_bomb_torizo_clean.json by default
```
...
|-------|---------------|-------|
| **B spine** (assisted Frog → Cathedral…) | N/A (primary) | P0 product |
...
| Ticket | Role |
|--------|------|
| [`SM-CLEAN-CONTRACT`](tasks/SM-CLEAN-CONTRACT.md) | Docs contract (this file + ASSIST pointer) |
| [`SM-CLEAN-ARTIFACTS`](tasks/SM-CLEAN-ARTIFACTS.md) | `_clean` paths; never overwrite assisted |
| [`SM-CLEAN-CLI`](tasks/SM-CLEAN-CLI.md) | `--clean` alias + path default |
| [`SM-CLEAN-INTEGRITY`](tasks/SM-CLEAN-INTEGRITY.md) | Zero resource-write asserts |
| [`SM-CLEAN-MORPH`](tasks/SM-CLEAN-MORPH.md) | Clean morph continuous |
| [`SM-CLEAN-BOMBS`](tasks/SM-CLEAN-BOMBS.md) | ★ Clean → Bomb Torizo continuous tip |
| [`SM-CLEAN-BT-ECONOMY`](tasks/SM-CLEAN-BT-ECONOMY.md) | Optional; only if clean BT RED |

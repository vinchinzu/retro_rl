# TASK SM-ROOM-SEG-42: Dual-track room segment — East Tunnel
...
## Own files only
- `policies/room_clears/room_cf80_from_d21c_to_cefb.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-42-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-42-note.md`
...
- Queue rank **102**, room `0xCF80` **East Tunnel**, problem `room_cf80_from_d21c_to_cefb`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_cf80_from_d21c_to_cefb`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_cf80_from_d21c_to_cefb
   uv run python super_metroid/scripts/room/run_problem.py teleport room_cf80_from_d21c_to_cefb
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_cf80_from_d21c_to_cefb
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_cf80_from_d21c_to_cefb
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_cf80_from_d21c_to_cefb --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-42-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-42-residual.md`). Planner owns queue

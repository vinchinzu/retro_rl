# TASK SM-ROOM-SEG-34: Dual-track room segment — Red Pirate Shaft
...
## Own files only
- `policies/room_clears/room_b139_from_af72_to_afce.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-34-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-34-note.md`
...
- Queue rank **106**, room `0xB139` **Red Pirate Shaft**, problem `room_b139_from_af72_to_afce`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_b139_from_af72_to_afce`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_b139_from_af72_to_afce
   uv run python super_metroid/scripts/room/run_problem.py teleport room_b139_from_af72_to_afce
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_b139_from_af72_to_afce
   ```
...
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-34-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-34-residual.md`). Planner owns queue

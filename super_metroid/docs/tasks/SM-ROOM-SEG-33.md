# TASK SM-ROOM-SEG-33: Dual-track room segment — Watering Hole
...
## Own files only
- `policies/room_clears/room_d13b_from_d16d_to_d16d.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-33-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-33-note.md`
...
- Queue rank **93**, room `0xD13B` **Watering Hole**, problem `room_d13b_from_d16d_to_d16d`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_d13b_from_d16d_to_d16d`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_d13b_from_d16d_to_d16d
   uv run python super_metroid/scripts/room/run_problem.py teleport room_d13b_from_d16d_to_d16d
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_d13b_from_d16d_to_d16d
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_d13b_from_d16d_to_d16d
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_d13b_from_d16d_to_d16d --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-33-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-33-residual.md`). Planner owns queue

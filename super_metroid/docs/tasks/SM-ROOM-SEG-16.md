# TASK SM-ROOM-SEG-16: Dual-track room segment — Crab Tunnel
...
## Own files only
- `policies/room_clears/room_d08a_from_d21c_to_cfc9.json` (create or edit)
...
- Queue rank **85**, room `0xD08A` **Crab Tunnel**, problem `room_d08a_from_d21c_to_cfc9`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_d08a_from_d21c_to_cfc9`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_d08a_from_d21c_to_cfc9
   uv run python super_metroid/scripts/room/run_problem.py teleport room_d08a_from_d21c_to_cfc9
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_d08a_from_d21c_to_cfc9
   ```
...
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_d08a_from_d21c_to_cfc9 --promote
   ```

# TASK SM-ROOM-SEG-30: Dual-track room segment — Kronic Boost Room
...
## Own files only
- `policies/room_clears/room_ae74_from_ae32_to_affb.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-30-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-30-note.md`
...
- Queue rank **102**, room `0xAE74` **Kronic Boost Room**, problem `room_ae74_from_ae32_to_affb`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_ae74_from_ae32_to_affb`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_ae74_from_ae32_to_affb
   uv run python super_metroid/scripts/room/run_problem.py teleport room_ae74_from_ae32_to_affb
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_ae74_from_ae32_to_affb
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ae74_from_ae32_to_affb
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ae74_from_ae32_to_affb --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-30-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-30-residual.md`). Planner owns queue

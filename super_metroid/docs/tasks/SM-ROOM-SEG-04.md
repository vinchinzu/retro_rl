# TASK SM-ROOM-SEG-04: Dual-track room segment — Space Jump Room
...
## Own files only
- `policies/room_clears/room_d9aa_from_da60_to_da60.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-04-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-04-note.md`
...
- Queue rank **60**, room `0xD9AA` **Space Jump Room**, problem `room_d9aa_from_da60_to_da60`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_d9aa_from_da60_to_da60`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_d9aa_from_da60_to_da60
   uv run python super_metroid/scripts/room/run_problem.py teleport room_d9aa_from_da60_to_da60
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_d9aa_from_da60_to_da60
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_d9aa_from_da60_to_da60
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_d9aa_from_da60_to_da60 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-04-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-04-residual.md`). Planner owns queue

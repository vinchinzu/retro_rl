# TASK SM-ROOM-SEG-43: Dual-track room segment — Cathedral
...
## Own files only
- `policies/room_clears/room_a788_from_afa3_to_a7b3.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-43-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-43-note.md`
...
- Queue rank **103**, room `0xA788` **Cathedral**, problem `room_a788_from_afa3_to_a7b3`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_a788_from_afa3_to_a7b3`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_a788_from_afa3_to_a7b3
   uv run python super_metroid/scripts/room/run_problem.py teleport room_a788_from_afa3_to_a7b3
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_a788_from_afa3_to_a7b3
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a788_from_afa3_to_a7b3
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a788_from_afa3_to_a7b3 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-43-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-43-residual.md`). Planner owns queue

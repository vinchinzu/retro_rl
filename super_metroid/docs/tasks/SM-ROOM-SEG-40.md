# TASK SM-ROOM-SEG-40: Dual-track room segment — Noob Bridge
...
## Own files only
- `policies/room_clears/room_9fba_from_a253_to_9e52.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-40-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-40-note.md`
...
- Queue rank **100**, room `0x9FBA` **Noob Bridge**, problem `room_9fba_from_a253_to_9e52`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_9fba_from_a253_to_9e52`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_9fba_from_a253_to_9e52
   uv run python super_metroid/scripts/room/run_problem.py teleport room_9fba_from_a253_to_9e52
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_9fba_from_a253_to_9e52
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_9fba_from_a253_to_9e52
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_9fba_from_a253_to_9e52 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-40-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-40-residual.md`). Planner owns queue

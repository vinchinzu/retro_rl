# TASK SM-ROOM-SEG-36: Dual-track room segment — Cosine Room
...
## Own files only
- `policies/room_clears/room_ab3b_from_ab07_to_ab07.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-36-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-36-note.md`
...
- Queue rank **96**, room `0xAB3B` **Cosine Room**, problem `room_ab3b_from_ab07_to_ab07`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_ab3b_from_ab07_to_ab07`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_ab3b_from_ab07_to_ab07
   uv run python super_metroid/scripts/room/run_problem.py teleport room_ab3b_from_ab07_to_ab07
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_ab3b_from_ab07_to_ab07
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ab3b_from_ab07_to_ab07
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ab3b_from_ab07_to_ab07 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-36-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-36-residual.md`). Planner owns queue

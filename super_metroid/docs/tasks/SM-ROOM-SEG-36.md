# TASK SM-ROOM-SEG-36: Dual-track room segment — Lower Norfair Farming Room
...
## Own files only
- `policies/room_clears/room_b37a_from_b32e_to_b482.json` (create or edit)
...
- Queue rank **108**, room `0xB37A` **Lower Norfair Farming Room**, problem `room_b37a_from_b32e_to_b482`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_b37a_from_b32e_to_b482`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_b37a_from_b32e_to_b482
   uv run python super_metroid/scripts/room/run_problem.py teleport room_b37a_from_b32e_to_b482
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_b37a_from_b32e_to_b482
   ```

# TASK SM-ROOM-SEG-19: Dual-track room segment — Statues Hallway
...
## Own files only
- `policies/room_clears/room_a5ed_from_a66a_to_99bd.json` (create or edit)
...
- Queue rank **79**, room `0xA5ED` **Statues Hallway**, problem `room_a5ed_from_a66a_to_99bd`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_a5ed_from_a66a_to_99bd`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_a5ed_from_a66a_to_99bd
   uv run python super_metroid/scripts/room/run_problem.py teleport room_a5ed_from_a66a_to_99bd
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_a5ed_from_a66a_to_99bd
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a5ed_from_a66a_to_99bd
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a5ed_from_a66a_to_99bd --promote
   ```

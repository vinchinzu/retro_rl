# TASK SM-ROOM-SEG-48: Dual-track room segment — Waterway
...
## Own files only
- `policies/room_clears/room_a0d2_from_9d19_to_9d19.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-48-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-48-note.md`
...
- Queue rank **108**, room `0xA0D2` **Waterway**, problem `room_a0d2_from_9d19_to_9d19`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_a0d2_from_9d19_to_9d19`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_a0d2_from_9d19_to_9d19
   uv run python super_metroid/scripts/room/run_problem.py teleport room_a0d2_from_9d19_to_9d19
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_a0d2_from_9d19_to_9d19
   ```
...
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a0d2_from_9d19_to_9d19 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-48-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-48-residual.md`). Planner owns queue

# TASK SM-ROOM-SEG-29: Dual-track room segment — Spiky Platforms Tunnel
...
## Own files only
- `policies/room_clears/room_ae07_from_ae32_to_ad5e.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-29-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-29-note.md`
...
- Queue rank **101**, room `0xAE07` **Spiky Platforms Tunnel**, problem `room_ae07_from_ae32_to_ad5e`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_ae07_from_ae32_to_ad5e`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_ae07_from_ae32_to_ad5e
   uv run python super_metroid/scripts/room/run_problem.py teleport room_ae07_from_ae32_to_ad5e
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_ae07_from_ae32_to_ad5e
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ae07_from_ae32_to_ad5e
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ae07_from_ae32_to_ad5e --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-29-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-29-residual.md`). Planner owns queue

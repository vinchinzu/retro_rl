# TASK SM-ROOM-SEG-03: Dual-track room segment — Etacoon Super Room
...
## Own files only
- `policies/room_clears/room_a051_from_a011_to_a011.json` (create or edit)
...
- Queue rank **59**, room `0xA051` **Etacoon Super Room**, problem `room_a051_from_a011_to_a011`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_a051_from_a011_to_a011`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_a051_from_a011_to_a011
   uv run python super_metroid/scripts/room/run_problem.py teleport room_a051_from_a011_to_a011
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_a051_from_a011_to_a011
   ```

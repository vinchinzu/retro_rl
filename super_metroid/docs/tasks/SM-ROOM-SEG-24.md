# TASK SM-ROOM-SEG-24: Dual-track room segment — Shaktool Room
...
## Own files only
- `policies/room_clears/room_d8c5_from_d6d0_to_d69a.json` (create or edit)
...
- Queue rank **84**, room `0xD8C5` **Shaktool Room**, problem `room_d8c5_from_d6d0_to_d69a`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_d8c5_from_d6d0_to_d69a`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_d8c5_from_d6d0_to_d69a
   uv run python super_metroid/scripts/room/run_problem.py teleport room_d8c5_from_d6d0_to_d69a
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_d8c5_from_d6d0_to_d69a
   ```
...
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_d8c5_from_d6d0_to_d69a --promote
   ```

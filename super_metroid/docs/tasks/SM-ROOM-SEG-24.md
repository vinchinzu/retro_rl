# TASK SM-ROOM-SEG-24: Dual-track room segment — Cathedral Entrance
...
## Own files only
- `policies/room_clears/room_a7b3_from_a788_to_a7de.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-24-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-24-note.md`
...
- Queue rank **93**, room `0xA7B3` **Cathedral Entrance**, problem `room_a7b3_from_a788_to_a7de`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_a7b3_from_a788_to_a7de`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_a7b3_from_a788_to_a7de
   uv run python super_metroid/scripts/room/run_problem.py teleport room_a7b3_from_a788_to_a7de
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_a7b3_from_a788_to_a7de
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a7b3_from_a788_to_a7de
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a7b3_from_a788_to_a7de --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-24-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-24-residual.md`). Planner owns queue

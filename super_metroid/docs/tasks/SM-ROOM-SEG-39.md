# TASK SM-ROOM-SEG-39: Dual-track room segment — Grapple Tutorial Room 3
...
## Own files only
- `policies/room_clears/room_ab64_from_ab07_to_abd2.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-39-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-39-note.md`
...
- Queue rank **99**, room `0xAB64` **Grapple Tutorial Room 3**, problem `room_ab64_from_ab07_to_abd2`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_ab64_from_ab07_to_abd2`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_ab64_from_ab07_to_abd2
   uv run python super_metroid/scripts/room/run_problem.py teleport room_ab64_from_ab07_to_abd2
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_ab64_from_ab07_to_abd2
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ab64_from_ab07_to_abd2
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_ab64_from_ab07_to_abd2 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-39-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-39-residual.md`). Planner owns queue

# TASK SM-ROOM-SEG-39: Dual-track room segment — Wrecked Ship East Super Room
...
## Own files only
- `policies/room_clears/room_cdf1_from_caf6_to_caf6.json` (create or edit)
...
- Queue rank **111**, room `0xCDF1` **Wrecked Ship East Super Room**, problem `room_cdf1_from_caf6_to_caf6`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_cdf1_from_caf6_to_caf6`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_cdf1_from_caf6_to_caf6
   uv run python super_metroid/scripts/room/run_problem.py teleport room_cdf1_from_caf6_to_caf6
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_cdf1_from_caf6_to_caf6
   ```
...
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_cdf1_from_caf6_to_caf6 --promote
   ```

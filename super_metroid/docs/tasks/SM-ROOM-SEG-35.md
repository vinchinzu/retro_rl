# TASK SM-ROOM-SEG-35: Dual-track room segment — Fast Ripper Room
...
## Own files only
- `policies/room_clears/room_b2da_from_b3a5_to_b6c1.json` (create or edit)
...
- Queue rank **107**, room `0xB2DA` **Fast Ripper Room**, problem `room_b2da_from_b3a5_to_b6c1`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_b2da_from_b3a5_to_b6c1`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_b2da_from_b3a5_to_b6c1
   uv run python super_metroid/scripts/room/run_problem.py teleport room_b2da_from_b3a5_to_b6c1
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_b2da_from_b3a5_to_b6c1
   ```

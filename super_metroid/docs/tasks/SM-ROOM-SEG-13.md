# TASK SM-ROOM-SEG-13: Dual-track room segment — The Moat
...
## Own files only
- `policies/room_clears/room_95ff_from_93fe_to_948c.json` (create or edit)
...
- Queue rank **82**, room `0x95FF` **The Moat**, problem `room_95ff_from_93fe_to_948c`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_95ff_from_93fe_to_948c`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_95ff_from_93fe_to_948c
   uv run python super_metroid/scripts/room/run_problem.py teleport room_95ff_from_93fe_to_948c
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_95ff_from_93fe_to_948c
   ```
...
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_95ff_from_93fe_to_948c --promote
   ```

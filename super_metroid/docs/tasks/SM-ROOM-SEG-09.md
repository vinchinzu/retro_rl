# TASK SM-ROOM-SEG-09: Dual-track room segment — Green Brinstar Fireflea Room
...
## Own files only
- `policies/room_clears/room_9c5e_from_9ad9_to_9c89.json` (create or edit)
- entry fixture under `custom_integrations/SuperMetroid-Snes/` for **this
  problem only** (bootstrap/teleport state if missing)
- optional residual: `docs/tasks/SM-ROOM-SEG-09-residual.md`
- optional note: `docs/tasks/SM-ROOM-SEG-09-note.md`
...
- Queue rank **78**, room `0x9C5E` **Green Brinstar Fireflea Room**, problem `room_9c5e_from_9ad9_to_9c89`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_9c5e_from_9ad9_to_9c89`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_9c5e_from_9ad9_to_9c89
   uv run python super_metroid/scripts/room/run_problem.py teleport room_9c5e_from_9ad9_to_9c89
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_9c5e_from_9ad9_to_9c89
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_9c5e_from_9ad9_to_9c89
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_9c5e_from_9ad9_to_9c89 --promote
   ```
5. Write residual with PROCESS schema. Next card may be a one-knob residual
   for this same problem (`SM-ROOM-SEG-09-R1`) or `none` if green+promoted.
...
## Done when
Residual filed (message and/or `SM-ROOM-SEG-09-residual.md`). Planner owns queue

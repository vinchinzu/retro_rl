# TASK SM-ROOM-SEG-09: Dual-track room segment — Hi-Jump Room
...
## Own files only
- `policies/room_clears/room_a9e5_from_aa41_to_aa41.json` (create or edit)
...
- Queue rank **69**, room `0xA9E5` **Hi-Jump Room**, problem `room_a9e5_from_aa41_to_aa41`.
...
- If residual exists for this problem, read the latest `docs/tasks/*-residual.md`
  or note mentioning `room_a9e5_from_aa41_to_aa41`.

## Do
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py bootstrap room_a9e5_from_aa41_to_aa41
   uv run python super_metroid/scripts/room/run_problem.py teleport room_a9e5_from_aa41_to_aa41
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py scaffold room_a9e5_from_aa41_to_aa41
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a9e5_from_aa41_to_aa41
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python super_metroid/scripts/room/run_problem.py run room_a9e5_from_aa41_to_aa41 --promote
   ```

...
# Back-compat alias used by tests / callers of the old dual-loader API.
def resolve_entry_door(
    problem: Mapping[str, Any],
    door_map: Mapping[tuple[int, int], int] | None = None,
) -> int | None:
    """Resolve entry door pointer (prefer catalog; optional map override)."""
...
def build_entry_door_map(
    *,
    ...
    hops_path: Path | None = None,  # retained for call-site compat; unused
) -> dict[tuple[int, int], int]:
    """Build ``(source, target) → door_ptr`` from the reference connection graph.

    Prefer problem ``entry.doorPtr`` via :func:`resolve_entry_door_ptr`. This
    helper remains for bulk tooling and tests; it no longer reimplements a
    second JSON walk or hops overlay.
    """
    from super_metroid.rooms.segment_contract import (
    ...
    del hops_path  # no longer consulted; single graph source of truth

...
def test_clean_artifact_stem_appends_once() -> None:
    assert clean_artifact_stem("start_to_bomb_torizo") == "start_to_bomb_torizo_clean"
    assert (
        clean_artifact_stem("start_to_bomb_torizo_clean")
        == "start_to_bomb_torizo_clean"
    )
...
def test_default_artifacts_assisted_vs_clean() -> None:
    video, report = default_artifacts("start_to_bomb_torizo")
    assert video.name == "start_to_bomb_torizo.mp4"
    assert report.name == "start_to_bomb_torizo.json"

    c_video, c_report = default_artifacts("start_to_bomb_torizo", clean=True)
    assert c_video.name == "start_to_bomb_torizo_clean.mp4"
    assert c_report.name == "start_to_bomb_torizo_clean.json"
...
def test_default_tip_artifact_paths_clean_isolated() -> None:
    ...
    # Primary assisted default tip stems unchanged.
    frog_v, frog_r = default_artifact_paths()
    assert frog_v.name == "start_to_frog_save.mp4"
    assert frog_r.name == "start_to_frog_save.json"
    clean_frog_v, clean_frog_r = default_artifact_paths(clean=True)
    assert clean_frog_v.name == "start_to_frog_save_clean.mp4"
    assert clean_frog_r.name == "start_to_frog_save_clean.json"


def test_default_tip_room_timing_clean_stem() -> None:
    normal = default_tip_room_timing_path("supers")
    assert normal.name == "start_to_supers_room_timing.json"
    clean = default_tip_room_timing_path("supers", clean=True)
    assert clean.name == "start_to_supers_clean_room_timing.json"
...
def test_run_to_accepts_clean_flags_on_early_tips() -> None:
    ...
    from super_metroid.routes.continuous import run_start_to_bombs, run_start_to_morph, run_to

    for fn in (run_start_to_morph, run_start_to_bombs, run_to):

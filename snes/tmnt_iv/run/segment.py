...
One loop, nine ``StageSpec`` rows. CLI: ``python -m tmnt_iv.scripts.run_segment --stage N``.
"""
...
def with_stage_progress(state: GameState, *, start_stage: int) -> GameState:
...
def maybe_save_png(
    obs: Any, path: Path, *, enabled: bool, bag: list[str]
) -> None:
    """Append a screenshot name when ``enabled``; no-op otherwise."""
    if not enabled or obs is None:
        return
    png = save_rgb_png(obs, path)
    bag.append(png.name)
...
def run_stage_segment(
    ...
    *,
    ...
    """Load a stage pin, chain waves, write JSON + mid states (PNGs opt-in)."""
    configure_headless()
    ...
    try:
        ...
        for frame_i in range(1, frames + 1):
            ...
            if (
                ...
                maybe_save_png(
                    obs,
                    out / f"{tag}_{frame_i:04d}_boss.png",
                    enabled=screenshots,
                    bag=shots,
                )
            ...
            if tracker.waves_cleared > last_cleared:
                ...
                maybe_save_png(
                    obs,
                    out / f"{tag}_{frame_i:04d}_wave{wave.index}_clear.png",
                    enabled=screenshots,
                    bag=shots,
                )
                healthy = (
        ...
        maybe_save_png(
            obs,
            out / f"{tag}_{tracker.frames:04d}_{end_tag}.png",
            enabled=screenshots,
            bag=shots,
        )

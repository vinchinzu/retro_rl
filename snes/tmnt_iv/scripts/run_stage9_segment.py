...
from tmnt_iv.paths import GAME, GAME_DIR, RECORDINGS_DIR, STAGE9_STATE
...
def _with_stage_progress(
    ...
    """Mark Stage 9 complete when the stage byte advances past 8."""
...
def run_stage9_segment(
    *,
    ...
    """Load Stage9, chain Starbase waves, write report + mid states."""
    configure_headless()
    available = get_available_states(GAME, GAME_DIR)
    preferred = (STAGE9_STATE, "Stage9_probe", "NONE")
    chosen = state_name or next(
        (name for name in preferred if name in available),
        "NONE",
    )
    out = out_dir or (RECORDINGS_DIR / "stage9_segment")
    ...
    try:
        ...
        final_state = state
        png = save_rgb_png(obs, out / "stage9_0000_start.png")
        ...
        for frame_i in range(1, max_frames + 1):
            ...
            if tracker.waves_cleared > last_cleared:
                ...
                png = save_rgb_png(
                    obs, out / f"stage9_{frame_i:04d}_{tag}.png"
                )
                screenshots.append(png.name)
                if (
                    save_wave_states
        ...
        if outcome is SegmentOutcome.SUCCESS and tracker.boss_reached:
            ...
            if save_wave_states and final_state.boss_active:
                path = save_state(env, GAME_DIR, GAME, "Boss9")
                saved_states.append(path.name)
            ...
            end_tag = "death"
        png = save_rgb_png(
            obs, out / f"stage9_{tracker.frames:04d}_{end_tag}.png"
        )
        screenshots.append(png.name)
        ...
        if (
            ...
            path = save_state(env, GAME_DIR, GAME, "Stage9_Clear")
            saved_states.append(path.name)
        ...
        report_path = write_json_report(
            out / "stage9_segment.json", report
        )

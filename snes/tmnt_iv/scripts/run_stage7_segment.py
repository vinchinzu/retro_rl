...
from tmnt_iv.paths import GAME, GAME_DIR, RECORDINGS_DIR, STAGE7_STATE
...
def _with_stage_progress(
    ...
    """Mark Stage 7 complete when the stage byte advances past 6."""
...
def run_stage7_segment(
    *,
    ...
    """Load Stage7, chain waves, write report + PNGs + mid states."""
    configure_headless()
    available = get_available_states(GAME, GAME_DIR)
    preferred = (STAGE7_STATE, "Stage7_probe", "NONE")
    chosen = state_name or next(
        (name for name in preferred if name in available),
        "NONE",
    )
    out = out_dir or (RECORDINGS_DIR / "stage7_segment")
    ...
    try:
        ...
        final_state = state
        png = save_rgb_png(obs, out / "stage7_0000_start.png")
        ...
        for frame_i in range(1, max_frames + 1):
            ...
            if tracker.waves_cleared > last_cleared:
                ...
                png = save_rgb_png(
                    obs, out / f"stage7_{frame_i:04d}_{tag}.png"
                )
                screenshots.append(png.name)
                if (
                    save_wave_states
        ...
        if outcome is SegmentOutcome.SUCCESS and tracker.boss_reached:
            ...
            if save_wave_states and final_state.boss_active:
                path = save_state(env, GAME_DIR, GAME, "Boss7")
                saved_states.append(path.name)
            ...
            end_tag = "death"
        png = save_rgb_png(
            obs, out / f"stage7_{tracker.frames:04d}_{end_tag}.png"
        )
        screenshots.append(png.name)
        ...
        if (
            ...
            path = save_state(env, GAME_DIR, GAME, "Stage7_Clear")
            saved_states.append(path.name)
        ...
        report_path = write_json_report(
            out / "stage7_segment.json", report
        )

...
Usage:
  SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \\
    uv run python -m tmnt_iv.scripts.slash_keep_trace \\
...
def _read_slash(ram: Any) -> tuple[int, int, int, int] | None:
    """Return (x, y, hp, status) for living Slash, else None."""
    for base in ENEMY_BASES:
        if read_u8(ram, base + OFF_CHAR) != _SLASH_CHAR:
            continue
...
def _pick(policy: Stage1Policy, state: Any) -> tuple[Any, str]:
    ...
    try:
        ...
        if slash0 is not None:
            ...
            rec0 = FrameRec(
                ...
                facing=0,
                behind=False,
                side=_side(state0.player_x, sx),
                player_hp=state0.health,
            )
            history.append(rec0)
            frames_log.append(rec0)
            status_hist[st] += 1
        prev_player_hp = state0.health if 0 < state0.health <= 0x60 else None

        for frame in range(1, max_frames + 1):
            final_frame = frame
            ram = env.get_ram()
            state = parse_game_state(ram, frame=frame)
            slash_raw = _read_slash(ram)
            slash_enemy = next(
            ...
            if (
                ...
                if slash_raw is not None:
                    sx, sy, shp, st = slash_raw
                    adx = abs(sx - state.player_x)
                    dy = sy - state.player_y
                    side = _side(state.player_x, sx)
                ...
                print(
                    ...
                    f"side={side} ifr={phit.iframes} phase={phase}"
                )
            ...
            if _heal(env, state, heal_mode):
                ...
                prev_player_hp = (
                    state.health if 0 < state.health <= 0x60 else prev_player_hp
                )
            ...
            action, reason = _pick(policy, state)
            if action[8]:
                action = idle_action()
                reason = "blocked_a"
                blocked_a += 1

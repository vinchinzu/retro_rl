...
Densify thrash / recover / corridor-decision apply live in
``crop_refill_verify``. ``CropRefillMixin`` is the public surface for
``CropWaterTask``.
"""
...
class CropRefillMixin(CropRefillVerifyMixin):
    ...
    def _queue_east_south_corridor_charge(
        ...
        Body lives in ``pond_hop.build_east_south_corridor_charge``.
        """
        if player is None:
            player = self._navigator.current_tile
    ...
    def _queue_gap_south_fallback(
        ...
        Body lives in ``pond_hop.build_gap_south_fallback``.
        """
        if player is None:
            player = self._navigator.current_tile
    ...
    def _queue_west_south_lip_charge(
        ...
        Body lives in ``pond_hop.build_west_south_lip_charge``.
        """
        if player is None:
            player = self._navigator.current_tile
    ...
    def _pond_corridor_gap_open(self, ram: np.ndarray) -> bool:
        ...
        Predicate lives in ``pond_hop.pond_corridor_gap_open``.
        """
    ...
    def _refill_hop_goal(
        ...
        Body lives in ``pond_hop.compute_refill_hop_goal``.
        """

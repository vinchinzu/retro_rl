...
class CropWaterTask(Task):
    ...
    def _start_refill(self, ram: np.ndarray):
        ...
        if (
            not getattr(self, "_pending_south_lip_charge", False)
            and not getattr(self, "_pending_gap_charge", False)
            and not self._action_queue
            and (
                getattr(self, "_south_lip_charges", 0) >= 6
                or getattr(self, "_east_south_charges", 0) >= 4
            )
        ):
            print(
                f"[CROP] Soft-reset refill charges at {player} "
                f"(lip={getattr(self, '_south_lip_charges', 0)} "
                f"es={getattr(self, '_east_south_charges', 0)})"
            )
            self._south_lip_charges = 0
            self._east_south_charges = 0
            self._refill_densify_stalls = 0
    ...
    def _handle_fence_open(self, world: WorldState) -> Optional[TaskResult]:
        ...
        if status == TaskStatus.SUCCESS:
            ...
            if player[1] >= 32:
                ...
                if (
                    tile_dist(player, (32, 34)) > 3
                    and player[0] <= 28
                    and getattr(self, "_south_lip_charges", 0) < 2
                ):
                    print(
                    ...
                    self._refill_pond_tile = (32, 34)
                    self._refill_pond_face = "up"
                    self._approach_tile = (32, 34)
                    self._target_tile = (32, 34)
                    self._face_direction = "up"
    ...
    def _find_nav_path(
        ...
        if self._plot_phase == "refill" and getattr(self, "_refill_multihop", False):
            ...
            # Power-on residual: south of wall densify thrash (rr-qc9r).
            # Near F0 (dist≤4 on lip): do NOT re-arm long lip charges — they
            # overshoot / oscillate. Force short east densify only.
            near_f0_start = (
                start[1] >= 33
                and 26 <= start[0] <= 31
                and goal[0] >= 32
            ...
            if north_thrash or south_thrash or east_thrash:
                ...
                # Near F0 densify stalls: reset stall counter so we keep
                # trying east hops without scripted overshoot charges.
                stalls = getattr(self, "_refill_densify_stalls", 0) + 1
                self._refill_densify_stalls = stalls
                self._refill_densify_last = (start, goal)
                if stalls >= 6 and getattr(self, "_south_lip_charges", 0) < 4:
                    # One short near-stand charge only.
                    print(
                        f"[CROP] Near-F0 densify stall at {start}; "
                        f"short lip charge"
                    )
                    self._queue_west_south_lip_charge(start)
                    self._refill_densify_stalls = 0
    ...
    def _handle_navigate(self, ram: np.ndarray) -> Optional[TaskResult]:
        ...
        if getattr(self, "_pending_multihop_after_drop", False):
            ...
            if player[1] >= 32:
                ...
                self._start_refill(ram)
                return None
            # Still north of wall — scripted east→south past fence end.
            print(
        ...
        if getattr(self, "_pending_gap_charge", False):
            ...
            if (
                ...
                and getattr(self, "_south_lip_charges", 0) < 10
            ):
                print(
        ...
        if getattr(self, "_pending_south_lip_charge", False):
            ...
            if tile_dist(player, (32, 34)) <= 1 or tile_dist(player, (33, 34)) <= 1:
                stand = (
                    (32, 34)
                    if tile_dist(player, (32, 34))
                    <= tile_dist(player, (33, 34))
                    else (33, 34)
                )
                self._refill_pond_tile = stand
                self._refill_pond_face = "up"
            ...
            if (
                ...
                and player[1] >= 32
                and tile_dist(player, (32, 34)) > 1
                and getattr(self, "_south_lip_charges", 0) < 12
            ):
                print(
            ...
            if (
                ...
                and player[1] >= 32
                and getattr(self, "_south_lip_charges", 0) < lip_cap
            ):
                print(

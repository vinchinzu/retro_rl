...
def _run_power_on(env, *, max_frames: int | None = None) -> tuple[dict[str, object], int]:
    task = PowerOnStartTask()
    if max_frames is not None:
        task.timeout = max_frames
    frame = 0
    task.reset(_world(env, frame))
    ...
    while frame < task.timeout:
        ...
        if result.status == TaskStatus.SUCCESS:
            ...
            print(
                ...
                f"tm=0x{int(read_ram_value(world.ram, 'tilemap')):02X} "
                f"pos=({int(read_ram_value(world.ram, 'player_x'))},"
                f"{int(read_ram_value(world.ram, 'player_y'))})",
                flush=True,
            )
...
def _save_state(env, name: str) -> Path:
...
def _end_state_paths(task_json: Path) -> list[Path]:
...
def _unset_headless_for_interactive() -> None:
...
def cmd_capture_entry(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if isinstance(obs, tuple):
            obs = obs[0]
        power_on, frames = _run_power_on(env)
        ...
        state_path = _save_state(env, args.name)
...
def cmd_record(args: argparse.Namespace) -> int:
    ...
    _unset_headless_for_interactive()
    ...
    try:
        ...
        if args.power_on:
            power_on_report, boot_frames = _run_power_on(env)
        ...
        for path in _end_state_paths(task_json):
            path.parent.mkdir(parents=True, exist_ok=True)
            with gzip.open(path, "wb") as handle:

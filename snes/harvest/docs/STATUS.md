...
- First mountain grape is a **ground spawn** at the x=20 stand `~(326,409)` tile `(20,25)` (west stump/rock to the east). Not the carpenter 2×2 plants. Walk the tape corridor (land → east gap, never A on Gotz `0x025B` ~(31,37) → west loop → stand). Return **jumps** the x=20 grape cliff onto mid terrace `~(328,568)` then east/south dirt — do not reverse the west loop (second cliff still blocks due-south). A on the grape pixel sets `held=0x03` and opens Eat / Don't eat — **Down then A to keep**. Mash-A eats it. Mountain dialogue with `held=0` is Gotz.
  ...
  **Same-day D2 plant GREEN 2026-08-14 (`rr-20w.1`):** natural `Y1_Inside_House` auto day-plan shipped the grape, bought potato seeds **0→1** for **$300→$100**, then completed reactive `CLEAR_PLOT` → shed hoe+seed fetch → `CROP_ESTABLISH`. The clearer tossed toward RAM-confirmed open ground, cleared `(12,25)`, `(14,27)`, and the `(13,28)` notch, then handed off at `cleared=3`; no repeat of the former ~(13,27) 1500f bush pin. Shed exit waits for its settled outdoor `(26,30)` stand before selecting the second shelf route. Hoe stands at `(13,29)`, faces up, verifies the `(13,28)` target, then seed use verifies the same target before `CROP_ESTABLISH -> sequence complete`. Evidence: `logs/d2_full_watch/watch_20260814_rr_20w_1_1_hoe_target.log`. Human tape `d2_farm_plant` remains reference only; production does not replay it.
  **Residual (not a plant failure):** the following can fetch succeeds, but sparse one-cell `CROP_WATER` reports `dry_crops=1` then skips the plot and soft-fails `watered=0`; tracked by `rr-rzpd`. Gate B is not promoted.

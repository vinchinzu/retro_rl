...
- First mountain grape is a **ground spawn** at the x=20 stand `~(326,409)` tile `(20,25)` (west stump/rock to the east). Not the carpenter 2×2 plants. Walk the tape corridor (land → east gap, never A on Gotz `0x025B` ~(31,37) → west loop → stand). Return **jumps** the x=20 grape cliff onto mid terrace `~(328,568)` then east/south dirt — do not reverse the west loop (second cliff still blocks due-south). A on the grape pixel sets `held=0x03` and opens Eat / Don't eat — **Down then A to keep**. Mash-A eats it. Mountain dialogue with `held=0` is Gotz.
  ...
  **Seed shop GREEN 2026-08-13** (`rr-uos8`): `BuySeedsTask` nav to `shop_door` plaza `(602,274)` face up → `0x1C` → clerk `(182,342)` Buy/Don't buy `0x033D` → potato **0→1**, money **300→100**, back on farm `0x00` at 2542f. Evidence: `recordings/buy_seeds_d2_probe.json`. CrossMap origin-return without those deltas is now a miss.
  **Spring D2 compose GREEN 2026-08-14** (`rr-zmss`): `Y1_Inside_House` auto day-plan grape then real shop then budget CLEAR → D3. `MOUNTAIN_BERRY` shipping_money **0→150**; `BUY_SEEDS` kind=`shop_buy` potato **0→1** money **300→100**; overnight wallet **$250** (grape credit − bag); `CLEAR_FIELD` after shop (`cleared=2` lift-only budget). Clean mid_run=0. Evidence: `recordings/rr_zmss_d2_shop.json`. Farm-bush `SHIP_BERRY` remains deferred.
  **Same-day plant splice (rr-20w.1) PARTIAL:** after a real shop close the orchestrator splices optional `CLEAR_PLOT` → `ENSURE_CROP_SEEDS`/`NAV_CROP`/`CROP_ESTABLISH` → can/`CROP_WATER`. Human ref only (no replay): `d2_farm_plant` cleared/hoed/planted/watered 1 cell. Production is reactive skills: fence-jump toss (B+Down through y=31, no east align), `SelectCarrySkill`, hoe/plant/water-until-tile. `CROP_ESTABLISH` is `pocket_plant_cell` (hoe+plant at `(13,28)`).
  **Live D2 watch 2026-08-14** (`logs/d2_full_watch/watch_20260814_151707.log`, quit 7914f mid-CLEAR): grape+shop still GREEN; `CLEAR_PLOT` now walks in (`[CLEAR] Pocket ready` at `(214,417)` tile `(13,26)`, not the west-gate/path empty scan). Then **bush pin**: stood `(217,441)` ~(13,27) ~1500f+ (`Toss held=0x09 before next lift` / clearing timeout at `(14,27)` / skip `(13,28)` / `[NAV] Stuck at (13,27)`). Plant never started. Earlier same-day watch (`watch_20260814_150308.log`) had a separate L/R shed-route stall (`field_to_shed` from y=29–30 into the y=31 fence, ~3600f) — route picker now only uses that path south of the fence (`y>=32`). Residual tip: west-pocket bush/held-weed pin; then shed bag-into-carry; stump `(18,28)` needs axe.

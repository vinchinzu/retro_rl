...
| Active tip | D2 sections `rr-20w.2` (nav push/bush first); grape+shop+plant closed (`rr-zmss` / `rr-20w.1`); Gate B `rr-5in` → epic `rr-20w` |
...
| **Day-plan soft fails** | Early ENSURE_CAN / CROP_WATER after plant | `rr-rzpd` (D2 water under `rr-20w.2`) | Open |
| **D2 spring sections** | Nav + bush-first clear + hoe/plant/water pins (no 3–4h soak) | `rr-20w.2` | Open — ready `rr-20w.2.1` / `rr-20w.2.2` |
...
1. **`rr-20w.2.1` / `rr-20w.2.2`** — D2 travel nav: push-facing tile is non-walkable; bushes/weeds (`0x03`) are travel non-walkable. Pin + unit, not a 3–4h day.
2. **`rr-20w.2.3`** — bush-first west-pocket `CLEAR_PLOT` (blocked on 2.2). Then shared replan `rr-20w.2.4` → mountain hop / hoe / plant. Water is existing `rr-rzpd`. Compose soak `rr-20w.2.9` is last.
3. **`rr-3ae8` / `rr-5in`** — Gate B re-soak after CROP_WATER refill exhaust (power-on continuous; money>$100 to Summer).

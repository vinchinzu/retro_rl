...
| Active tip | `rr-5in` (Gate B soak: water/income) → epic `rr-20w` |
...
1. **`rr-5in`** — Gate B re-soak after CROP_WATER refill thrash fix (power-on continuous; money>$100 to Summer).
2. **`rr-rzpd`** — day-plan ENSURE_CAN/CROP_WATER soft fails after plant (related water path).

...
class DayPlanSequenceCropTests(unittest.TestCase):
    ...
    def test_buy_seeds_success_splices_same_day_plant(self) -> None:
        ...
        self.assertEqual(rocks.params.get("handoff"), "type_clear")
        self.assertNotIn("quota", rocks.params)
        stumps = next(p for p in plan._schedule.active if p.phase == "CLEAR_STUMPS")
        self.assertEqual(stumps.params.get("handoff"), "type_clear")
        self.assertNotIn("quota", stumps.params)

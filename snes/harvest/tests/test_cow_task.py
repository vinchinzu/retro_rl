"""Unit tests for CowChoresTask -- no ROM needed."""
...
def _set_player_tile(ram: np.ndarray, tile: tuple[int, int]) -> None:
    x = tile[0] * 16 + 8
    y = tile[1] * 16 + 8
    _write_u16(ram, ADDR_X, x)
    _write_u16(ram, ADDR_Y, y)


def _set_player_px(ram: np.ndarray, pos: tuple[int, int]) -> None:
    _write_u16(ram, ADDR_X, pos[0])
    _write_u16(ram, ADDR_Y, pos[1])
...
def _set_cow_slot(ram: np.ndarray, slot: int, tile: tuple[int, int], *, status: int = 0x05) -> None:
    ...
    x = tile[0] * 16 + 8
    y = tile[1] * 16 + 8
    _write_u16(ram, offset + 8, x)
    _write_u16(ram, offset + 10, y)


def _set_cow_daily(ram: np.ndarray, slot: int, *, flags: int, happiness: int) -> None:
    offset = COW_SLOT_BASE + slot * COW_SLOT_SIZE
    ram[offset + 1] = flags
...
def _make_barn_ram(
    *,
    ...
    _set_player_tile(ram, player_tile)
    for i in range(64 * 64):
        ram[ADDR_MAP + i] = 0xA1
    ram[ADDR_NUM_COWS] = cows
    ram[ADDR_FED_COWS_N] = fed
    _write_u16(ram, ADDR_STORED_GRASS, hay)
    ...
    for slot in range(cows):
        _set_cow_slot(ram, slot, (9 + slot, 17))
    return ram

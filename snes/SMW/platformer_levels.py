"""Super Mario World platformer LevelConfig segments (game-owned).

Registers into ``retro_harness.platformer.level_config.LEVEL_REGISTRY`` on
import. First-pass SMW integration for the shared platformer tooling:
stable-retro built-in level states and a RAM layout backed by SMWDisX
symbols. Chained any% states can be published under SMW later without

#!/usr/bin/env python3
"""Replay the model-free power-on Liu Kang Match 1+2 input tape."""
...
from mortal_kombat.natural_fight2_tape import (  # noqa: E402
...
NATURAL_THROUGH_FIGHT2_FRAMES = NATURAL_FIGHT1_FRAMES + NATURAL_FIGHT2_FRAMES
NATURAL_THROUGH_FIGHT2_RLE = NATURAL_FIGHT1_RLE + NATURAL_FIGHT2_RLE
...
def play_tape(env) -> tuple[object, int, dict[int, tuple[int, int]], str]:
    ...
    prev_health: dict[int, tuple[int, int] | None] = {0: None, 1: None}
    ...
    kos = {match: (p1_kos[match], p2_kos[match]) for match in (0, 1)}
...
def main() -> int:
    ...
    for attempt in range(args.repeat):
        ...
        try:
            ...
            env.close()
        transitioned = snap.match_counter >= 2 and snap.p1_character == LIU_KANG_ID
        ...
        print(
            ...
            f"identical={identical} frame={frame}/{NATURAL_THROUGH_FIGHT2_FRAMES} "

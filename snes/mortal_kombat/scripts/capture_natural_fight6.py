#!/usr/bin/env python3
"""Capture a model-free Match 6 continuation from the natural Fight 5 pin.

Replays NATURAL_THROUGH_FIGHT5_RLE from power-on, snapshots the emulator,
identifies the live Match 6 opponent (leftover HUD is ignored), then drives
Match 6 with RAM specialists first. Pixel ladder-ft is the fallback after
...
into Sub-Zero ice or Scorpion spear (Cage's shadow kick is the same trap).
Live Match 6 after Cage → Sonya → Sub-Zero → Raiden → Kano is Johnny Cage
again (id 0), not Scorpion; leftover pin HUD still shows Kano. Match 7
is next (identify before fighting; leftover Cage HUD is not the opponent).
"""
...
from mortal_kombat.scripts.replay_natural_fight5 import (  # noqa: E402
...
def format_rle(pairs: list[tuple[int, int]], width: int = 8) -> str:
    ...
    lines = ["NATURAL_FIGHT6_RLE: tuple[tuple[int, int], ...] = ("]
...
def replay_through_fight5(env) -> None:
    reset_obs(env)
    for mask, count in NATURAL_THROUGH_FIGHT5_RLE:
...
def identify_live_match6(env, pin, *, max_frames: int, screenshot: Path | None):
    """Idle through VS/load until Match 6 is fight-ready and on-screen.

    First fight-ready can still be a black fade with p2_character=0 (Cage).
    Wait for a visible frame so leftover/default HUD is not the opponent.
    """
    ...
    for frame in range(0 if is_fight_ready(last) and last.match_counter == MATCH6 else 1, max_frames + 1):
        if frame > 0:
        ...
        if last.match_counter == MATCH6 and is_fight_ready(last):
            if ready_at is None:
                ready_at = frame
                print(
    ...
    *,
    ...
    def on_frame(_env, frame, snap, _prev) -> bool:
        ...
        fight_started = fight_started or (
            snap.match_counter == MATCH6
            and snap.p1_character == LIU_KANG_ID
            and snap.timer > 50
            and health == (161, 161)
        )
        if fight_started and live_p2 is None and snap.match_counter == MATCH6:
        ...
        if fight_started and snap.match_counter == MATCH6 and prev_health is not None:
            p1_kos += int(prev_health[1] > 0 and health[1] == 0)
            p2_kos += int(prev_health[0] > 0 and health[0] == 0)
        prev_health = health if fight_started and snap.match_counter == MATCH6 else None
        if frame % 500 == 0:
            print(f"  f={frame} kos={p1_kos}-{p2_kos} {describe(snap)}", flush=True)
        if snap.screen is Screen.CONTINUE:
            return True
        return snap.match_counter >= MATCH7 and snap.p1_character == LIU_KANG_ID
    ...
    won = last_snap.match_counter >= MATCH7 and last_snap.p1_character == LIU_KANG_ID
    print(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("snes/mortal_kombat/scratch/natural_fight6_rle.py"),
    )
    parser.add_argument(
        "--screenshot",
        type=Path,
        default=Path("snes/mortal_kombat/recordings/natural_match6_start.png"),
    )
    ...
    try:
        print("replaying natural fight 1+2+3+4+5…", flush=True)
        replay_through_fight5(env)
        pin_snap = parse_ram(env.unwrapped.get_ram())
        print(f"pin {describe(pin_snap)}", flush=True)
        if pin_snap.match_counter < MATCH6 or pin_snap.p1_character != LIU_KANG_ID:
            print("fight 5 pin is not at Match 6")
            return 1
        pin = get_emulator_state(env.unwrapped)

        ident_frames, ident_snap = identify_live_match6(
            env,
            pin,
            max_frames=args.max_identify_frames,
            screenshot=args.screenshot,
        )
        live = ident_snap.match_counter == MATCH6 and is_fight_ready(ident_snap)
        print(
            f"identify live={live} frames={ident_frames} {describe(ident_snap)}",
            flush=True,
        )
        if not live:
            print("Match 6 fight never became ready; leftover HUD is not the opponent")
            return 1
        print(
            f"live Match 6 opponent is {ident_snap.p2_character}/"
            f"{char_name(ident_snap.p2_character)} "
            "(pin HUD may still show Kano)"
        )
        if args.identify_only:
            return 0

        # RAM specialists first. Match6 v3 is the Scorpion save-state zip;
        # Match5 v3 is the strongest generalist and closed Match 5 stochastically.
        # Pixel ladder-ft / Match7 pixel only after RAM oracles miss.
        attempts = (
        ...
        for label, ladder, scripted, pixel in attempts:
            for attempt in range(args.repeats):
                ...
                body = (
                    f"NATURAL_FIGHT6_FRAMES = {len(masks)}\n"
                ...
                return 0
        print("no oracle reached match_counter=6")
        return 1
    finally:
        env.close()

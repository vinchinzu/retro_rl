#!/usr/bin/env python3
"""Capture a model-free Match 7 continuation from the natural Fight 6 pin.

Replays NATURAL_THROUGH_FIGHT6_RLE from power-on, snapshots the emulator,
identifies the live Match 7 opponent (leftover HUD is ignored), then drives
Match 7 with RAM specialists first. Pixel Match7 / ladder-ft are fallbacks
after RAM oracles miss. Runtime artifacts are RLE only.
...
into Sub-Zero ice or Scorpion spear. Live Match 6 was Johnny Cage again
(id 0); leftover pin HUD still shows Cage. Live Match 7 is the Liu Kang
mirror (id 3). Wait for a visible fight frame — first fight-ready can
still be a black fade.
"""
...
def format_rle(pairs: list[tuple[int, int]], width: int = 8) -> str:
    ...
    lines = ["NATURAL_FIGHT7_RLE: tuple[tuple[int, int], ...] = ("]
...
def replay_through_fight6(env) -> None:
    reset_obs(env)
    for mask, count in NATURAL_THROUGH_FIGHT6_RLE:
...
def identify_live_match7(env, pin, *, max_frames: int, screenshot: Path | None):
    """Idle through VS/load until Match 7 is fight-ready and on-screen.
    ...
    already = last.match_counter == MATCH7 and is_fight_ready(last)
    for frame in range(0 if already else 1, max_frames + 1):
        ...
        if last.match_counter == MATCH7 and is_fight_ready(last):
            if ready_at is None:
                ready_at = frame
                print(
    ...
    *,
    ...
    def on_frame(_env, frame, snap, _prev) -> bool:
        ...
        fight_started = fight_started or (
            snap.match_counter == MATCH7
            and snap.p1_character == LIU_KANG_ID
            and snap.timer > 50
            and health == (161, 161)
        )
        if fight_started and live_p2 is None and snap.match_counter == MATCH7:
        ...
        if fight_started and snap.match_counter == MATCH7 and prev_health is not None:
            p1_kos += int(prev_health[1] > 0 and health[1] == 0)
            p2_kos += int(prev_health[0] > 0 and health[0] == 0)
        prev_health = health if fight_started and snap.match_counter == MATCH7 else None
        if frame % 500 == 0:
            print(f"  f={frame} kos={p1_kos}-{p2_kos} {describe(snap)}", flush=True)
        if snap.screen is Screen.CONTINUE:
            return True
        return snap.match_counter >= ENDURANCE1 and snap.p1_character == LIU_KANG_ID
    ...
    won = last_snap.match_counter >= ENDURANCE1 and last_snap.p1_character == LIU_KANG_ID
    print(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("snes/mortal_kombat/scratch/natural_fight7_rle.py"),
    )
    parser.add_argument(
        "--screenshot",
        type=Path,
        default=Path("snes/mortal_kombat/recordings/natural_match7_start.png"),
    )
    ...
    try:
        print("replaying natural fight 1+2+3+4+5+6…", flush=True)
        replay_through_fight6(env)
        pin_snap = parse_ram(env.unwrapped.get_ram())
        print(f"pin {describe(pin_snap)}", flush=True)
        if pin_snap.match_counter < MATCH7 or pin_snap.p1_character != LIU_KANG_ID:
            print("fight 6 pin is not at Match 7")
            return 1
        pin = get_emulator_state(env.unwrapped)

        ident_frames, ident_snap = identify_live_match7(
            env,
            pin,
            max_frames=args.max_identify_frames,
            screenshot=args.screenshot,
        )
        live = ident_snap.match_counter == MATCH7 and is_fight_ready(ident_snap)
        print(
            f"identify live={live} frames={ident_frames} {describe(ident_snap)}",
            flush=True,
        )
        if not live:
            print("Match 7 fight never became ready; leftover HUD is not the opponent")
            return 1
        print(
            f"live Match 7 opponent is {ident_snap.p2_character}/"
            f"{char_name(ident_snap.p2_character)} "
            "(pin HUD may still show Johnny Cage)"
        )
        if args.identify_only:
            return 0

        # Match5 v3 closed Match 6 det and has the first v3 Match7 save-state
        # wins (2/5). Match7 v3 final was 0/5; pixel Match7 / ladder-ft after
        # RAM oracles miss. Stochastic repeats closed Match 5.
        attempts = (
        ...
        for label, ladder, scripted, pixel in attempts:
            for attempt in range(args.repeats):
                ...
                body = (
                    f"NATURAL_FIGHT7_FRAMES = {len(masks)}\n"
                ...
                return 0
        print("no oracle reached match_counter=7")
        return 1
    finally:
        env.close()

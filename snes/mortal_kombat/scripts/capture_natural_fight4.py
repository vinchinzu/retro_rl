#!/usr/bin/env python3
"""Capture a model-free Match 4 continuation from the natural Fight 3 pin.

Replays NATURAL_THROUGH_FIGHT3_RLE from power-on, snapshots the emulator,
identifies the live Match 4 opponent (leftover HUD is ignored), then drives
Match 4 with the Fight 2/3 oracle family. Runtime artifacts are RLE only.

Likely opponent after Cage → Sonya → Sub-Zero is Scorpion. Liu Kang CPU
walkthrough: jump-kick into flying kick (F,F,HK), fireball (F,F,HP) on
wakeup. Block Scorpion spear; do not jump into it.
"""
...
from mortal_kombat.scripts.replay_natural_fight3 import (  # noqa: E402
...
def format_rle(pairs: list[tuple[int, int]], width: int = 8) -> str:
    ...
    lines = ["NATURAL_FIGHT4_RLE: tuple[tuple[int, int], ...] = ("]
...
def replay_through_fight3(env) -> None:
    reset_obs(env)
    for mask, count in NATURAL_THROUGH_FIGHT3_RLE:
...
def identify_live_match4(env, pin, *, max_frames: int, screenshot: Path | None):
    """Idle through VS/load until Match 4 is actually fight-ready."""
    ...
    if last.match_counter == MATCH4 and is_fight_ready(last):
    ...
    *,
    ...
    def on_frame(_env, frame, snap, _prev) -> bool:
        ...
        fight_started = fight_started or (
            snap.match_counter == MATCH4
            and snap.p1_character == LIU_KANG_ID
            and snap.timer > 50
            and health == (161, 161)
        )
        if fight_started and live_p2 is None and snap.match_counter == MATCH4:
        ...
        if fight_started and snap.match_counter == MATCH4 and prev_health is not None:
            p1_kos += int(prev_health[1] > 0 and health[1] == 0)
            p2_kos += int(prev_health[0] > 0 and health[0] == 0)
        prev_health = health if fight_started and snap.match_counter == MATCH4 else None
        if frame % 500 == 0:
            print(f"  f={frame} kos={p1_kos}-{p2_kos} {describe(snap)}", flush=True)
        if snap.screen is Screen.CONTINUE:
            return True
        return snap.match_counter >= MATCH5 and snap.p1_character == LIU_KANG_ID
    ...
    won = last_snap.match_counter >= MATCH5 and last_snap.p1_character == LIU_KANG_ID
    print(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("snes/mortal_kombat/scratch/natural_fight4_rle.py"),
    )
    parser.add_argument(
        "--screenshot",
        type=Path,
        default=Path("snes/mortal_kombat/recordings/natural_match4_start.png"),
    )
    ...
    try:
        print("replaying natural fight 1+2+3…", flush=True)
        replay_through_fight3(env)
        pin_snap = parse_ram(env.unwrapped.get_ram())
        print(f"pin {describe(pin_snap)}", flush=True)
        if pin_snap.match_counter < MATCH4 or pin_snap.p1_character != LIU_KANG_ID:
            print("fight 3 pin is not at Match 4")
            return 1
        pin = get_emulator_state(env.unwrapped)

        ident_frames, ident_snap = identify_live_match4(
            env,
            pin,
            max_frames=args.max_identify_frames,
            screenshot=args.screenshot,
        )
        live = ident_snap.match_counter == MATCH4 and is_fight_ready(ident_snap)
        print(
            f"identify live={live} frames={ident_frames} {describe(ident_snap)}",
            flush=True,
        )
        if not live:
            print("Match 4 fight never became ready; leftover HUD is not the opponent")
            return 1
        print(
            f"live Match 4 opponent is {ident_snap.p2_character}/"
            f"{char_name(ident_snap.p2_character)} "
            "(pin HUD may still show Sub-Zero)"
        )
        if args.identify_only:
            return 0

        attempts = (
            ("match5-ladder", "mk1_v3_Match5_ppo_final.zip", False, None),
        ...
        for label, ladder, scripted, pixel in attempts:
            ...
            body = (
                f"NATURAL_FIGHT4_FRAMES = {len(masks)}\n"
            ...
            return 0
        print("no oracle reached match_counter=4")
        return 1
    finally:
        env.close()

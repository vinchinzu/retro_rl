...
| Spazer | **Early detour (P2)** after Below Spazer; walljump path | Collect `0xA447` → secondary tip → fold into default spine for 100%; epic [TRACK_100.md](TRACK_100.md) |
...
| Evidence | `recordings/supers.json` (prefix of red-tower run) |
...
| Evidence | `recordings/red_tower.json` (**80,445** frames, integrity green) |
| Code | `run_to("red_tower")` / `run_to("red_tower")` / `routes/kpdr/` Super→farm→Big Pink main→GHZ→Noob→Red |
...
uv run python snes/super_metroid/scripts/probe/kpdr.py pure noob-to-red \
...
| Continuous evidence | `recordings/kraid.json` (**97,170** frames, integrity green); prefixes: Hi-Jump 87,696f; Warehouse 83,512f; Bat 81,652f |
...
```bash
uv run python snes/super_metroid/scripts/probe/kpdr.py pure red-to-warehouse \
...
| Field | Value |
|-------|-------|
| Status | **continuous through Bat Cave** (primary tip; Frog Save is side tip only); first Bubble path **repathed** |
...
| Graph | `START_TO_SPEED_GRAPH` — Varia return + Business→Frog **continuous** (side); Cathedral climb + Bubble→Bat **continuous** (on tip); Speed Hall+ still `unverified` |
| Continuous evidence | `recordings/bat_cave*.json` (**122,304f** twice; 0 loads / progression / capacity / deaths); Frog side tip `frog*.json` (114,923f ×2) |
| Pure stack | CATH-01…04 GREEN; **Bubble → Bat pure GREEN R19** (2012f → `0xB07A`; see [STATUS.md](../STATUS.md)) |
| ★ First open hop | Bat → Speed Hall pure from `scratch/post_bat_cave_continuous.state` / `post_bubble_to_bat_pure.state` |
...
| K4 | Frog / Speed / Wave / Ice | **Bat Cave** | partial | ★ Next: Bat → Speed Hall pure (Speedway post-Speed parked) |
...
1. **Continuous through Bat Cave done** (`bat_cave`, 122,304f integrity green twice; Frog Save remains side tip at 114,923f).
2. **★ Next pure:** Bat → Speed Hall from `post_bat_cave_continuous` / `post_bubble_to_bat_pure`;
   Cathedral + Bubble→Bat pure stack already green. Frog Speedway is post-Speed only (`SM-K4.2-PURE` RED without Speed).
...
| Anchor | Keep for |
|--------|----------|
| `natural_post_spore_spawn` / supers | Continuous prefix |

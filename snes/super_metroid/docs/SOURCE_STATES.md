...
| `post_ws_poweron` | `scratch/post_ws_poweron.state` | `0xCA08` WS Entrance ~(57,139) p1 | power-on `--to ws` dual **176141f** ×2 (rr-p2bw, scratch; not STATUS); items `0x3105` beams `0x1007` PB 5/5 gs=8 | ship interior / Phantoon |
| `post_ws_entrance_to_main` | `scratch/post_ws_entrance_to_main.state` | `0xCAF6` WS Main Shaft ~(1063,907) p9 | pure dual GREEN Entrance→Main (rr-ahjo, **403f** ×2); items `0x3105` beams `0x1007` gs=8 | Main Shaft → basement |
| `post_ws_main_to_basement` | `scratch/post_ws_main_to_basement.state` | `0xCC6F` WS Basement ~(657,92) p24 | pure dual GREEN Main Shaft→basement (rr-4btp, **1208f** ×2); items `0x3105` beams `0x1007` gs=8 sel=2 | Basement → Phantoon |
| `post_ws_basement_to_phantoon` | `scratch/post_ws_basement_to_phantoon.state` | `0xCD13` Phantoon's Room ~(39,124) p81 | pure dual GREEN Basement→Phantoon room (rr-cjpp, **718f** ×2); items `0x3105` beams `0x1007` gs=8 sel=2; **not** fight | Phantoon fight |
| `post_phantoon_poweron` | `scratch/post_phantoon_poweron.state` | `0xCD13` Phantoon's Room ~(37,187) p1 | assist dual-green charge fight **20537f** ×2 (rr-tlaq); HP 0, `$D82B` bit 0, gs=8, health 299; **research** (spine is now doppler); do not clobber `post_phantoon_defeated` | in-room kill leave |
| `post_phantoon_leave` | `scratch/post_phantoon_leave.state` | `0xCC6F` WS Basement ~(1240,139) p10 | living-tip leave (STATUS Phantoon); doppler + loot/exit dual **12455f** ×2; `$D82B` bit 0, gs=8 | Gravity `rr-kw8t` |
...
| Landing Site shine practice | `0x91F8` | `scratch/landing_site_speed_practice.state` ~(899,1163) items `0x3105` (not escape `0xF32F`) | `kpdr.py pure` + `routes/skills/shinespark.py` |

...
### Result
PARTIAL — stack beads split from human tape Phase B; first **three** hops
dual pure GREEN (Wave→Double, Double→Single, Single→Bubble). Remaining 4 hops
...
`double_to_single`, `single_to_bubble`, + geometry `WAVE_*` / `DTS_*` /
`STB_*`). One module per hop until multi-room reverse solidifies.
...
- `routes/kpdr/wave/geometry.py` — `WAVE_*` + `DTS_*` + `STB_*` bands
- `routes/kpdr/wave/__init__.py`, `k4_wave.py`, `k4_norfair.py`, `registry.py`
- `scripts/probe/kpdr.py` — pure CLI segments
- `source_states.py` — `dev_wave_collected`, post Wave/Double/Single handoffs
...
- **Next card ID:** `rr-czg9` — Pure Bubble → Upper Norfair Farm

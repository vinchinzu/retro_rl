...
def play_red_tower_to_bat(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_RED_TOWER, "red_to_bat")
    unmorph(session)
    ...
    for _ in range(1600):
        ...
        if state.samus_x >= 225:
            ...
            direction = "RIGHT"
        hold(session, 1, direction, "B", reason="red_upper_zigzag")
    else:
    ...
    for _ in range(100):
        ...
        hold(session, 1, direction, reason="red_floor_bomb_position")
    hold(session, 2, "X", reason="red_floor_bomb")
    hold(session, 8, "LEFT", reason="red_floor_bomb_retreat")
    hold(session, 40, reason="red_floor_bomb_wait")
    for _ in range(180):
        state = hold(session, 1, "RIGHT", reason="red_floor_cross")
    ...
    for frame in range(500):
        ...
        state = hold(session, 1, *buttons, reason="red_tunnel_bomb_roll")
    ...
    for _ in range(300):
        state = hold(session, 1, "RIGHT", reason="red_tunnel_right")
    ...
    for _ in range(600):
        ...
        if state.samus_x >= 220:
            ...
            direction = "RIGHT"
        hold(session, 1, direction, reason="red_lower_zigzag")
    else:
    ...
    for frame in range(420):
        ...
        if phase < 5:
            ...
            buttons = ("RIGHT", "B")
        state = hold(session, 1, *buttons, reason="red_bottom_exit")
    ...
    for frame in range(240):
        state = hold(session, 1, "LEFT", reason="red_to_bat_brake")
        if (
        ...
        if (
            ...
            return state
    return wait_ordinary_room(session, ROOM_BAT, settle_frames=60, label="red_to_bat")
...
def play_bat_to_below_spazer(session: ControllerSession) -> SuperMetroidState:
    ...
    """
    require_room(session, ROOM_BAT, "bat_to_below_spazer")
    unmorph(session)
    select_weapon(session, 0)
    ...
    if state.samus_y > 125 or abs(state.velocity_y) > 0:
        for _ in range(60):
            state = hold(session, 1, reason="bat_land_wait")
    ...
    if session.state.samus_y <= 125:
        ...
        hold(session, 5, reason="bat_entry_glide")
        hold(session, 35, "RIGHT", "B", reason="bat_first_runup")
        hold(session, 60, "RIGHT", "B", "A", reason="bat_first_jump")
        settle_hold(session, 30, reason="bat_first_land")
    else:
        # Low left ledge / post-fall continuous entry.
        hold(session, 8, reason="bat_low_ready")
        hold(session, 15, "RIGHT", "B", reason="bat_low_runup")
        hold(session, 80, "RIGHT", "B", "A", reason="bat_low_jump")
    ...
    hold(session, 8, "RIGHT", "B", reason="bat_second_runup")
    hold(session, 20, "RIGHT", "B", "A", reason="bat_second_jump")
    ...
    hold(session, 48, "RIGHT", "B", "A", reason="bat_third_jump")
...
def play_east_to_warehouse(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_EAST_TUNNEL, "east_to_warehouse")
    for frame in range(1600):
        ...
        if phase < 5:
            ...
            buttons = ("RIGHT", "B")
        state = hold(session, 1, *buttons, reason="east_tunnel_right")

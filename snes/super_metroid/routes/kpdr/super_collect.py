...
def play_super_room_collect(session: ControllerSession) -> SuperCollectEvidence:
    ...
    for _ in range(80):
        state = hold(session, 1, "RIGHT", "B", reason="super_shaft_approach")
        if state.samus_x >= 140:
            break

    # Crouch-shot floor blocks.
    for _ in range(15):
        hold(session, 3, "DOWN", "X", reason="super_shaft_shot")
        hold(session, 2, "DOWN", reason="super_shaft_shot")
    hold(session, 10, "DOWN", reason="super_shaft_morph")
    for _ in range(8):
        hold(session, 2, "A", reason="super_shaft_bomb")
        hold(session, 30, reason="super_shaft_bomb_wait")

    # Jump right into the cleared shaft path.
    for _ in range(50):
        state = hold(session, 2, "RIGHT", "A", "B", reason="super_shaft_jump")
    ...
    for i in range(200):
        phase = i % 10
        if phase < 3:
            state = hold(session, 4, "RIGHT", "B", "X", reason="super_shaft_explore")
        elif phase < 5:
            state = hold(session, 4, "RIGHT", "A", "B", reason="super_shaft_explore")
        elif phase < 7:
            state = hold(session, 3, "DOWN", "X", reason="super_shaft_explore")
        elif phase < 8:
            hold(session, 8, "DOWN", reason="super_shaft_explore")
            hold(session, 2, "A", reason="super_shaft_explore")
            state = hold(session, 20, reason="super_shaft_explore")
        else:
            state = hold(session, 4, "RIGHT", "B", reason="super_shaft_explore")
        if state.samus_y > 500:
            break

    for _ in range(800):
        state = hold(session, 2, reason="super_shaft_fall")
    ...
    for i in range(400):
        if state.samus_x < 412:
            state = hold(session, 2, "RIGHT", "B", reason="super_item_approach")
        elif state.samus_x > 428:
            state = hold(session, 2, "LEFT", "B", reason="super_item_approach")
        else:
            state = hold(session, 2, reason="super_item_approach")
        if i % 12 == 0:
            state = hold(session, 4, "X", reason="super_item_shoot")
        if i % 40 == 20:
            state = hold(session, 6, "A", reason="super_item_jump")
        if state.max_super_missiles > 0:
    ...
    for i in range(300):
        state = hold(session, 1, reason="super_item_fanfare")
        if (
            state.game_state == 8
...
def play_super_room_to_farming(session: ControllerSession) -> SuperMetroidState:
    ...
    state = session.state
    require_room(session, ROOM_SUPER, "to_farming")
    if state.max_super_missiles <= 0:
    ...
    for _ in range(80):
        state = hold(session, 2, "LEFT", "B", reason="super_gate_approach")
        if state.samus_x <= 320:
            break

    hold(session, 12, "DOWN", reason="super_gate_bomb")
    for _ in range(15):
        hold(session, 2, "A", reason="super_gate_bomb")
        hold(session, 35, reason="super_gate_bomb")
        state = hold(session, 4, "LEFT", reason="super_gate_bomb")
        if state.samus_x < 200:
            break
    unmorph(session)

    for _ in range(120):
        state = hold(session, 2, "LEFT", "B", reason="super_door_approach")
        if state.samus_x < 50:
            break

    for _ in range(50):
        hold(session, 3, "LEFT", "X", reason="super_door_shot")
        state = hold(session, 5, "LEFT", "B", reason="super_door_enter")
        if state.room_id == ROOM_FARMING:
            break
    for _ in range(200):
        state = hold(session, 1, reason="farming_settle")
        if (
            ...
            break
    require_room(session, ROOM_FARMING, "farming entry")
    return state
...
def play_farming_to_big_pink(session: ControllerSession) -> SuperMetroidState:
    ...
    state = session.state
    require_room(session, ROOM_FARMING, "farming_to_pink")
    try:
    ...
    for i in range(500):
        if session.state.pose in (39, 40, 137, 138):
            unmorph(session)
        state = hold(session, 3, "LEFT", "A", "B", reason="farming_cross")
        if i % 5 == 0:
            hold(session, 2, "LEFT", "X", reason="farming_super")
        if i % 25 == 12:
            hold(session, 8, "DOWN", reason="farming_bomb")
            hold(session, 2, "A", reason="farming_bomb")
            hold(session, 30, reason="farming_bomb")
            unmorph(session)
        if state.room_id == ROOM_BIG_PINK:
            break
    for _ in range(150):
        state = hold(session, 1, reason="big_pink_settle")
        if (
            ...
            break
    require_room(session, ROOM_BIG_PINK, "big pink entry")
    return state

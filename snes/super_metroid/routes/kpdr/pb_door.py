...
def play_big_pink_enter_pb_door_from_sill(
    ...
    *,
    ...
    """
    require_room(session, ROOM_BIG_PINK, "enter_pb_door_from_sill")
    state = session.state
    # Run-shoot opens blue door; spin carries through alcove.
    hold(session, 10, "LEFT", "B", reason="pb_door_run")
    hold(session, 4, "LEFT", "B", "X", reason="pb_door_shoot")
    hold(session, 30, "LEFT", "B", "A", reason="pb_door_spin")
    entered = False
    for _ in range(120):
        state = hold(session, 1, "LEFT", reason="pb_door_hold")
    ...
    for frame in range(settle_frames):
        state = hold(session, 1, reason="pb_door_settle")
        if (

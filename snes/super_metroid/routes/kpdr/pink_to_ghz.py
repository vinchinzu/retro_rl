...
def play_big_pink_to_ghz(session: ControllerSession) -> SuperMetroidState:
    ...
    for _ in range(500):
        state = hold(session, 1, "LEFT", reason="big_pink_lower_left")
    ...
    unmorph(session)
    for _ in range(220):
        state = hold(session, 1, "RIGHT", "B", "A", reason="big_pink_lower_drop")
        if state.samus_x >= 665 and state.samus_y >= 1660:
            break
    else:
        raise TimeoutError(f"big_pink_to_ghz: missed lower mass: {state}")

    hold(session, 30, "RIGHT", "B", reason="big_pink_mass_run")
    ...
    hold(session, 50, reason="big_pink_tunnel_lip_settle")
    ensure_morph(session)
    ...
    for _ in range(220):
        state = hold(session, 1, "RIGHT", reason="big_pink_door_roll")
    ...
    hold(session, 25, "LEFT", reason="big_pink_super_standoff")
    hold(session, 3, "RIGHT", reason="big_pink_face_door")
    hold(session, 3, reason="big_pink_face_door_release")
    hold(session, 2, "RIGHT", "X", reason="big_pink_green_door_super")
    hold(session, 60, reason="big_pink_green_door_fuse")
    for _ in range(300):

...
# 1019→827: plant (1098, 1019) p9, B+RIGHT up the slope to wall (1243, 907)
# p137 then LEFT+A. Leftover (1187, 817) p48 is a 1019 spin, not a 827 plant.
# Tape take02 lands (1205, 827) p10.
SLOPE_1019_Y = 1019
SLOPE_1019_X = (1060, 1256)
SLOPE_1019_SEAT_Y = (900, 1036)
SLOPE_1019_TAKEOFF = TakeoffWindow((1240, 1246), "LEFT", min_momentum=0)
SLOPE_1019_AIR_Y = (800, 920)
SLOPE_1019_WALL_X = 1240

# 827→mid_climb: plant (1205, 827) p10, B+LEFT to wall (1061, 763) p138
# then A / RIGHT+A. Jump-on-land from (1188, 1220) LEFT+A hops off p165.
SLOPE_827_Y = 827
SLOPE_827_X = (1048, 1232)
SLOPE_827_SEAT_Y = (756, 840)
SLOPE_827_TAKEOFF = TakeoffWindow((1058, 1064), "RIGHT", min_momentum=0)
SLOPE_827_AIR_Y = (655, 780)
SLOPE_827_WALL_X = 1064

# 651→attic: plant (1098, 651) p9, B+RIGHT up the slope to wall (1243, 587)
# then LEFT to ~(1231, 587) LEFT+A. Wall-hug p137+A leftover (1240, 587)
# p38 is turning, not attic_seat. Tape take02 peaks ~493 then plants 523.
SLOPE_651_Y = 651
SLOPE_651_X = (1048, 1256)
SLOPE_651_SEAT_Y = (576, 660)
SLOPE_651_TAKEOFF = TakeoffWindow((1228, 1234), "LEFT", min_momentum=0)
SLOPE_651_AIR_Y = (490, 590)
SLOPE_651_WALL_X = 1240
# 587 wall: take02 5f UP+X / 6f UP. Three near-Samus shot-blocks before jump.
WALL_SHOT_FIRE = 5
WALL_SHOT_GAP = 6
UPPER_WALL_CLEAR = 3
UPPER_WALL_SHOT_X = 1228
# 523 ledge after the 587 jump. Take02 plants ~(1204, 523), LEFT+X to ~1077.
# Bounce-lane Atomics at (1084, 514) eat LEFT+A. p138 LEFT+A peaks 499;
# take03 UP+A from 1077 clears 488 onto 443.
SLOPE_523_Y = 523
SLOPE_523_SEAT_Y = (508, 540)
SLOPE_523_TAKEOFF_X = (1074, 1080)
SLOPE_523_FIRE = 5
SLOPE_523_GAP = 5

# Successful takes 02/03 use this upper ladder after the 523 gun-jump:
# right to 443, right to 363, left to 267, left to 171, then right to the
# ceiling-door seat at y=91.  These are real seats, not minimum-y records.
UPPER_SHAFT_HOPS: tuple[PlatformHop, ...] = (
    PlatformHop(
        443, 1061, 1170, TakeoffWindow((1110, 1140), "RIGHT", min_momentum=0)
    ),
    PlatformHop(
        363, 1180, 1243, TakeoffWindow((1210, 1234), "RIGHT", min_momentum=0)
    ),
    PlatformHop(
        267, 1140, 1170, TakeoffWindow((1150, 1164), "LEFT", min_momentum=0)
    ),
    PlatformHop(
        171, 1077, 1120, TakeoffWindow((1077, 1084), "LEFT", min_momentum=0)
    ),
    PlatformHop(
        91, 1111, 1159, TakeoffWindow((1120, 1140), "RIGHT", min_momentum=0)
    ),
)
...
SHAFT_HOPS: tuple[PlatformHop, ...] = (
    ...
    PlatformHop(
        SLOPE_1019_Y, SLOPE_1019_X[0], SLOPE_1019_X[1], SLOPE_1019_TAKEOFF
    ),
    PlatformHop(
        SLOPE_827_Y, SLOPE_827_X[0], SLOPE_827_X[1], SLOPE_827_TAKEOFF
    ),
    PlatformHop(
        SLOPE_651_Y, SLOPE_651_X[0], SLOPE_651_X[1], SLOPE_651_TAKEOFF
    ),
    *UPPER_SHAFT_HOPS,
)
...
def at_ws_main_slope_1019(
    samus_x: int, samus_y: int, pose: int, velocity_y: int = 0
) -> bool:
    """Planted 1019 slope through the 907 wall. Leftover (1187, 817) p48 is out."""
    x, y = int(samus_x), int(samus_y)
    return (
        SLOPE_1019_X[0] <= x <= SLOPE_1019_X[1]
        and SLOPE_1019_SEAT_Y[0] <= y <= SLOPE_1019_SEAT_Y[1]
        and int(pose) in GROUNDED_POSES | TURN_POSES
        and abs(int(velocity_y)) <= 1
    )


def at_ws_main_slope_827(
    samus_x: int, samus_y: int, pose: int, velocity_y: int = 0
) -> bool:
    """Planted 827 slope through the 763 wall. Jump-on-land p165+A is out."""
    x, y = int(samus_x), int(samus_y)
    return (
        SLOPE_827_X[0] <= x <= SLOPE_827_X[1]
        and SLOPE_827_SEAT_Y[0] <= y <= SLOPE_827_SEAT_Y[1]
        and int(pose) in GROUNDED_POSES | TURN_POSES | {164, 165}
        and abs(int(velocity_y)) <= 1
    )


def at_ws_main_slope_651(
    samus_x: int, samus_y: int, pose: int, velocity_y: int = 0
) -> bool:
    """Planted 651 slope through the 587 wall. Leftover (1061, 752) p77 is out."""
    x, y = int(samus_x), int(samus_y)
    return (
        SLOPE_651_X[0] <= x <= SLOPE_651_X[1]
        and SLOPE_651_SEAT_Y[0] <= y <= SLOPE_651_SEAT_Y[1]
        and int(pose) in GROUNDED_POSES | TURN_POSES | {164, 165, 166, 167}
        and abs(int(velocity_y)) <= 1
    )


def at_ws_main_mid_climb(state: SuperMetroidState) -> bool:
    """Planted mid-shaft seat y~640–680. Airborne leftover (1117, 640) p47 is out."""
    if int(state.room_id) in (ROOM_WS_WEST_SUPER, ROOM_WS_SAVE, ROOM_WS_ATTIC):
        return False
    x, y = int(state.samus_x), int(state.samus_y)
    pose = int(state.pose)

...
def play_warehouse_to_zeela_with_hijump(
    ...
    settle_hold(session, 12, reason="warehouse_hj_release")
    for _ in range(120):
        if session.state.samus_x <= 445:
            break
        hold(session, 1, "LEFT", reason="warehouse_hj_backoff")
    hold(session, 5, "RIGHT", reason="warehouse_hj_brake")
    settle_hold(session, 8, reason="warehouse_hj_jump_release")
    for frame in range(180):
        buttons = ("A",) if frame < 25 else ("RIGHT", "B", "A")
        state = hold(session, 1, *buttons, reason="warehouse_hj_climb")
        if state.samus_x >= 720 and state.samus_y <= 160:
            break
    settle_hold(session, 30, reason="warehouse_hj_door_settle")
    unmorph(session)
    select_weapon(session, 0)
    hold(session, 3, "RIGHT", reason="warehouse_face_zeela")
    settle_hold(session, 3, reason="warehouse_face_zeela_release")
    hold(session, 2, "RIGHT", "X", reason="warehouse_zeela_door_shot")
    settle_hold(session, 30, reason="warehouse_zeela_door_open")
    for _ in range(420):
        state = hold(session, 1, "RIGHT", "B", "A", reason="warehouse_enter_zeela")
...
def play_zeela_to_kihunter(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_ZEELA, "zeela_to_kihunter")
    unmorph(session)
    select_weapon(session, 0)
    settle_hold(session, 10, reason="zeela_entry_release")
    hold(session, 10, "A", reason="zeela_first_drop_jump")
    hold(session, 1, "DOWN", reason="zeela_first_drop_aim")
    hold(session, 2, "X", reason="zeela_first_drop_shot")
    settle_hold(session, 80, reason="zeela_first_drop")
    ensure_morph(session)
    for _ in range(300):
        state = hold(session, 1, "RIGHT", reason="zeela_middle_roll")
        if state.samus_x >= 105 and state.samus_y >= 325:
            break
    unmorph(session)
    settle_hold(session, 30, reason="zeela_middle_land")
    select_weapon(session, 0)
    hold(session, 8, "A", reason="zeela_second_drop_jump")
    hold(session, 1, "DOWN", reason="zeela_second_drop_aim")
    hold(session, 2, "X", reason="zeela_second_drop_shot")
    for _ in range(180):
        state = hold(session, 1, "LEFT", reason="zeela_second_drop")
        if state.samus_y >= 395:
            break
    settle_hold(session, 40, reason="zeela_bottom_land")
    ensure_morph(session)
    for frame in range(700):
        buttons = ("RIGHT", "X") if frame % 45 < 2 else ("RIGHT",)
        state = hold(session, 1, *buttons, reason="zeela_bottom_bomb_roll")
        if state.samus_x >= 400:
            break
    else:
        raise TimeoutError(f"zeela_to_kihunter: tunnel stalled: {state}")
    unmorph(session)
    settle_hold(session, 40, reason="zeela_up_door_stand")
    select_weapon(session, 0)
    hold(session, 2, "UP", reason="zeela_up_door_aim")
    hold(session, 2, "UP", "X", reason="zeela_up_door_shot")
    settle_hold(session, 35, reason="zeela_up_door_open")
    for _ in range(400):
        state = hold(session, 1, "A", reason="zeela_enter_kihunter")
...
def play_kihunter_to_baby_kraid(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_WAREHOUSE_KIHUNTER, "kihunter_to_baby")
    settle_hold(session, 80, reason="kihunter_entry_floor")
    unmorph(session)
    select_weapon(session, 0)
    for _ in range(300):
        if session.state.samus_x >= 350:
            break
        hold(session, 1, "RIGHT", "B", reason="kihunter_drop_position")
    hold(session, 6, "LEFT", reason="kihunter_drop_brake")
    settle_hold(session, 10, reason="kihunter_drop_settle")
    hold(session, 3, "RIGHT", reason="kihunter_drop_exact")
    hold(session, 2, "LEFT", reason="kihunter_drop_exact_brake")
    settle_hold(session, 10, reason="kihunter_drop_exact_settle")
    ensure_morph(session)
    hold(session, 2, "X", reason="kihunter_floor_bomb")
    settle_hold(session, 55, reason="kihunter_floor_bomb_wait")
    hold(session, 2, "X", reason="kihunter_floor_bomb2")
    for _ in range(180):
        state = hold(session, 1, reason="kihunter_floor_drop")
        if state.samus_y >= 310:
            break
    ensure_morph(session)
    for _ in range(160):
        state = hold(session, 1, "LEFT", reason="kihunter_shaft_align")
        if state.samus_y >= 350:
            break
    for _ in range(360):
        state = hold(session, 1, "RIGHT", reason="kihunter_lower_roll")
    ...
    for frame in range(500):
        ...
        state = hold(session, 1, *buttons, reason="kihunter_enter_baby")
    ...
    *,
    ...
    for frame in range(limit):
        ...
        if phase < 3:
            ...
            buttons = (direction, "B")
        state = hold(session, 1, *buttons, reason=label)
...
def play_baby_kraid_to_eye(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_BABY_KRAID, "baby_kraid_to_eye")
    settle_hold(session, 100, reason="baby_kraid_entry_floor")
    unmorph(session)
    ...
    for _ in range(600):
        state = hold(session, 1, "RIGHT", "B", "A", reason="baby_kraid_enter_eye")
...
def play_eye_to_kraid(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_KRAID_EYE, "eye_to_kraid")
    settle_hold(session, 100, reason="kraid_eye_entry_floor")
    unmorph(session)
    select_weapon(session, 2)
    for frame in range(1800):
        ...
        if phase < 3:
            ...
            buttons = ("RIGHT", "B")
        state = hold(session, 1, *buttons, reason="kraid_eye_run")
...
def play_kraid_entry_to_varia(session: ControllerSession) -> SuperMetroidState:
    ...
    require_room(session, ROOM_KRAID, "kraid_entry_to_varia")

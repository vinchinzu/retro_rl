...
def play_big_pink_crest_pocket(session: ControllerSession) -> SuperMetroidState:
    ...
    """
    state = session.state
    require_room(session, ROOM_BIG_PINK, "crest_pocket")
    unmorph(session)
    try:
    ...
    for _ in range(40):
        state = hold(session, 1, "LEFT", "B", reason="big_pink_pocket_approach")
        if state.samus_x <= 1160:
            break
    hold(session, 25, "RIGHT", "B", reason="big_pink_pocket_runup")

    for _ in range(40):
        state = hold(session, 12, "LEFT", "A", "B", reason="big_pink_pocket_crest")
        state = hold(session, 6, "LEFT", "B", reason="big_pink_pocket_crest")
...
def play_big_pink_clear_super_block(session: ControllerSession) -> SuperMetroidState:
    ...
    """
    state = session.state
    require_room(session, ROOM_BIG_PINK, "clear_super_block")
    # Unspin from crest land (pose often 0x8A ran-into-wall).
    hold(session, 4, "A", reason="big_pink_unspin")
    settle_hold(session, 40, reason="big_pink_unspin_settle")
    try:
        select_weapon(session, 2)
    except RuntimeError:
        pass
    hold(session, 15, "DOWN", reason="big_pink_crouch_super")
    for _ in range(8):
        hold(session, 3, "LEFT", "X", reason="big_pink_crouch_super")
        state = hold(session, 18, "LEFT", "DOWN", reason="big_pink_crouch_super")
...
def play_big_pink_morph_to_tunnel(session: ControllerSession) -> SuperMetroidState:
    ...
    """
    require_room(session, ROOM_BIG_PINK, "morph_to_tunnel")
    # Leave crouch/super pose if still holding down from the Super clear.
    hold(session, 12, reason="big_pink_morph_stand")
    # Double-tap DOWN: crouch then morph (hold-DOWN alone stays crouch).
    hold(session, 1, "DOWN", reason="big_pink_morph_tap1")
    hold(session, 4, reason="big_pink_morph_gap")
    hold(session, 1, "DOWN", reason="big_pink_morph_tap2")
    state = hold(session, 18, "DOWN", reason="big_pink_morph_hold")
    on_tunnel = 1395 <= state.samus_y <= 1410 and state.samus_x <= 1155
    if not on_tunnel:
        # One retry with a slightly longer second tap.
        hold(session, 8, reason="big_pink_morph_retry_stand")
        hold(session, 2, "DOWN", reason="big_pink_morph_retry_tap1")
        hold(session, 4, reason="big_pink_morph_retry_gap")
        hold(session, 8, "DOWN", reason="big_pink_morph_retry_tap2")
        state = hold(session, 20, "DOWN", reason="big_pink_morph_retry_hold")
        on_tunnel = 1395 <= state.samus_y <= 1410 and state.samus_x <= 1155
    if not on_tunnel:
        raise TimeoutError(
...
def play_big_pink_tunnel_west(
    ...
    *,
    ...
    """
    state = session.state
    require_room(session, ROOM_BIG_PINK, "tunnel_west")
    try:
        select_weapon(session, 0)
    except RuntimeError:
        pass
    # Already morphed after morph_to_tunnel; short DOWN keeps ball if needed.
    hold(session, 8, "DOWN", reason="big_pink_tunnel_morph")

    for i in range(max_frames):
        state = hold(session, 1, "LEFT", "B", reason="big_pink_tunnel_roll")
        if i % 18 == 5:
            # Morph bombs: X (A is unreliable here against these BTS-4 blocks).
            hold(session, 2, "X", reason="big_pink_tunnel_bomb")
            hold(session, 50, reason="big_pink_tunnel_bomb_wait")
...
def play_big_pink_drop_to_pocket(session: ControllerSession) -> SuperMetroidState:
    ...
    raised tunnel ledge.
    """
    require_room(session, ROOM_BIG_PINK, "drop_to_pocket")
    settle_hold(session, 10, reason="big_pink_pocket_drop_settle")
    hold(session, 4, "A", reason="big_pink_pocket_drop_unspin")
    hold(session, 25, reason="big_pink_pocket_drop_unspin")
    hold(session, 40, "RIGHT", "B", reason="big_pink_pocket_drop")
    hold(session, 50, reason="big_pink_pocket_drop_land")
    unmorph(session)
    return session.state
...
def play_big_pink_bomb_to_walkway_edge(
    ...
    *,
    ...
    """
    state = session.state
    require_room(session, ROOM_BIG_PINK, "bomb_to_edge")
    # Approach lip if still east.
    for _ in range(60):
        if state.samus_x <= 1168:
            break
        state = hold(session, 1, "LEFT", "B", reason="big_pink_edge_approach")
    try:
        select_weapon(session, 0)
    except RuntimeError:
        pass
    hold(session, 12, "DOWN", reason="big_pink_edge_morph")
    hold(session, 3, "LEFT", reason="big_pink_edge_press")
    hold(session, 2, "X", reason="big_pink_edge_bomb")
    hold(session, fuse_frames, "LEFT", reason="big_pink_edge_fuse")
    hold(session, jump_frames, "LEFT", "A", reason="big_pink_edge_boost")
    # Idle fall/land on walkway edge (no left — left pulls back to crest).
    for _ in range(50):
        state = hold(session, 1, reason="big_pink_edge_land")

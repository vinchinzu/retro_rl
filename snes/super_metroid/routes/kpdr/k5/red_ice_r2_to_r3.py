"""Red Tower Ice edge: frozen lower_ripper_2 → lower_ripper_3.

Same family as r1→r2: freeze the next Ripper with a vertical Ice shot
(no d-pad — walking falls off the ice), drop aim-up, Hi-Jump standing,
drift onto the ice top from above.
...
POLICY_ID = "red_tower_ice_ripper2_to_ripper3"

_STAND = frozenset({1, 2})
_TRUE_MORPH = frozenset({29, 30, 31, 32})
# Same offsets as r1→r2, shifted by the 96px Ripper spacing.
_JUMP_UNTIL_Y = LOW_RIPPER_3_LAND_Y - 27  # 2132
_DRIFT_HIGH_Y = LOW_RIPPER_3_LAND_Y - 10  # 2149
_HOVER_Y = (LOWER_RIPPER_3.y_range[0], LOWER_RIPPER_3.y_range[1] + 5)
...
class RedIceRipper23EdgeRunner:
    """One-action-per-call runner: freeze r3 at offset, standing hop on top."""

    policy_id = POLICY_ID
    variant_id = VARIANT_ID
    from_checkpoint = LOWER_RIPPER_2.checkpoint_id
    to_checkpoint = LOWER_RIPPER_3.checkpoint_id
    ...
    def __init__(self, env: Any, *, max_frames: int = 280, max_attempts: int = 2) -> None:
        ...
        self.failed = False
        self.failure = ""
        self.last_reason = "red_ice_r23_init"
    ...
    def _retry_or_fail(self, state: SuperMetroidState, reason: str) -> None:
        self.attempts += 1
        if self.attempts >= self.max_attempts or not LOWER_RIPPER_2.matches(state):
            self._fail(reason)
            return
        ...
        while not self.complete and not self.failed:
            if self.phase == "stand":
                if int(state.pose) in _STAND or int(state.pose) in (3, 4):
                    self._set_phase("acquire", "track r3")
                    continue
                return self._emit(_action("UP"), "red_ice_r23_stand")

            if self.phase == "acquire":
                support = ripper_at_height(self.env, MID_RIPPER_Y)
                enemy = ripper_at_height(self.env, LOW_RIPPER_3_Y)
                if support is None or support.freeze_timer < 22:
                    self._fail("r2 thawed before r3 freeze")
                    break
                if enemy is None:
                    return self._emit(idle_action(), "red_ice_r23_wait_r3")
                signed = int(enemy.x) - int(state.samus_x)
                # Wave+Ice vertical only connects on the facing (right) side.
                # r3 starts left of the Ice-pin r2 seat; shooting that approach
                # freezes it overhead and the standing hop bonks the underside.
                if enemy.freeze_timer > 40 and signed >= 8:
                    self._target_x = int(enemy.x)
                    self._set_phase("drop_aim", f"frozen x={enemy.x}")
                    continue
                if 8 <= signed <= 36:
                    return self._emit(_action("UP", "X"), "red_ice_r23_freeze_shot")
                return self._emit(_action("UP"), "red_ice_r23_wait_dx")

            if self.phase == "drop_aim":
                if int(state.pose) in _STAND or self._phase_frames >= 10:
                    self._set_phase("jump", "standing Hi-Jump")
                    continue
                return self._emit(idle_action(), "red_ice_r23_drop_aim")

            if self.phase == "jump":
                if int(state.samus_y) <= _JUMP_UNTIL_Y or self._phase_frames >= 32:
                    self._set_phase("land", "drift onto ice top")
                    continue
                return self._emit(_action("A"), "red_ice_r23_jump")

            if self.phase == "land":
                if checkpoint_supported(self.env, state, LOWER_RIPPER_3):
                ...
                if grounded and LOWER_RIPPER_1.matches(state):
                    self._fail(
                        f"fell past r2 xy=({state.samus_x},{state.samus_y})"
                    )
                    break
                if grounded and LOWER_RIPPER_2.matches(state):
                    self._retry_or_fail(state, "landed back on r2")
                    continue
                if int(state.pose) in _TRUE_MORPH:
                    return self._emit(_action("UP"), "red_ice_r23_unmorph")
                enemy = ripper_at_height(self.env, LOW_RIPPER_3_Y)
                ...
                if y <= _DRIFT_HIGH_Y and abs(x - ex) > 3:
                    ...
                    return self._emit(_action(direction), "red_ice_r23_drift_high")
                if _HOVER_Y[0] <= y <= _HOVER_Y[1]:
                    return self._emit(idle_action(), "red_ice_r23_hover")
                if abs(x - ex) > 3:
                    direction = "RIGHT" if x < ex else "LEFT"
                    return self._emit(_action(direction), "red_ice_r23_track")
                return self._emit(idle_action(), "red_ice_r23_fall")

            if self.phase == "settle":
                if not checkpoint_supported(self.env, state, LOWER_RIPPER_3):
                    self._retry_or_fail(state, "unstable r3 support")
                    continue
                if self._settle_frames >= 8:
                    self.complete = True
                    self._set_phase("complete", LOWER_RIPPER_3.checkpoint_id)
                    break
                self._settle_frames += 1
                return self._emit(idle_action(), "red_ice_r23_checkpoint_settle")
...
def play_ripper2_to_ripper3(
    ...
    *,
    ...
    """Synchronous facade: frozen r2 → grounded frozen r3."""
    env = getattr(session, "env", None)
    if env is None:
        raise RuntimeError("Red Ice r2→r3 needs session.env")
    if not can_attach_ripper2_edge(session.state):
        raise TimeoutError(
            f"{POLICY_ID}: not on lower_ripper_2 "
            f"xy=({session.state.samus_x},{session.state.samus_y}) "
            f"p={session.state.pose}"
        )
    runner = RedIceRipper23EdgeRunner(env, max_frames=max_frames)

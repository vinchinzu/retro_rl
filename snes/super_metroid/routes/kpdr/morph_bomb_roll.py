    ...
    *,
    ...
    while session.state.samus_x > target_x and frames < max_frames:
        ...
        if in_pit:
            ...
            if deep_pit:
                hold(session, 4, "UP", reason="pit_unmorph")
                for _ in range(18):
                    hold(session, 1, "RIGHT", reason="pit_right")
                ...
                if not is_morph(session.state.pose) or session.state.samus_y <= pit_y + 5:
                    hold(session, 6, "A", "RIGHT", reason="pit_jump")
                    hold(session, 8, reason="pit_settle")
                try:
                ...
                hold(session, 6, "UP", reason="pit_unmorph")
                hold(session, 8, "A", "RIGHT", reason="pit_jump")
                hold(session, 10, reason="pit_settle")
                try:
                    ensure_morph(session)
                except TimeoutError:
                    pass
        ...
        if stall_frames > 0 and frames_since_progress >= stall_frames:
            ...
            hold(session, 2, "X", reason="safe_watchdog_bomb")
            hold(session, 10, reason="safe_watchdog_pause")
            frames += 12
            frames_since_progress = 0
            _log("watchdog")
            phase = MorphBombRollPhase.ON_BAND
        ...
        phase = MorphBombRollPhase.ON_BAND
        hold(session, 2, "X", reason="safe_bomb")
        frames += 2
        for step in range(cycle_len):
            ...
            if s.samus_y < elev_y or (s.samus_y <= max_y and step > cycle_len // 2):
                hold(session, 1, "LEFT", reason="safe_roll")
            elif s.samus_y <= max_y + 8:
                # On band but early cycle: short wait so bomb can pop.
                if step < 8:
                    hold(session, 1, reason="safe_bomb_wait")
                else:
                    hold(session, 1, "LEFT", reason="safe_roll")
            else:
                hold(session, 1, reason="safe_band_wait")
            frames += 1
        ...
        if session.state.samus_x < last_progress_x - 2:
            ...
            hold(session, 6, reason="safe_stall_pause")
            frames += 6

...
from super_metroid.routes.kpdr.morph_bomb_roll import bomb_roll_left_safe
...
def play_pink_pb_break_maze_wall(
    ...
    *,
    ...
    """
    require_room(session, ROOM_PINK_PB, "pink_pb_break_maze_wall")
    ...
    for _ in range(40):
        if session.state.samus_x <= 440:
            break
        hold(session, 1, "LEFT", reason="pb_maze_to_wall")
...
def play_pink_pb_morph_bomb_collect(
    ...
    *,
    ...
    """
    require_room(session, ROOM_PINK_PB, "pink_pb_morph_collect")
    if session.state.max_power_bombs > 0:
        return session.state
    ensure_morph(session)
    state = bomb_roll_left_safe(
    ...
    if state.samus_x <= 225:
        hold(session, 5, "UP", reason="pb_collect_unmorph")
        hold(session, 10, reason="pb_collect_unmorph")
        for _ in range(100):
            state = hold(session, 1, "LEFT", reason="pb_collect_walk")
            if state.max_power_bombs > 0:
                return state
        for _ in range(40):
            state = hold(session, 1, "RIGHT", reason="pb_collect_walk_back")
            if state.max_power_bombs > 0:
                return state
    if session.state.max_power_bombs <= 0:
        raise TimeoutError(
...
def play_pink_pb_from_left_zone(
    ...
    """
    require_room(session, ROOM_PINK_PB, "from_left_zone")
    if session.state.max_power_bombs > 0:
        return session.state
    ...
    if session.state.samus_y < 385 and session.state.samus_x <= 230:
        for i in range(80):
            ...
            state = hold(session, 1, d, reason="pb_leftzone_drop")
            if state.samus_y >= 385 or state.max_power_bombs > 0:
                break
            if state.samus_x > 230:
                hold(session, 5, "LEFT", reason="pb_leftzone_back")
    if session.state.max_power_bombs > 0:
        return session.state
...
def play_pink_pb_mid_maze_to_collect(
    ...
    *,
    ...
    """
    require_room(session, ROOM_PINK_PB, "mid_maze")
    if session.state.max_power_bombs > 0:
        return session.state

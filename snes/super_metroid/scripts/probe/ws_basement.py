#!/usr/bin/env python3
"""Wrecked Ship Basement → Phantoon's Room pure hop (rr-cjpp).

Unpowered basement hallway. Map station LEFT is dead — skip. Walk RIGHT.
Bomb the morph-tunnel obstruction (X while morph). Unmorph. Super the
Gadora eye door. Enter 0xCD13 and settle. Do not fight Phantoon.
https://wiki.supermetroid.run/Wrecked_Ship_Basement

```bash
uv run python snes/super_metroid/scripts/probe/ws_basement.py bench
uv run python snes/super_metroid/scripts/probe/ws_basement.py dump
uv run python snes/super_metroid/scripts/probe/ws_basement.py pure
uv run python snes/super_metroid/scripts/probe/ws_basement.py pure --dual
```

Default source: ``scratch/post_ws_main_to_basement.state`` (p24 gs=8, falling).
...
DEFAULT_OUT = SCRATCH / "post_ws_basement_to_phantoon.state"
DEFAULT_REPORT = SCRATCH / "ws_basement_to_phantoon.json"
DEFAULT_DUAL = SCRATCH / "ws_basement_to_phantoon_dual.json"
DEFAULT_BEFORE = SCRATCH / "ws_basement_to_phantoon_before.json"
...
def _run_hop(
    ...
    *,
    ...
    try:
        ...
        error = None
        try:
            st = ws.play_ws_basement_to_phantoon(sess)
        except Exception as exc:  # noqa: BLE001
            error = f"{type(exc).__name__}: {exc}"
            st = sess.state
        ok = error is None and ws.ws_basement_phantoon_settled(st)
...
def cmd_bench(args: argparse.Namespace) -> int:
    ...
    report["hop"] = "ws_basement_to_phantoon"
...
def _halt_reason(st: Any, *, idle_done: bool, stall: int) -> str | None:
    room = int(st.room_id)
    x = int(st.samus_x)
    if room == ROOM_WS_MAP:
        return "map_0xCCCB"
    if room == ws.ROOM_WS_MAIN:
        return "back_main_0xCAF6"
    if room == ws.ROOM_PHANTOON:
        return "phantoon_0xCD13"
    if room != ws.ROOM_WS_BASEMENT:
        return f"wrong_room_0x{room:04X}"
    if x < 80:
        return "left_door_band"
    if idle_done and stall >= 28:
        return "x_stall"
...
def cmd_dump(args: argparse.Namespace) -> int:
    """Land from p24, then walk. Halt at first miss. No free-place."""
    source = Path(args.source or DEFAULT_SOURCE)
    env = make_dev_env()
    ...
    try:
        ...
        idle = idle_action()
        landed = False
        for i in range(min(80, args.frames)):
            st = sess.state
            pose = int(st.pose)
            vy = int(st.velocity_y)
            if pose not in FALLING_POSES and vy == 0:
                landed = True
                samples.append(_rich(env, st, {"frame": sess.frame, "phase": "landed"}))
                break
            st = sess.step(idle)
            if i % 10 == 9 or int(st.room_id) != ws.ROOM_WS_BASEMENT:
                samples.append(_rich(env, st, {"frame": sess.frame, "phase": "land"}))
            halt = _halt_reason(st, idle_done=False, stall=0)
            if halt:
                break
        walk = buttons(*btn_names) if btn_names else idle_action()
        prev_x = int(sess.state.samus_x)
        stall = 0
        if halt is None:
            for i in range(args.frames):
                st = sess.step(walk)
                x = int(st.samus_x)
                stall = stall + 1 if abs(x - prev_x) < 2 else 0
                prev_x = x
                take = (
                    i % 15 == 14
                    or int(st.room_id) != ws.ROOM_WS_BASEMENT
                    or stall >= 28
                ...
                if take:
                    samples.append(
                        _rich(env, st, {"frame": sess.frame, "phase": "walk"})
                    )
                halt = _halt_reason(st, idle_done=True, stall=stall)
                if halt:
                    if not take:
                        samples.append(
                            _rich(env, st, {"frame": sess.frame, "phase": "halt"})
                        )
                    break
        report = {
            "command": "dump",
            "hop": "ws_basement_to_phantoon",
            "source": str(source),
            "hypothesis": (
                f"land idle then {btn_names}; halt at first miss; landed={landed}"
            ),
...
def cmd_pure(args: argparse.Namespace) -> int:
    ...
    for row in runs:
        row["command"] = "pure"
        row["hop"] = "ws_basement_to_phantoon"
    primary = runs[0]
    timed = primary["time"]
    dual_exact = True
    if args.dual:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_dump = sub.add_parser(
        "dump", help="RAM walk (land / morph tunnel / eye door). Halt at first miss."
    )
    _common(p_dump)
    ...
    p_dump.add_argument(
        "--buttons",
        default="RIGHT,B",
        help="Comma-separated hold after landing (default RIGHT,B)",
    )

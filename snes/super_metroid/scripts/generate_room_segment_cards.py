...
Usage (from repo root):
  uv run python snes/super_metroid/scripts/generate_room_segment_cards.py --count 8
  uv run python snes/super_metroid/scripts/generate_room_segment_cards.py --count 16 --print-ids
"""
...
1. If no teleport fixture: bootstrap this problem only:
   ```bash
   uv run python snes/super_metroid/scripts/room/run_problem.py bootstrap {pid}
   uv run python snes/super_metroid/scripts/room/run_problem.py teleport {pid}
   ```
2. Scaffold policy if missing:
   ```bash
   uv run python snes/super_metroid/scripts/room/run_problem.py scaffold {pid}
   ```
3. Iterate isolated run until **green** or honest residual with pin:
   ```bash
   uv run python snes/super_metroid/scripts/room/run_problem.py run {pid}
   ```
4. Promote **only** on green isolated run (practice track):
   ```bash
   uv run python snes/super_metroid/scripts/room/run_problem.py run {pid} --promote
   ```

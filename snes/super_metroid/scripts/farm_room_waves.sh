...
#   ./snes/super_metroid/scripts/farm_room_waves.sh
#   ./snes/super_metroid/scripts/farm_room_waves.sh --rounds 10 --parallel 8
#   ./snes/super_metroid/scripts/farm_room_waves.sh --rounds 1 --parallel 8 --dry-run
#   ./snes/super_metroid/scripts/farm_room_waves.sh --rounds 3 --deadline-hours 2
#
# Between rounds:
...
source "$ROOT/snes/super_metroid/scripts/farm_rollup.sh"
...
FARM_LOG_DIR="$ROOT/snes/super_metroid/docs/tasks/logs/farm"
...
git -C "$ROOT" status --porcelain 2>/dev/null \
  | awk '{print $NF}' \
  | rg 'snes/super_metroid/(routes/continuous\.py|docs/STATUS\.md|routes/kpdr/|progression\.py)' \
...
generate_batch() {
  ...
  uv run python snes/super_metroid/scripts/generate_room_segment_cards.py \
...
latest_log_for() {
  local id="$1"
  ls -1t "$ROOT/snes/super_metroid/docs/tasks/logs/${id}_"*.log 2>/dev/null | head -1 || true
}
...
review_batch() {
  ...
  for id in "${ids[@]}"; do
    ...
    if [[ -n "$log" && -f "$log" ]]; then
      ...
      result="$(farm_card_result "$id" "$log" "$ROOT/snes/super_metroid/docs/tasks")"
      next="$(
  ...
  if [[ "$DRY_RUN" -eq 0 ]]; then
    uv run python snes/super_metroid/scripts/export/room_work_queue.py 2>/dev/null \
...
dispatch_batch() {
  ...
  out="$(
    SM_OPENCODE_VARIANT="$VARIANT" \
      ./snes/super_metroid/scripts/dispatch_opencode.sh --luna --variant "$VARIANT" \

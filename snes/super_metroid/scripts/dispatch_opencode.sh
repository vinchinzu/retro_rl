...
#   ./snes/super_metroid/scripts/dispatch_opencode.sh --model flash SM-K4-05
#   ./snes/super_metroid/scripts/dispatch_opencode.sh SM-K4-03 SM-K4-04 SM-K4-05
#   ./snes/super_metroid/scripts/dispatch_opencode.sh --foreground SM-K4-04
#   ./snes/super_metroid/scripts/dispatch_opencode.sh --flash SM-ROLLUP-STATUS
#   ./snes/super_metroid/scripts/dispatch_opencode.sh --variant max SM-ROOM-SEG-01
#
...
#
# Process: snes/super_metroid/docs/tasks/PROCESS.md
  ...
  awk '
    ...
    in_own && /^- / {
      ...
      if (line ~ /\// || line ~ /\.(py|md|csv|json)$/) {
        # normalize snes/super_metroid/ prefix away
...
hot_key_for_path() {
  ...
  case "$p" in
    ...
    *return_hijump.py) echo "routes/kpdr/return_hijump.py" ;;
    *varia_return.py) echo "routes/kpdr/varia_return.py" ;;
    *spore_spawn_controller.py|*spore_spawn.py) echo "routes/kpdr/spore_spawn.py" ;;
...
check_ownership_conflicts() {
  ...
  for id in "${ids[@]}"; do
    card="$ROOT/snes/super_metroid/docs/tasks/${id}.md"
    if [[ ! -f "$card" ]]; then
      continue
    fi
...
run_one() {
  local id="$1"
  local card="$ROOT/snes/super_metroid/docs/tasks/${id}.md"
  if [[ ! -f "$card" ]]; then
  ...
  # Working directory is snes/super_metroid/ (--dir). Prefer package-relative paths
  # (tests/..., routes/..., docs/...). Repo-root paths snes/super_metroid/... also
...
Paths:
- OpenCode cwd is the snes/super_metroid/ package root.
- Read/Edit: package-relative (tests/..., routes/..., combat/..., docs/..., policies/...).
- Shell: either \`cd\` to monorepo root and use snes/super_metroid/... paths, or use paths that work from cwd; if a path 404s, retry without the snes/super_metroid/ prefix.
...
if [[ "$FOREGROUND" -eq 0 && ${#PIDS[@]} -gt 0 ]]; then
  ...
  echo "  tail -f snes/super_metroid/docs/tasks/logs/*.log"
  echo "Wait pids: ${PIDS[*]}"
fi

"""Natural boss activation capture harness (library API only).
...
from super_metroid.routes.continuous import play_bombs
...
def capture_natural_bomb_torizo_activation(
    *,
    ...
    def play_prefix(ctx: PlayContext) -> None:
        play_bombs(ctx.session, ctx.splits, ctx.segments)
    ...
    return capture_natural_activation(
        NaturalCaptureConfig(
            ...
            source_note="continuous play_bombs prefix",
        )
    )

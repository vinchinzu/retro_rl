...
# ---------------------------------------------------------------------------
# Boss catalog: single data table + thin public accessors
# ---------------------------------------------------------------------------
...
_BOSS_TABLE: tuple[BossCatalogEntry, ...] = (
    ...
    # slots. Hitbox dims approximate for contact — strategy lane-keeps rather
    # than AABB aim. sm-json-data dims 192×303; feature box tighter for contact.
    BossCatalogEntry(
        name="Kraid",
        room_id=0xA59F,
        max_hp=1000,
    ...
    # Acid-push fight — not pure HP damage (sm-json hp=0).
    BossCatalogEntry(
        name="Crocomire",
        room_id=0xA98D,
        max_hp=0,
    ...
    # Multi-phase: Zebetites → brain → rainbow. Primary max_hp is MB1.
    BossCatalogEntry(
        name="Mother Brain",
        room_id=0xDD58,
...
BOSS_CATALOG: dict[str, BossCatalogEntry] = {e.boss_id: e for e in _BOSS_TABLE}

...
def test_kpdr_profile_is_tracker_24() -> None:
    ids = [s[0] for s in KPDR_ANY_SPLITS]
    assert ids[0] == "ceres_station"
    assert ids[-1] == "ship"
    assert "charge_beam" in ids
    assert "golden_four" in ids
    assert "mother_brain_1" in ids
    assert len(ids) == 24
...
def test_build_and_format_kpdr_rows() -> None:
    timeline = [
        {
            "room_id": 0xDF45,
            "abs_entry": 6416,
        ...
        {"room_id": 0x91F8, "abs_entry": 9286, "dwell": 961, "hop_key": "ls", "source": "s1"},
        {"room_id": 0x9E9F, "abs_entry": 13103, "dwell": 1370, "hop_key": "morph", "source": "s1"},
        {"room_id": 0xA107, "abs_entry": 15721, "dwell": 563, "hop_key": "mis", "source": "s2"},
    ]
    rows = build_kpdr_split_rows(timeline, hop_pb={"elev_esc": 2640})
    by_id = {r.split_id: r for r in rows}
    assert by_id["ceres_station"].hit
    assert by_id["ceres_station"].best_frames == 9056
    assert by_id["ceres_station"].gold_frames == 9056
    assert by_id["morph_ball"].best_frames == 13103
    assert by_id["ship"].hit is False
    table = format_kpdr_split_table(rows, product_frames=392208)
    assert "KPDR ANY%" in table
    assert "Personal Best" in table
    assert "Best Possible" in table
    assert "Ceres Station" in table

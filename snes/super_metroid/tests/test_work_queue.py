...
def test_work_queue_is_sorted_easiest_first_and_has_progress() -> None:
    ...
    # Practice board must not own continuous-spine product priority.
    assert "continuousSpineBlockers" not in summary["workFocus"]
    assert "continuousSpineBlockersReady" not in summary["percentComplete"]
    assert "STATUS.md" in summary["workFocus"]["productNextWork"]
    assert summary["workFocus"]["bossDeferred"] >= 1
    # Top ranks should not be bosses.
    assert all(int(row["queue"]) < 4 for row in rows[:10])
    assert "not continuous" in payload["unitNote"].lower() or "dual-track" in (
        payload["unitNote"].lower()
    )


def test_resolve_practice_status_prefers_verified_policy() -> None:
    problem = {
        "practice": {
            "status": "unstarted",
            "stateFile": "missing.state",
            "policyFile": "missing.json",
            "reportFile": "missing_report.json",
        }
    }
    assert (
        resolve_practice_status(problem, state_ok=False, policy_status=None)
        == "unstarted"
    )
    assert (
        resolve_practice_status(
            problem,
            state_ok=True,
            policy_status="verified_development_state",
        )
        == "ready"
    )
    assert (
        resolve_practice_status(problem, state_ok=True, policy_status="generated")
        == "state_ready"
    )

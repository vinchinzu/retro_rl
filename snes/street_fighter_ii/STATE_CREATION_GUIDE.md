# SF2 Turbo State Creation Guide

## Current States

### Opponent States (Tournament Progression)
- `Fight_StreetFighterIITurbo.state` - vs Opponent 1
...
1. Load SF2 Turbo ROM in RetroArch/SNES9x/etc
2. Play to character select
3. Choose character
4. Save state when first fight starts
5. Copy .state file to `custom_integrations/StreetFighterIITurbo-Snes/`
...
env = retro.make(game="StreetFighterIITurbo-Snes", state=state)
```
...
**TODO for next time:** Create a `CharSelect_StreetFighterIITurbo.state` saved right at the character select screen. This would make creating character states much faster:
...
# At character select screen, manually save as CharSelect_StreetFighterIITurbo.state
...
- Try using a later checkpoint: `sf2_ppo_1000000_steps.zip`

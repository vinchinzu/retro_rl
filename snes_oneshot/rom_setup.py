...
def extract_rom_from_zip(zip_path: Path, dest_dir: Path) -> Path:
    """Extract the first SNES ROM member from a zip into dest_dir.
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        if not members:
            raise FileNotFoundError(f"No SNES ROM in zip: {zip_path}")
        member = members[0]
...
def link_rom_into_integration(
    ...
    *,
    ...
    link_path = integration_dir / link_name
...
def setup_game_rom(
    *,
    ...
    rom_path = extract_rom_from_zip(shared_zip, roms_dir)

...
**Status:** continuous power-on Survival is live through the `0x30` Vire
clear and KEY-RIGHT into `0x31` `(16,141)`. The next natural boundary is the
`0x31` maze Vire clear, then free RIGHT into `0x32`. Do not claim Clean
STATUS — Survival assist only for this segment.
...
| **0x31** | **live pure clear+RIGHT 2/2; continuous enter 1/1** | 5× `0x12` Vire | Maze; leftover (16,141); clear opens R; free RIGHT → 0x32 | `rr-n1wn` / `rr-resv` |
...
```bash
# Continuous power-on verified through 0x31 enter

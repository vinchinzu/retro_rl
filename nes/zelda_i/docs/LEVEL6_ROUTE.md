...
| **0x09** | N of 0x19 | **3× blue `0x23` + 2× orange `0x24`** | `0x03` | spine enter v2 1/1 keys 5→4; clear v1 1/1 leftover `(112,173)`; left 0x68 pushes then gone; NE hole decorative v8 |
...
### Spawn settle (0x18) — **spine 1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover `0x18` `(120,189)` |
| Policy | IDLE 512f; sample objects every 12f. Do not walk. |
| Stop | `--through level6-settle18` play-ready `0x18` TF `0x1F` |
| Census | **`0x44`×1** (never `0x43`/`0x46` during idle) + `0x56`×1; leftover PNG shows 3-head Gleeok |
| Doors | `cur_opened_doors` 4→0; `open_doorway_mask` 0; `room_item_id` `0x03` |
| Leftover | `(120,189)` south mouth |
| Track | **assisted Survival** |

```bash
uv run python nes/zelda_i/scripts/run_survival_spine.py --through level6-settle18 --no-video --trials 1
```

### Gleeok fight (0x18) — **spine 1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover `0x18` `(120,189)` after settle |
| Combat | LEFT+UP while y>173 (y=189 UP solid; slides west) then L4 south-stand on body `0x44` |
| Stop | `--through level6-gleeok18` body `0x44` gone; head `0x46` / fireball residual OK |
| Leftover | `(121,133)`; east shutter still closed; north stairs live; sparkle residual |
| Keys | 5 (no spend); map still `0x0A` (no L6 map bit) |
| Track | **assisted Survival** |
| Notes | Walkthrough type `0x43` is L4 2-head; L6 3-head is **`0x44`**. Do not require Map this hop. |

```bash
uv run python nes/zelda_i/scripts/run_survival_spine.py --through level6-gleeok18 --no-video --trials 1
```

### Post-Gleeok residual (0x18) — **spine 1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover `0x18` `(121,133)` after body-gone |
| Policy | IDLE census 192f; south-stand if `0x46`; occupancy dodge `0x56`. Do not walk stairs. |
| Stop | `--through level6-postgleeok18` play-ready `0x18` heads gone |
| Census | no `0x44` / no `0x46`; `0x56` then gone; `cur_opened_doors` 0→4→5; **`open_doorway_mask` 0** |
| Leftover | `(156,133)` (fireball dodge drifted RIGHT) |
| Track | **assisted Survival** |
| Notes | v1 aborted on `cur_opened_doors` RIGHT at 17f; PNG shutter still black. v2 waited the census. Do not treat doors RAM 5 as Map. |
...
### Clear 0x19 — **spine 1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover `0x19` `(16,141)` |
| Policy | IDLE 160f census (do not walk into beam) then occupancy-patrol |
| Census | **2× `0x13` Zol** + **2× `0x17` Like-Like** + 2× `0x2b`; RoomItemId **`0x17` Map** |
| Stop | `--through level6-clear19` play-ready empty `0x19` (ignore Bubble/`0x2b`) |
| Leftover | `(176,158)`; Map sprite on floor; bubbles residual |
| Map | still `0x0A` (no L6 bit) |
| Track | **assisted Survival** |

```bash
uv run python nes/zelda_i/scripts/run_survival_spine.py --through level6-clear19 --no-video --trials 1
```

### Map pickup 0x19 — **not on tape (skipped)**
...
| v6 | axis LEFT (free) then idle `(136,137)` 6000f; no persistent item object slot |
| Notes | Graphic idle, center idle, and `(136,141)` idle are not `ADDR_MAP\|0x20`. Occupancy from leftover is unrecoverable. Map is optional; skip toward Rod. Do not grant Map. |

### Skip Map north (0x19 leftover → 0x09) — **spine 1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover `0x19` `(176,158)` after clear (skips Map) |
| v1 | axis LEFT to x=120 then occupancy UP; leftover `(32,189)` keys 5→4 **south** door |
| v2 | axis LEFT to x=136 then occupancy KEY-UP; play `0x09` `(120,205)` keys 5→4 hop 275f |
| Stop | `--through level6-room09` play-ready dest ≠ `0x19` |
| Track | **assisted Survival** |
| Notes | x=120 UP from y=157 freeze-misses. Halt y>=181. Do not poke the door. |

```bash
uv run python nes/zelda_i/scripts/run_survival_spine.py --through level6-room09 --no-video --trials 1
```

### Clear 0x09 — **spine 1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover `0x09` `(120,205)` |
| Census | **3× `0x23` blue** + **2× `0x24` orange** + left 0x68 `(96,144)` |
| Stop | `--through level6-clear09` play-ready empty `0x09` |
| Leftover | `(112,173)`; blocks unpushed; map still `0x0A` |
| Track | **assisted Survival** |
| Notes | occupancy-patrol 1,419f. Do not push 0x68 this hop. Rod stairs residual. |
...
### South 0x09 → play 0x19 — **1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover play `0x09` `(192,141)` rod=1 |
| Path | occupancy to `(120,189)` then DOWN; 10 DOWN misses at x=192 (remaining block / east column) then south-band LEFT |
| Stop | `--through level6-south09` play-ready dest (RAM `0x19`) rod=1 |
| Leftover | `(120,77)` north mouth of `0x19`; keys=4 bombs=8 TF=`0x1F` bow=0 arrows=0 |
| v1 | **1/1** hop 251f, 213,305f |
| Track | **assisted Survival** |
| Notes | 0x19 already cleared; no combat hop. Dest is RAM — do not invent Gohma. Next east/south mouths on leftover PNG. Do not poke `ADDR_ARROWS`. |

```bash
uv run python nes/zelda_i/scripts/run_survival_spine.py --through level6-south09 --no-video --trials 1
```

### South 0x19 → play 0x29 — **1/1**

| Field | Live |
|-------|------|
| Start | Survival leftover play `0x19` `(120,77)` rod=1 keys=4 |
| Path | occupancy to `(120,189)` then DOWN; never UP |
| Stop | `--through level6-south19` play-ready dest (RAM `0x29`) rod=1 |
| Leftover | `(120,77)` north mouth of dark `0x29`; keys 4→3 bombs=8 TF=`0x1F` bow=0 arrows=0 |
| v1 | **1/1** hop 663f, 213,968f |
| Track | **assisted Survival** |
| Notes | PNG south mouth of 0x19 is **KEY**. West of 0x19 is Gleeok `0x18`, not south. Dark Vire room; enter-stop. Do not poke `ADDR_ARROWS`. |
...
| 0x19 north | **key** (PNG lock) | −1 | **spine 1/1** v2 KEY-UP keys 5→4 → `0x09` |
...
- `recordings/l6_south09_continuous_v1.json` — occupancy south `0x09`→`0x19` 1/1 hop 251f leftover `(120,77)` rod=1 bow=0 arrows=0
- `recordings/l6_south19_continuous_v1.json` — occupancy KEY-DOWN `0x19`→`0x29` 1/1 hop 663f leftover dark `(120,77)` keys 4→3
...
- Spine: `uv run python nes/zelda_i/scripts/run_survival_spine.py --through level6-south19 --no-video --trials 1`
...
13. Vire clear / wizzrobe path → Gohma arrow → Heart → TF `0x20` — Gohma needs an arrow; do not poke

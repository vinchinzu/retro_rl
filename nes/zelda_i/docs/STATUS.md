...
| Power-on → L6 stairs `0x09` | **1/1 continuous Survival**; left 0x68 y-move then NE 0x68 south-face UP onto `0x71`; 210,902f hop 203f; leftover mode 9 `0x75` `(208,93)` rod=0; deaths 0; progression/capacity writes 0; no state load | `l6_stairs09_continuous_v14.json` / `_final.png` |
| Power-on → L6 Magical Rod | **1/1 continuous Survival**; south y=189 RIGHT, east RIGHT+UP, LEFT+UP pedestal; 211,529f hop 627f; leftover mode 9 `0x75` `(136,141)` `ADDR_ROD=1`; deaths 0; progression/capacity writes 0; no state load | `l6_rod_continuous_v15.json` / `_final.png` |
| Power-on → L6 exit cellar `0x75` | **1/1 continuous Survival**; RIGHT y=141, LEFT+DOWN drop, west spit UP; 213,054f hop 1,525f; leftover play `0x09` `(192,141)` rod=1 bow=0 arrows=0 keys=4 bombs=8 TF=`0x1F`; deaths 0; progression/capacity writes 0; no state load | `l6_exit75_continuous_v3.json` / `_final.png` |
| Power-on → L6 south `0x09` → `0x19` | **1/1 continuous Survival**; occupancy south from leftover `(192,141)` (10 DOWN misses at x=192); 213,305f hop 251f; leftover play `0x19` `(120,77)` rod=1 bow=0 arrows=0 keys=4 bombs=8 TF=`0x1F` map=`0x0A`; deaths 0; progression/capacity writes 0; no state load | `l6_south09_continuous_v1.json` / `_final.png` |
...
Gleeok fight, Rod, cellar return, and south `0x09`→`0x19` are on the tape
(`l6_south09_continuous_v1` 1/1, leftover play `0x19` `(120,77)` rod=1
bow=0 arrows=0). 0x19 already cleared. Gohma / TF `0x20` residual —
Gohma needs an arrow.
...
1. **Active Survival tip:** `--through level6-south09` is green
   (`l6_south09_continuous_v1` 1/1, play `0x19` `(120,77)` rod=1 hop
   251f, tape 213,305f). 0x19 already cleared. Next door from leftover
   PNG (east/south mouths). Gohma / TF `0x20` residual — Gohma needs an
   arrow; do not poke `ADDR_ARROWS`/`ADDR_BOW`. Do not invent Gohma
   ...
   beads. No seamed compose. Bomb/key count pokes are documented

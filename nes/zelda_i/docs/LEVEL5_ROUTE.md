...
Power-on Survival `--through level5` is **1/1**
(`l5_tf_continuous_v1`, 173,961f, TF=`0x1F`, room `0x14` mode 18
`(120,149)`). L4 TF settle uses `PostL4TriforceSettleController` (do not
reload mid-fanfare). OW path is `POST_L4_TO_LEVEL5_HOPS` /
`Level5EntranceFromL4` class, not old At4A. 0x66 uses occupancy (v1
cardinal timeout north of the river). East key from north-bank leftover
`(32,101)` aligns ladder x=56 before DOWN. Recorder is earned in cellar

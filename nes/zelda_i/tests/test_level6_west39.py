"""Unit tests for Level 6 west of 0x39 after clear3a leftover."""
...
def test_level6_west39_reuses_north39_prefix_then_west_not_north() -> None:
    from retro_harness.nes import nes_action, nes_idle_action

    stages = level6_west39_stages()
    assert [name for name, _, _ in stages] == ["level6_west_0x39"]
    ...
    assert ctl._goal() == (WEST_CLIP_X, 141)
    assert ctl.walker.grid.xmin == WEST_SPAWN_XMIN == 16
    # v1 occupancy-halt leftover LEFT miss f2. north39 replans this same
    # miss then enters 0x39. Halt only after leaving 0x3A.
    replan = make_west39_controller()
    ...
    reclear = make_west39_controller()
    reclear.room = 0x39
    reclear.keys = 4
    ...
    assert list(act.action) == list(nes_action("LEFT"))
    assert list(act.action) != list(nes_action("LEFT", "UP"))
    # v2 timeout occupancy_stand (32,93) tile 200. north39 west_align DOWN.
    mouth = _ram(level=6, screen=0x3A, x=32, y=93, keys=4)
    mouth[ADDR_ROD] = 1
    align = make_west39_controller()
    ...
    assert list(act.action) == list(nes_idle_action())
    assert level6_west39_success(read_snapshot(ram_dest))
    north = _ram(level=6, screen=0x29, x=120, y=205, keys=4)
    north[ADDR_ROD] = 1
    north_fail = make_west39_controller()
    ...
    assert not level6_west39_success(read_snapshot(north))
    still = _ram(level=6, screen=0x3A, x=144, y=141, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_west39_success(read_snapshot(still))
    via = _ram(level=6, screen=0x39, x=208, y=141, keys=4)
    via[ADDR_ROD] = 1
    assert not level6_west39_success(read_snapshot(via))
    back = _ram(level=6, screen=0x3A, x=16, y=141, keys=4)
    back[ADDR_ROD] = 1
    back_fail = make_west39_controller()
    back_fail.room = 0x39
    back_fail.keys = 4
    ...
    key_up[ADDR_ROD] = 1
    up_fail = make_west39_controller()
    up_fail.room = 0x39
    up_fail.keys = 4
    ...
    assert "level6-north39" not in L6_THROUGH[: L6_THROUGH.index("level6-west39")]

...
def test_level6_stairs3a_occupancy_south_face_then_up() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.level6_path import south_face_stand
    ...
    stages = level6_stairs3a_stages()
    assert [name for name, _, _ in stages] == ["level6_stairs_0x3a"]
    leftover = _ram(level=6, screen=0x3A, x=144, y=141)
    leftover[ADDR_ROD] = 1
    _plant_block(leftover, 11, 120, 144)
    ctl = make_stairs_3a_controller()
    act = ctl.step(read_snapshot(leftover))
    ...
    assert south_face_stand(block) == (120, 160)
    at_stand = make_stairs_3a_controller()
    ram_stand = _ram(level=6, screen=0x3A, x=120, y=160)
    ram_stand[ADDR_ROD] = 1
    _plant_block(ram_stand, 11, 120, 144)
    act = at_stand.step(read_snapshot(ram_stand))
    ...
    assert list(act.action) == list(nes_action("UP"))
    assert any(n.startswith("pushed_120_144_to_120_128") for n in at_stand.notes)
    ram_stand[ADDR_LINK_Y] = 133
    act = at_stand.step(read_snapshot(ram_stand))
    assert act.reason == "hole_y"
    assert list(act.action) == list(nes_action("DOWN"))
    ram_stand[ADDR_LINK_Y] = 144
    act = at_stand.step(read_snapshot(ram_stand))
    assert act.reason == "hole_idle"
    assert list(act.action) == list(nes_idle_action())
    ram = _ram(level=6, screen=0x3A, x=120, y=144, mode=9)
    ram[ADDR_ROD] = 1
    arrive = make_stairs_3a_controller()
    act = arrive.step(read_snapshot(ram))
    assert arrive.success
    assert act.reason == "warped_9"
    assert level6_stairs3a_success(read_snapshot(ram))
    still = _ram(level=6, screen=0x3A, x=144, y=141)
    still[ADDR_ROD] = 1
    assert not level6_stairs3a_success(read_snapshot(still))
    dest = _ram(level=6, screen=0x39, x=16, y=141)
    dest[ADDR_ROD] = 1
    assert level6_stairs3a_success(read_snapshot(dest))
    dest[ADDR_ROD] = 0
    assert not level6_stairs3a_success(read_snapshot(dest))
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-stairs3a", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_stairs_0x3a"
    assert "level6-stairs3a" in L6_THROUGH


def test_level6_exit_ow_occupancy_west_from_clear3a() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    ...
    stages = level6_exit_ow_stages()
    assert [name for name, _, _ in stages] == ["level6_exit_ow_0x22"]
    assert EXIT_OW_LEGS[0][0] == 0x3A
    assert EXIT_OW_LEGS[0][1] == "LEFT"
    assert EXIT_OW_LEGS[-1][0] == 0x79
    assert EXIT_OW_LEGS[-1][1] == "DOWN"
    ...
    assert list(act.action) != list(nes_action("RIGHT"))
    assert list(act.action) != list(nes_action("UP"))
    west = _ram(level=6, screen=0x3A, x=48, y=141, keys=4)
    west[ADDR_ROD] = 1
    push = make_exit_ow_controller()
    act = push.step(read_snapshot(west))
    assert act.reason == "west_push"
    assert list(act.action) == list(nes_action("LEFT"))
    dest39 = _ram(level=6, screen=0x39, x=208, y=141, keys=4)
    dest39[ADDR_ROD] = 1
    arrived = make_exit_ow_controller()
    arrived.hop = 0
    arrived.keys = 4
    act = arrived.step(read_snapshot(dest39))
    assert not arrived.failed
    assert arrived.hop == 1
    assert any(n.startswith("arrived_39_") for n in arrived.notes)
    boxed = _ram(level=6, screen=0x39, x=208, y=141, keys=4)
    boxed[ADDR_ROD] = 1
    clip = make_exit_ow_controller()
    act = clip.step(read_snapshot(boxed))
    assert act.reason in ("up_path", "left_path")
    clip.walker.misses = 1
    act = clip.step(read_snapshot(boxed))
    assert act.reason == "east_spawn_clip"
    assert list(act.action) == list(nes_action("LEFT", "UP"))
    assert list(act.action) != list(nes_idle_action())
    north = _ram(level=6, screen=0x39, x=120, y=93, keys=4)
    north[ADDR_ROD] = 1
    up = make_exit_ow_controller()
    act = up.step(read_snapshot(north))
    assert act.reason == "north_push"
    assert list(act.action) == list(nes_action("UP"))
    from zelda_i.dungeon_ids import VIRE_OBJECT_TYPE

    vires = _ram(level=6, screen=0x39, x=120, y=93, keys=4)
    vires[ADDR_ROD] = 1
    vires[ADDR_OBJ_TYPE + 1] = VIRE_OBJECT_TYPE
    vires[ADDR_OBJ_HP + 1] = 144
    reclear = make_exit_ow_controller()
    ...
    spent = _ram(level=6, screen=0x19, x=120, y=205, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_exit_ow_controller()
    key_hop.hop = 2
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_29_to_19_4->3") for n in key_hop.notes)
    assert key_hop.hop == 3
    mouth = _ram(level=6, screen=0x79, x=120, y=189, keys=4)
    mouth[ADDR_ROD] = 1
    down = make_exit_ow_controller()
    act = down.step(read_snapshot(mouth))
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("DOWN"))
    ow = _ram(level=0, screen=0x22, x=48, y=77)
    ow[ADDR_ROD] = 1
    done = make_exit_ow_controller()
    done.hop = len(EXIT_OW_LEGS) - 1
    done.keys = 4
    act = done.step(read_snapshot(ow))
    assert done.success
    assert act.reason == "exited_ow"
    assert level6_exit_ow_success(read_snapshot(ow))
    still = _ram(level=6, screen=0x3A, x=144, y=141)
    still[ADDR_ROD] = 1
    assert not level6_exit_ow_success(read_snapshot(still))
    cellar = _ram(level=6, screen=0x3A, x=112, y=144, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_exit_ow_controller()
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    assert WEST_SPAWN_XMIN == 16
    run = SpineRun(through="level6-exit-ow", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_exit_ow_0x22"
    assert "level6-exit-ow" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_north39_enter_stop_from_clear3a() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import VIRE_OBJECT_TYPE
    from zelda_i.level6_exit_ow import make_north39_controller
    from zelda_i.level6_overworld import WIZZROBE_ORANGE_TYPE
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOW, ADDR_ROD

    stages = level6_north39_stages()
    assert [name for name, _, _ in stages] == ["level6_north39_0x29"]
    leftover = _ram(level=6, screen=0x3A, x=144, y=141, keys=4)
    leftover[ADDR_ROD] = 1
    ...
    dest39 = _ram(level=6, screen=0x39, x=208, y=141, keys=4)
    dest39[ADDR_ROD] = 1
    arrived = make_north39_controller()
    arrived.hop = 0
    arrived.keys = 4
    act = arrived.step(read_snapshot(dest39))
    assert not arrived.failed
    assert not arrived.success
    assert arrived.hop == 1
    assert any(n.startswith("arrived_39_") for n in arrived.notes)
    boxed = _ram(level=6, screen=0x39, x=208, y=141, keys=4)
    boxed[ADDR_ROD] = 1
    clip = make_north39_controller()
    act = clip.step(read_snapshot(boxed))
    clip.walker.misses = 1
    act = clip.step(read_snapshot(boxed))
    assert act.reason == "east_spawn_clip"
    ...
    vires[ADDR_OBJ_TYPE + 1] = VIRE_OBJECT_TYPE
    vires[ADDR_OBJ_HP + 1] = 144
    reclear = make_north39_controller()
    act = reclear.step(read_snapshot(vires))
    assert any(n.startswith("reclear_39_") for n in reclear.notes)
    assert act.reason != "north_push"
    assert reclear.fighter is not None
    enter = _ram(level=6, screen=0x29, x=120, y=205, keys=4)
    enter[ADDR_ROD] = 1
    enter[ADDR_OBJ_TYPE + 1] = WIZZROBE_ORANGE_TYPE
    enter[ADDR_OBJ_HP + 1] = 144
    arrive29 = make_north39_controller()
    arrive29.hop = 1
    arrive29.keys = 4
    act = arrive29.step(read_snapshot(enter))
    assert arrive29.success
    assert not arrive29.failed
    assert act.reason == "entered_29"
    assert list(act.action) == list(nes_idle_action())
    assert any(n.startswith("arrived_29_120_205") for n in arrive29.notes)
    assert level6_north39_success(read_snapshot(enter))
    inland = make_north39_controller()
    inland.hop = 2
    inland.keys = 4
    act = inland.step(read_snapshot(enter))
    assert inland.success
    assert act.reason == "entered_29"
    assert act.reason != "north_push"
    still = _ram(level=6, screen=0x3A, x=144, y=141, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_north39_success(read_snapshot(still))
    ow = _ram(level=0, screen=0x22, x=48, y=77)
    ow[ADDR_ROD] = 1
    assert not level6_north39_success(read_snapshot(ow))
    ow_fail = make_north39_controller()
    ow_fail.hop = 1
    ow_fail.keys = 4
    act = ow_fail.step(read_snapshot(ow))
    assert ow_fail.failed
    assert act.reason.startswith("ow_early")
    snap = read_snapshot(leftover)
    assert snap.bow == 0
    assert snap.arrows == 0
    leftover[ADDR_BOW] = 1
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-north39", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_north39_0x29"
    assert "level6-north39" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_inland29_clips_off_tile_244_column() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.level6_inland29 import make_inland29_controller
    from zelda_i.level6_overworld import WIZZROBE_ORANGE_TYPE
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOW, ADDR_ROD

    stages = level6_inland29_stages()
    assert [name for name, _, _ in stages] == ["level6_inland_0x29"]
    leftover = _ram(level=6, screen=0x29, x=120, y=205, keys=4)
    leftover[ADDR_ROD] = 1
    ...
    assert list(act.action) == list(nes_action("LEFT", "UP"))
    assert list(act.action) != list(nes_action("UP"))
    boxed = _ram(level=6, screen=0x29, x=120, y=157, keys=4)
    boxed[ADDR_ROD] = 1
    miss = make_inland29_controller()
    act = miss.step(read_snapshot(boxed))
    assert act.reason == "inland_clip"
    assert list(act.action) == list(nes_action("LEFT", "UP"))
    assert list(act.action) != list(nes_action("UP"))
    aisle = _ram(level=6, screen=0x29, x=64, y=141, keys=4)
    aisle[ADDR_ROD] = 1
    path = make_inland29_controller()
    act = path.step(read_snapshot(aisle))
    assert act.reason == "north_path"
    assert list(act.action) != list(nes_action("LEFT", "UP"))
    wizz = _ram(level=6, screen=0x29, x=120, y=93, keys=4)
    wizz[ADDR_ROD] = 1
    wizz[ADDR_OBJ_TYPE + 1] = WIZZROBE_ORANGE_TYPE
    wizz[ADDR_OBJ_HP + 1] = 144
    reclear = make_inland29_controller()
    act = reclear.step(read_snapshot(wizz))
    assert any(n.startswith("reclear_29_") for n in reclear.notes)
    assert act.reason != "north_push"
    assert reclear.fighter is not None
    band = _ram(level=6, screen=0x29, x=48, y=109, keys=4)
    band[ADDR_ROD] = 1
    door = make_inland29_controller()
    act = door.step(read_snapshot(band))
    assert act.reason == "door_clip"
    assert list(act.action) == list(nes_action("RIGHT", "UP"))
    assert list(act.action) != list(nes_action("RIGHT"))
    north = _ram(level=6, screen=0x29, x=120, y=93, keys=4)
    north[ADDR_ROD] = 1
    push = make_inland29_controller()
    act = push.step(read_snapshot(north))
    assert act.reason == "north_push"
    assert list(act.action) == list(nes_action("UP"))
    dest = _ram(level=6, screen=0x19, x=120, y=205, keys=4)
    dest[ADDR_ROD] = 1
    arrive = make_inland29_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert act.reason == "arrived_19"
    assert level6_inland29_success(read_snapshot(dest))
    spent = _ram(level=6, screen=0x19, x=120, y=205, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_inland29_controller()
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_29_to_19_4->3") for n in key_hop.notes)
    still = _ram(level=6, screen=0x29, x=120, y=93, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_inland29_success(read_snapshot(still))
    back = _ram(level=6, screen=0x39, x=120, y=77, keys=4)
    back[ADDR_ROD] = 1
    assert not level6_inland29_success(read_snapshot(back))
    south = make_inland29_controller()
    act = south.step(read_snapshot(back))
    assert south.failed
    assert act.reason.startswith("backtrack_39")
    snap = read_snapshot(leftover)
    assert snap.bow == 0
    assert snap.arrows == 0
    leftover[ADDR_BOW] = 1
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-inland29", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_inland_0x29"
    assert "level6-inland29" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_south28_occupancy_down_from_north_mouth() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.level6_south28 import SOUTH_DOOR_X, SOUTH_DOOR_Y, make_south28_controller
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOW, ADDR_ROD

    stages = level6_south28_stages()
    assert [name for name, _, _ in stages] == ["level6_south_0x28"]
    ...
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("UP"))
    face = _ram(level=6, screen=0x28, x=120, y=93, keys=4)
    face[ADDR_ROD] = 1
    clip = make_south28_controller()
    act = clip.step(read_snapshot(face))
    assert act.reason == "diamond_clip"
    assert list(act.action) == list(nes_action("LEFT", "DOWN"))
    past = _ram(level=6, screen=0x28, x=112, y=101, keys=4)
    past[ADDR_ROD] = 1
    inland = make_south28_controller()
    act = inland.step(read_snapshot(past))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("RIGHT"))
    drift = _ram(level=6, screen=0x28, x=128, y=77, keys=4)
    drift[ADDR_ROD] = 1
    align = make_south28_controller()
    act = align.step(read_snapshot(drift))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("LEFT"))
    door = _ram(level=6, screen=0x28, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    door[ADDR_ROD] = 1
    push = make_south28_controller()
    act = push.step(read_snapshot(door))
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("RIGHT", "DOWN"))
    assert list(act.action) != list(nes_action("DOWN"))
    dest = _ram(level=6, screen=0x38, x=120, y=77, keys=4)
    dest[ADDR_ROD] = 1
    arrive = make_south28_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_38"
    assert level6_south28_success(read_snapshot(dest))
    spent = _ram(level=6, screen=0x38, x=120, y=77, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_south28_controller()
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_28_to_38_4->3") for n in key_hop.notes)
    still = _ram(level=6, screen=0x28, x=120, y=189, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_south28_success(read_snapshot(still))
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    assert not level6_south28_success(read_snapshot(key_up))
    north_fail = make_south28_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back = _ram(level=6, screen=0x18, x=120, y=205, keys=4)
    back[ADDR_ROD] = 1
    gleeok = make_south28_controller()
    act = gleeok.step(read_snapshot(back))
    assert gleeok.failed
    assert act.reason.startswith("backtrack_18")
    cellar = _ram(level=6, screen=0x28, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_south28_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-south28", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_south_0x28"
    assert "level6-south28" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_aisle28_occupancy_west_aisle_down() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import LIKE_LIKE_OBJECT_TYPE, WIZZROBE_BLUE_OBJECT_TYPE
    from zelda_i.level6_aisle28 import AISLE_X, SOUTH_DOOR_Y, make_aisle28_controller
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOW, ADDR_ROD

    stages = level6_aisle28_stages()
    assert [name for name, _, _ in stages] == ["level6_aisle_0x28"]
    leftover = _ram(level=6, screen=0x28, x=120, y=77, keys=4)
    leftover[ADDR_ROD] = 1
    ...
    reclear = make_aisle28_controller()
    act = reclear.step(read_snapshot(live))
    assert any(n.startswith("reclear_28_") for n in reclear.notes)
    assert act.reason != "diamond_clip"
    assert reclear.fighter is not None
    wall = _ram(level=6, screen=0x28, x=AISLE_X, y=SOUTH_DOOR_Y, keys=4)
    wall[ADDR_ROD] = 1
    wall[ADDR_OBJ_TYPE + 1] = WIZZROBE_BLUE_OBJECT_TYPE
    wall[ADDR_OBJ_HP + 1] = 64
    aisle_live = make_aisle28_controller()
    act = aisle_live.step(read_snapshot(wall))
    assert any(n.startswith("reclear_28_") for n in aisle_live.notes)
    assert act.reason != "south_push"
    assert aisle_live.fighter is not None
    face = _ram(level=6, screen=0x28, x=120, y=93, keys=4)
    face[ADDR_ROD] = 1
    clip = make_aisle28_controller()
    act = clip.step(read_snapshot(face))
    assert act.reason == "diamond_clip"
    assert list(act.action) == list(nes_action("LEFT", "DOWN"))
    inland = _ram(level=6, screen=0x28, x=96, y=101, keys=4)
    inland[ADDR_ROD] = 1
    path = make_aisle28_controller()
    act = path.step(read_snapshot(inland))
    assert act.reason == "south_path"
    # v2 dest-same-y LEFT boxed (96,101) then UP halt. Goal is (64,189).
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("LEFT"))
    assert list(act.action) != list(nes_idle_action())
    aisle = _ram(level=6, screen=0x28, x=AISLE_X, y=141, keys=4)
    aisle[ADDR_ROD] = 1
    down = make_aisle28_controller()
    act = down.step(read_snapshot(aisle))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("DOWN"))
    door = _ram(level=6, screen=0x28, x=AISLE_X, y=SOUTH_DOOR_Y, keys=4)
    door[ADDR_ROD] = 1
    push = make_aisle28_controller()
    act = push.step(read_snapshot(door))
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("RIGHT", "DOWN"))
    ...
    mouth[ADDR_ROD] = 1
    skip = make_aisle28_controller()
    act = skip.step(read_snapshot(mouth))
    assert act.reason == "center_mouth_skip"
    assert list(act.action) == list(nes_action("LEFT"))
    assert list(act.action) != list(nes_action("DOWN"))
    dest = _ram(level=6, screen=0x38, x=64, y=77, keys=4)
    dest[ADDR_ROD] = 1
    arrive = make_aisle28_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_38"
    assert level6_aisle28_success(read_snapshot(dest))
    spent = _ram(level=6, screen=0x38, x=64, y=77, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_aisle28_controller()
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_28_to_38_4->3") for n in key_hop.notes)
    still = _ram(level=6, screen=0x28, x=AISLE_X, y=189, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_aisle28_success(read_snapshot(still))
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    assert not level6_aisle28_success(read_snapshot(key_up))
    north_fail = make_aisle28_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back = _ram(level=6, screen=0x18, x=120, y=205, keys=4)
    back[ADDR_ROD] = 1
    gleeok = make_aisle28_controller()
    act = gleeok.step(read_snapshot(back))
    assert gleeok.failed
    assert act.reason.startswith("backtrack_18")
    cellar = _ram(level=6, screen=0x28, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_aisle28_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-aisle28", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_aisle_0x28"
    assert "level6-aisle28" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_clear28_south_reclear_then_center_south() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import LIKE_LIKE_OBJECT_TYPE, WIZZROBE_BLUE_OBJECT_TYPE
    ...
    stages = level6_clear28_south_stages()
    assert [name for name, _, _ in stages] == ["level6_clear_south_0x28"]
    leftover = _ram(level=6, screen=0x28, x=120, y=77, keys=4)
    leftover[ADDR_ROD] = 1
    ...
    live[ADDR_OBJ_TYPE + 1] = LIKE_LIKE_OBJECT_TYPE
    live[ADDR_OBJ_HP + 1] = 64
    reclear = make_clear28_south_controller()
    act = reclear.step(read_snapshot(live))
    assert any(n.startswith("reclear_28_") for n in reclear.notes)
    assert act.reason != "diamond_clip"
    assert reclear.fighter is not None
    aisle_live = _ram(level=6, screen=0x28, x=WEST_AISLE_X, y=SOUTH_DOOR_Y, keys=4)
    aisle_live[ADDR_ROD] = 1
    aisle_live[ADDR_OBJ_TYPE + 1] = WIZZROBE_BLUE_OBJECT_TYPE
    aisle_live[ADDR_OBJ_HP + 1] = 64
    reclear_aisle = make_clear28_south_controller()
    act = reclear_aisle.step(read_snapshot(aisle_live))
    assert any(n.startswith("reclear_28_") for n in reclear_aisle.notes)
    assert act.reason != "south_push"
    assert reclear_aisle.fighter is not None
    face = _ram(level=6, screen=0x28, x=120, y=93, keys=4)
    face[ADDR_ROD] = 1
    clip = make_clear28_south_controller()
    act = clip.step(read_snapshot(face))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("DOWN"))
    act = clip.step(read_snapshot(face))
    assert any(n.startswith("miss_f") for n in clip.notes)
    assert act.reason == "diamond_clip"
    assert list(act.action) == list(nes_action("LEFT", "DOWN"))
    inland = _ram(level=6, screen=0x28, x=96, y=101, keys=4)
    inland[ADDR_ROD] = 1
    path = make_clear28_south_controller()
    act = path.step(read_snapshot(inland))
    assert act.reason == "south_path"
    assert list(act.action) != list(nes_action("LEFT"))
    assert list(act.action) != list(nes_idle_action())
    aisle = _ram(level=6, screen=0x28, x=WEST_AISLE_X, y=SOUTH_DOOR_Y, keys=4)
    aisle[ADDR_ROD] = 1
    skip = make_clear28_south_controller()
    act = skip.step(read_snapshot(aisle))
    assert act.reason == "west_aisle_skip"
    assert list(act.action) == list(nes_action("RIGHT"))
    assert list(act.action) != list(nes_action("DOWN"))
    door = _ram(level=6, screen=0x28, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    door[ADDR_ROD] = 1
    push = make_clear28_south_controller()
    act = push.step(read_snapshot(door))
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("RIGHT", "DOWN"))
    dest = _ram(level=6, screen=0x38, x=120, y=77, keys=4)
    dest[ADDR_ROD] = 1
    arrive = make_clear28_south_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_38"
    assert level6_clear28_south_success(read_snapshot(dest))
    spent = _ram(level=6, screen=0x38, x=120, y=77, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_clear28_south_controller()
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_28_to_38_4->3") for n in key_hop.notes)
    still = _ram(level=6, screen=0x28, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_clear28_south_success(read_snapshot(still))
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    assert not level6_clear28_south_success(read_snapshot(key_up))
    north_fail = make_clear28_south_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back = _ram(level=6, screen=0x18, x=120, y=205, keys=4)
    back[ADDR_ROD] = 1
    gleeok = make_clear28_south_controller()
    act = gleeok.step(read_snapshot(back))
    assert gleeok.failed
    assert act.reason.startswith("backtrack_18")
    cellar = _ram(level=6, screen=0x28, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_clear28_south_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-clear28-south", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_clear_south_0x28"
    assert "level6-clear28-south" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_south38_occupancy_down_skip_traps() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import LIKE_LIKE_OBJECT_TYPE
    from zelda_i.level6_south38 import SOUTH_DOOR_X, SOUTH_DOOR_Y, make_south38_controller
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOW, ADDR_ROD

    stages = level6_south38_stages()
    assert [name for name, _, _ in stages] == ["level6_south_0x38"]
    ...
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("LEFT", "DOWN"))
    live = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    live[ADDR_ROD] = 1
    live[ADDR_OBJ_TYPE + 1] = LIKE_LIKE_OBJECT_TYPE
    live[ADDR_OBJ_HP + 1] = 64
    skip = make_south38_controller()
    act = skip.step(read_snapshot(live))
    assert act.reason == "south_path"
    assert not any(n.startswith("reclear_") for n in skip.notes)
    assert getattr(skip, "fighter", None) is None
    face = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    face[ADDR_ROD] = 1
    clip = make_south38_controller()
    act = clip.step(read_snapshot(face))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("DOWN"))
    act = clip.step(read_snapshot(face))
    ...
    assert act.reason != "diamond_clip"
    assert list(act.action) != list(nes_action("LEFT", "DOWN"))
    mouth = _ram(level=6, screen=0x38, x=112, y=93, keys=4)
    mouth[ADDR_ROD] = 1
    peel = make_south38_controller()
    act = peel.step(read_snapshot(mouth))
    assert act.reason != "north_door_halt"
    assert list(act.action) != list(nes_idle_action())
    assert list(act.action) != list(nes_action("UP"))
    door = _ram(level=6, screen=0x38, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    door[ADDR_ROD] = 1
    push = make_south38_controller()
    act = push.step(read_snapshot(door))
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("DOWN"))
    dest = _ram(level=6, screen=0x48, x=120, y=77, keys=4)
    dest[ADDR_ROD] = 1
    arrive = make_south38_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_48"
    assert level6_south38_success(read_snapshot(dest))
    spent = _ram(level=6, screen=0x48, x=120, y=77, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_south38_controller()
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_38_to_48_4->3") for n in key_hop.notes)
    still = _ram(level=6, screen=0x38, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_south38_success(read_snapshot(still))
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    assert not level6_south38_success(read_snapshot(key_up))
    north_fail = make_south38_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back = _ram(level=6, screen=0x28, x=120, y=205, keys=4)
    back[ADDR_ROD] = 1
    north = make_south38_controller()
    act = north.step(read_snapshot(back))
    assert north.failed
    assert act.reason.startswith("backtrack_28")
    cellar = _ram(level=6, screen=0x38, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_south38_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-south38", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_south_0x38"
    assert "level6-south38" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_clear38_south_reclear_then_center_south() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import LIKE_LIKE_OBJECT_TYPE
    ...
    stages = level6_clear38_south_stages()
    assert [name for name, _, _ in stages] == ["level6_clear_south_0x38"]
    leftover = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    leftover[ADDR_ROD] = 1
    ...
    assert list(act.action) == list(nes_idle_action())
    assert any(n.startswith("spawn_wait_38_") for n in ctl.notes)
    assert list(act.action) != list(nes_action("DOWN"))
    expired = make_clear38_south_controller()
    expired.frames = SPAWN_WAIT_FRAMES
    act = expired.step(read_snapshot(leftover))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("LEFT", "DOWN"))
    assert list(act.action) != list(nes_action("RIGHT", "DOWN"))
    ...
    live[ADDR_OBJ_TYPE + 1] = LIKE_LIKE_OBJECT_TYPE
    live[ADDR_OBJ_HP + 1] = 64
    reclear = make_clear38_south_controller()
    act = reclear.step(read_snapshot(live))
    assert any(n.startswith("reclear_38_") for n in reclear.notes)
    assert act.reason != "diamond_clip"
    assert reclear.fighter is not None
    wizz = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    wizz[ADDR_ROD] = 1
    wizz[ADDR_OBJ_TYPE + 1] = WIZZROBE_ORANGE_TYPE
    wizz[ADDR_OBJ_HP + 1] = 144
    reclear_wizz = make_clear38_south_controller()
    act = reclear_wizz.step(read_snapshot(wizz))
    assert any(n.startswith("reclear_38_") for n in reclear_wizz.notes)
    assert reclear_wizz.fighter is not None
    face = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    face[ADDR_ROD] = 1
    clip = make_clear38_south_controller()
    clip.frames = SPAWN_WAIT_FRAMES
    act = clip.step(read_snapshot(face))
    assert act.reason == "south_path"
    assert list(act.action) == list(nes_action("DOWN"))
    act = clip.step(read_snapshot(face))
    ...
    mouth = _ram(level=6, screen=0x38, x=112, y=93, keys=4)
    mouth[ADDR_ROD] = 1
    peel = make_clear38_south_controller()
    peel.frames = SPAWN_WAIT_FRAMES
    act = peel.step(read_snapshot(mouth))
    assert act.reason != "north_door_halt"
    assert list(act.action) != list(nes_idle_action())
    assert list(act.action) != list(nes_action("UP"))
    door = _ram(level=6, screen=0x38, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    door[ADDR_ROD] = 1
    push = make_clear38_south_controller()
    push.frames = SPAWN_WAIT_FRAMES
    act = push.step(read_snapshot(door))
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("DOWN"))
    assert list(act.action) != list(nes_action("RIGHT", "DOWN"))
    ...
    assert act.reason == "south_push"
    assert list(act.action) == list(nes_action("DOWN"))
    trap_ram = _ram(level=6, screen=0x38, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    trap_ram[ADDR_ROD] = 1
    trap_ram[ADDR_OBJ_TYPE + 1] = 0x2B
    trap_ram[ADDR_OBJ_HP + 1] = 240
    trap_ram[ADDR_LINK_X + 1] = SOUTH_DOOR_X - 16
    trap_ram[ADDR_LINK_Y + 1] = SOUTH_DOOR_Y
    peel_trap = make_clear38_south_controller()
    peel_trap.frames = SPAWN_WAIT_FRAMES
    act = peel_trap.step(read_snapshot(trap_ram))
    assert act.reason == "trap_peel"
    assert list(act.action) != list(nes_action("DOWN"))
    dest = _ram(level=6, screen=0x48, x=120, y=77, keys=4)
    dest[ADDR_ROD] = 1
    arrive = make_clear38_south_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_48"
    assert level6_clear38_south_success(read_snapshot(dest))
    spent = _ram(level=6, screen=0x48, x=120, y=77, keys=3)
    spent[ADDR_ROD] = 1
    key_hop = make_clear38_south_controller()
    key_hop.keys = 4
    act = key_hop.step(read_snapshot(spent))
    assert any(n.startswith("key_spent_38_to_48_4->3") for n in key_hop.notes)
    still = _ram(level=6, screen=0x38, x=SOUTH_DOOR_X, y=SOUTH_DOOR_Y, keys=4)
    still[ADDR_ROD] = 1
    assert not level6_clear38_south_success(read_snapshot(still))
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    assert not level6_clear38_south_success(read_snapshot(key_up))
    north_fail = make_clear38_south_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back = _ram(level=6, screen=0x28, x=120, y=205, keys=4)
    back[ADDR_ROD] = 1
    north = make_clear38_south_controller()
    act = north.step(read_snapshot(back))
    assert north.failed
    assert act.reason.startswith("backtrack_28")
    cellar = _ram(level=6, screen=0x38, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_clear38_south_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-clear38-south", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_clear_south_0x38"
    assert "level6-clear38-south" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_bomb38_south_occupancy_east_west_census() -> None:
    from retro_harness.nes import nes_action
    ...
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOMBS, ADDR_BOW, ADDR_ROD

    stages = level6_bomb38_south_stages()
    assert [name for name, _, _ in stages] == ["level6_bomb_south_0x38"]
    leftover = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    leftover[ADDR_ROD] = 1
    ...
    act = ctl.step(read_snapshot(leftover))
    assert act.reason == "to_east"
    assert list(act.action) in (list(nes_action("DOWN")), list(nes_action("RIGHT")))
    assert list(act.action) != list(nes_action("LEFT", "DOWN"))
    assert ctl.phase == "east_path"
    trap = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    trap[ADDR_ROD] = 1
    trap[ADDR_BOMBS] = 8
    trap[ADDR_OBJ_TYPE + 1] = INVULN_MOVER_OBJECT_TYPE
    trap[ADDR_OBJ_HP + 1] = 240
    trap[ADDR_LINK_X + 1] = 97
    trap[ADDR_LINK_Y + 1] = SOUTH_DOOR_Y
    skip = make_bomb38_south_controller()
    ...
    mouth[ADDR_ROD] = 1
    mouth[ADDR_BOMBS] = 8
    back = make_bomb38_south_controller()
    act = back.step(read_snapshot(mouth))
    assert act.reason == "mouth_back"
    assert list(act.action) == list(nes_action("UP"))
    assert list(act.action) != list(nes_action("DOWN"))
    assert BOMB_38_SOUTH_STAND == (120, 173)
    dated = _ram(level=6, screen=0x38, x=120, y=181, keys=4)
    dated[ADDR_ROD] = 1
    dated[ADDR_BOMBS] = 8
    skip_dated = make_bomb38_south_controller()
    act = skip_dated.step(read_snapshot(dated))
    assert act.reason == "to_east"
    ...
    east[ADDR_ROD] = 1
    east[ADDR_BOMBS] = 8
    pusher = make_bomb38_south_controller()
    act = pusher.step(read_snapshot(east))
    assert act.reason == "east_push"
    assert list(act.action) == list(nes_action("RIGHT"))
    assert pusher.phase == "east_push"
    assert pusher.east_census
    for _ in range(CENSUS_PUSH_FRAMES):
        act = pusher.step(read_snapshot(east))
        if pusher.phase == "west_path":
            break
    assert pusher.phase == "west_path"
    assert any(n == "east_no_scroll" for n in pusher.notes)
    wx, wy = WEST_DOOR
    west = _ram(level=6, screen=0x38, x=wx, y=wy, keys=4)
    west[ADDR_ROD] = 1
    west[ADDR_BOMBS] = 8
    west_ctl = make_bomb38_south_controller()
    west_ctl.phase = "west_path"
    west_ctl.goal = WEST_DOOR
    act = west_ctl.step(read_snapshot(west))
    assert act.reason == "west_push"
    assert list(act.action) == list(nes_action("LEFT"))
    dest = _ram(level=6, screen=0x48, x=120, y=77, keys=4)
    dest[ADDR_ROD] = 1
    dest[ADDR_BOMBS] = 8
    arrive = make_bomb38_south_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_48"
    assert level6_bomb38_south_success(read_snapshot(dest))
    still = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    still[ADDR_ROD] = 1
    still[ADDR_BOMBS] = 8
    assert not level6_bomb38_south_success(read_snapshot(still))
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    north_fail = make_bomb38_south_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    ...
    back28[ADDR_ROD] = 1
    north = make_bomb38_south_controller()
    act = north.step(read_snapshot(back28))
    assert north.failed
    assert act.reason.startswith("backtrack_28")
    cellar = _ram(level=6, screen=0x38, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_bomb38_south_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-bomb38-south", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_bomb_south_0x38"
    assert "level6-bomb38-south" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_east38_clips_dated_tile_118_then_halts_next_miss() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import INVULN_MOVER_OBJECT_TYPE
    ...
    from zelda_i.ram import ADDR_ARROWS, ADDR_BOMBS, ADDR_BOW, ADDR_COLLIDING_TILE, ADDR_ROD

    stages = level6_east38_stages()
    assert [name for name, _, _ in stages] == ["level6_east_0x38"]
    leftover = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    leftover[ADDR_ROD] = 1
    leftover[ADDR_BOMBS] = 8
    ...
    trap[ADDR_OBJ_TYPE + 1] = INVULN_MOVER_OBJECT_TYPE
    trap[ADDR_OBJ_HP + 1] = 240
    skip = make_east38_controller()
    act = skip.step(read_snapshot(trap))
    assert act.reason == "to_east"
    assert not any("peel" in n or n.startswith("reclear_") for n in skip.notes)
    cx, cy = CLIP_BOX
    boxed = _ram(level=6, screen=0x38, x=cx, y=cy, keys=4)
    boxed[ADDR_ROD] = 1
    boxed[ADDR_BOMBS] = 8
    boxed[ADDR_COLLIDING_TILE] = 118
    clip = make_east38_controller()
    act = clip.step(read_snapshot(boxed))
    assert act.reason == "east_clip"
    assert list(act.action) == list(nes_action("RIGHT", "DOWN"))
    assert list(act.action) != list(nes_action("RIGHT"))
    ...
    south_face = _ram(level=6, screen=0x38, x=cx, y=149, keys=4)
    south_face[ADDR_ROD] = 1
    south_face[ADDR_BOMBS] = 8
    south_face[ADDR_COLLIDING_TILE] = 119
    col = make_east38_controller()
    act = col.step(read_snapshot(south_face))
    assert act.reason == "east_clip"
    assert list(act.action) == list(nes_action("RIGHT", "DOWN"))
    ...
    east[ADDR_ROD] = 1
    east[ADDR_BOMBS] = 8
    pusher = make_east38_controller()
    act = pusher.step(read_snapshot(east))
    assert act.reason == "east_push"
    assert list(act.action) == list(nes_action("RIGHT"))
    mid = _ram(level=6, screen=0x38, x=160, y=141, keys=4)
    mid[ADDR_ROD] = 1
    mid[ADDR_BOMBS] = 8
    halt = make_east38_controller()
    for x in range(16, 217):
        for y in range(77, 206):
            halt.walker.grid.blocked.add((x, y))
    act = halt.step(read_snapshot(mid))
    assert halt.failed
    assert act.reason.startswith("occupancy_halt_160_141")
    dest = _ram(level=6, screen=0x39, x=16, y=141, keys=4)
    dest[ADDR_ROD] = 1
    dest[ADDR_BOMBS] = 8
    arrive = make_east38_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_39"
    assert list(act.action) == list(nes_idle_action())
    assert level6_east38_success(read_snapshot(dest))
    still = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    still[ADDR_ROD] = 1
    still[ADDR_BOMBS] = 8
    assert not level6_east38_success(read_snapshot(still))
    south = _ram(level=6, screen=0x48, x=120, y=77, keys=4)
    south[ADDR_ROD] = 1
    assert not level6_east38_success(read_snapshot(south))
    south_fail = make_east38_controller()
    south_fail.keys = 4
    ...
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    north_fail = make_east38_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back28 = _ram(level=6, screen=0x28, x=120, y=205, keys=4)
    back28[ADDR_ROD] = 1
    north = make_east38_controller()
    act = north.step(read_snapshot(back28))
    assert north.failed
    assert act.reason.startswith("backtrack_28")
    cellar = _ram(level=6, screen=0x38, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_east38_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-east38", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_east_0x38"
    assert "level6-east38" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",
...
def test_level6_east38_lane_clips_onto_y141_then_occupancy_right() -> None:
    from retro_harness.nes import nes_action, nes_idle_action
    from zelda_i.dungeon_ids import INVULN_MOVER_OBJECT_TYPE
    from zelda_i.level6_east38 import CLIP_BOX, EAST_DOOR, SOUTH_DOOR_Y
    ...
    stages = level6_east38_lane_stages()
    assert [name for name, _, _ in stages] == ["level6_east_lane_0x38"]
    leftover = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    leftover[ADDR_ROD] = 1
    leftover[ADDR_BOMBS] = 8
    ...
    assert list(act.action) in (list(nes_action("DOWN")), list(nes_action("RIGHT")))
    assert list(act.action) != list(nes_action("RIGHT", "UP"))
    assert list(act.action) != list(nes_action("LEFT", "UP"))  # leftover start; clip is later
    assert list(act.action) != list(nes_action("B", "DOWN"))
    assert list(act.action) != list(nes_action("B"))
    assert not hasattr(ctl, "bomb")
    trap = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    ...
    trap[ADDR_OBJ_TYPE + 1] = INVULN_MOVER_OBJECT_TYPE
    trap[ADDR_OBJ_HP + 1] = 240
    skip = make_east38_lane_controller()
    act = skip.step(read_snapshot(trap))
    assert act.reason == "to_east"
    assert not any("peel" in n or n.startswith("reclear_") for n in skip.notes)
    cx, cy = CLIP_BOX
    ...
    boxed[ADDR_BOMBS] = 8
    boxed[ADDR_COLLIDING_TILE] = 118
    clip = make_east38_lane_controller()
    act = clip.step(read_snapshot(boxed))
    assert act.reason == "east_clip"
    assert list(act.action) == list(nes_action("RIGHT", "DOWN"))
    assert not clip.failed
    ...
    south_face[ADDR_BOMBS] = 8
    south_face[ADDR_COLLIDING_TILE] = 119
    col = make_east38_lane_controller()
    act = col.step(read_snapshot(south_face))
    assert act.reason == "east_clip"
    assert list(act.action) == list(nes_action("RIGHT", "DOWN"))
    dated = _ram(level=6, screen=0x38, x=LANE_X, y=149, keys=4)
    dated[ADDR_ROD] = 1
    dated[ADDR_BOMBS] = 8
    dated[ADDR_COLLIDING_TILE] = 177
    lane = make_east38_lane_controller()
    act = lane.step(read_snapshot(dated))
    assert act.reason == "lane_clip"
    assert list(act.action) == list(nes_action("LEFT", "UP"))
    assert list(act.action) != list(nes_action("RIGHT", "UP"))
    assert list(act.action) != list(nes_action("RIGHT", "DOWN"))
    assert list(act.action) != list(nes_action("UP"))
    assert lane.past_lane
    assert not lane.failed
    bounce = _ram(level=6, screen=0x38, x=cx, y=149, keys=4)
    bounce[ADDR_ROD] = 1
    bounce[ADDR_BOMBS] = 8
    bounce[ADDR_COLLIDING_TILE] = 119
    act = lane.step(read_snapshot(bounce))
    assert act.reason == "lane_clip"
    assert list(act.action) == list(nes_action("LEFT", "UP"))
    assert list(act.action) != list(nes_action("RIGHT", "DOWN"))
    assert not lane.failed
    on_lane = _ram(level=6, screen=0x38, x=144, y=LANE_Y, keys=4)
    on_lane[ADDR_ROD] = 1
    on_lane[ADDR_BOMBS] = 8
    right = make_east38_lane_controller()
    act = right.step(read_snapshot(on_lane))
    assert act.reason == "to_east"
    assert list(act.action) == list(nes_action("RIGHT"))
    assert right.on_lane
    mouth = _ram(level=6, screen=0x38, x=120, y=SOUTH_DOOR_Y, keys=4)
    mouth[ADDR_ROD] = 1
    mouth[ADDR_BOMBS] = 8
    mouth[ADDR_COLLIDING_TILE] = 170
    back = make_east38_lane_controller()
    act = back.step(read_snapshot(mouth))
    assert act.reason == "mouth_back"
    assert list(act.action) == list(nes_action("UP"))
    assert list(act.action) != list(nes_action("DOWN"))
    ...
    east[ADDR_ROD] = 1
    east[ADDR_BOMBS] = 8
    pusher = make_east38_lane_controller()
    act = pusher.step(read_snapshot(east))
    assert act.reason == "east_push"
    assert list(act.action) == list(nes_action("RIGHT"))
    mid = _ram(level=6, screen=0x38, x=160, y=141, keys=4)
    mid[ADDR_ROD] = 1
    mid[ADDR_BOMBS] = 8
    halt = make_east38_lane_controller()
    halt.on_lane = True
    for x in range(16, 217):
        for y in range(77, 206):
            halt.walker.grid.blocked.add((x, y))
    ...
    dest[ADDR_ROD] = 1
    dest[ADDR_BOMBS] = 8
    arrive = make_east38_lane_controller()
    arrive.keys = 4
    act = arrive.step(read_snapshot(dest))
    assert arrive.success
    assert not arrive.failed
    assert act.reason == "arrived_39"
    assert list(act.action) == list(nes_idle_action())
    assert level6_east38_lane_success(read_snapshot(dest))
    still = _ram(level=6, screen=0x38, x=120, y=93, keys=4)
    still[ADDR_ROD] = 1
    still[ADDR_BOMBS] = 8
    assert not level6_east38_lane_success(read_snapshot(still))
    south = _ram(level=6, screen=0x48, x=120, y=77, keys=4)
    south[ADDR_ROD] = 1
    assert not level6_east38_lane_success(read_snapshot(south))
    south_fail = make_east38_lane_controller()
    south_fail.keys = 4
    act = south_fail.step(read_snapshot(south))
    assert south_fail.failed
    assert act.reason.startswith("south_dated_48")
    key_up = _ram(level=6, screen=0x09, x=120, y=205, keys=3)
    key_up[ADDR_ROD] = 1
    north_fail = make_east38_lane_controller()
    north_fail.keys = 4
    act = north_fail.step(read_snapshot(key_up))
    assert north_fail.failed
    assert act.reason.startswith("key_up_09")
    back28 = _ram(level=6, screen=0x28, x=120, y=205, keys=4)
    back28[ADDR_ROD] = 1
    north = make_east38_lane_controller()
    act = north.step(read_snapshot(back28))
    assert north.failed
    assert act.reason.startswith("backtrack_28")
    cellar = _ram(level=6, screen=0x38, x=120, y=96, keys=4, mode=9)
    cellar[ADDR_ROD] = 1
    warp = make_east38_lane_controller()
    act = warp.step(read_snapshot(cellar))
    assert warp.failed
    assert act.reason.startswith("warped_cellar")
    snap = read_snapshot(leftover)
    ...
    assert armed.bow == 1
    assert armed.arrows == 1
    run = SpineRun(through="level6-east38-lane", success=True, boot_frames=199)
    assert run.report()["stop"] == "level6_east_lane_0x38"
    assert "level6-east38-lane" in L6_THROUGH
    assert L6_THROUGH[-8:] == (
        "level6-east38",
        "level6-east38-lane",
        "level6-bomb38-south",

...
| `level*_path.py` (L5 facade + west/whistle/cellar/tf; L6 north 0x68 / 0x18 settle), `level6_gleeok18.py`, `level6_stairs18.py`, `level6_room19.py`, `level6_stairs09.py`, `level6_rod.py`, `level6_exit75.py`, `level6_south09.py`, `level6_south19.py`, `level6_east29.py`, `level6_south29.py`, `level6_east39.py`, `level*_boss_*` | Path controllers + timing knobs |
...
- L6 0x58 north leftover PNG shutter is **walkable** (clear58 v1 wrong belief;
  ...
  **does** push (v2 `96,144→96,136` then gone). Occupancy from leftover
  ...
  is the pedestal. Cellar return v3 1/1: cardinal RIGHT y=141 to x=176,
  LEFT+DOWN drop (v1 DOWN @ (168,141) tile 250; v2 UP @ (208,141) tile
  243), floor LEFT x=48, UP west spit, idle then UP. Bow/arrows 0.
  0x09 south occupancy: leftover `(192,141)` DOWN misses 10× at x=192
  (remaining block / east column) then south-band LEFT; dest play `0x19`
  `(120,77)` hop 251f keys 4. 0x19 south PNG mouth is **KEY** (keys 4→3);
  dest RAM play `0x29` dark `(120,77)` hop 663f. West of 0x19 is Gleeok
  `0x18`, not south. 0x29 census is **3× blue `0x23` + 2× orange `0x24`**
  ...
  enters play `0x39` `(120,93)`. 0x39 census is **5× Vire `0x12`**.
  Occupancy from leftover `(136,173)` tile 118 boxed 4-cardinal (east39
  v1). v2 RIGHT+UP reaches y=141 then occupancy RIGHT boxed `(176,141)`.
  v3 cardinal RIGHT on y=141 enters play `0x3A` `(16,141)` keys 4.
  0x3A census 3× Like-Like `0x17` + 2× blue `0x23` + 2× orange `0x24`
  + center 0x68 `(112,144)`. Ignore 0x59 shots. Do not invent Gohma.
  Do not poke arrows.
...
`(136,141)` rod=1 hop 627f. `--through level6-exit75` **1/1**
`l6_exit75_continuous_v3` play `0x09` `(192,141)` rod=1 hop 1,525f
tape 213,054f. `--through level6-south09` **1/1**
`l6_south09_continuous_v1` play `0x19` `(120,77)` hop 251f tape
213,305f. `--through level6-south19` **1/1**
`l6_south19_continuous_v1` play `0x29` `(120,77)` hop 663f tape
213,968f keys 4→3. `--through level6-clear29` **1/1**
`l6_clear29_continuous_v2` play `0x29` `(55,133)` hop 1,406f tape
...
`(120,93)` hop 288f tape 215,822f rod=1 keys=4. `--through
level6-settle39` **1/1** 5× Vire `0x12`. `--through level6-clear39`
**1/1** hop 1,330f leftover `(136,173)`. `--through level6-east39`
**1/1** `l6_east39_continuous_v3` play `0x3A` `(16,141)` hop 320f
tape 217,632f keys=4 (v1 boxed; v2 occupancy boxed at 176). `--through
level6-settle3a` **1/1** 3×`0x17`+2×`0x23`+2×`0x24`+0x68. `--through
level6-clear3a` **1/1** `l6_clear3a_continuous_v1` leftover `(144,141)`
hop 1,857f tape 219,649f rod=1 keys=4 bow=0 arrows=0. Center 0x68
unpushed. Bow=0 arrows=0 (L1 bow skipped). Gohma needs an arrow — do
not poke. Do not invent Gohma room id. Map stays skipped. Stairs /
Gohma / TF `0x20` residual. Do not grant Map. Do not poke doors/keys.
Ignore 0x2b.

...
from zelda_i.level6_overworld import LEVEL6, LEVEL6_MAP_ROOM
...
SOUTH_BAND_Y = 181
SOUTH_DOOR_TOL = 4
SOUTH19_MAX_FRAMES = 4000
SOUTH19_SAMPLE_PERIOD = 12
CELLAR_MODE = 9


@dataclass
class Level6South19Controller:
    """Occupancy south mouth of cleared 0x19. Dest is RAM, not Gohma."""

    spec_id: str = "level6_south_0x19"
    room: int = LEVEL6_MAP_ROOM
    goal: tuple[int, int] = (SOUTH_DOOR_X, SOUTH_DOOR_Y)
    max_frames: int = SOUTH19_MAX_FRAMES
    frames: int = 0
    success: bool = False
    failed: bool = False
    ...
    def _emit(
        ...
        if force or self.frames <= 2 or self.frames % SOUTH19_SAMPLE_PERIOD == 0:
            self.samples.append(
                {
                    "frame": self.frames,
                    "x": int(snap.link_x),
    ...
    def step(self, snap: ZeldaSnapshot) -> FrameAction:
        ...
        if direction == "UP":
            # North is 0x09. Walkthrough is south to Vires.
    ...
    def report(self) -> dict[str, Any]:
        return {
            ...
            "policy": (
                "occupancy to (120,189) then DOWN; never UP; dest is RAM"
            ),
...
def make_south19_controller() -> Level6South19Controller:
    """Occupancy south of cleared 0x19. Do not poke bow/arrows/doors."""
    return Level6South19Controller()
...
def level6_south19_stages():
    ...
    return (
        ("level6_south_0x19", ctl, ctl.max_frames),
    )

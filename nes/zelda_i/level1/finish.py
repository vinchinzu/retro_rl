"""Level 1 west-route suffix: 0x42 exit, backtrack 0x44, Aquamentus, TF 0x01."""
...
class Level1AquamentusController:
    """Defeat Aquamentus at wooden-sword range. Survival tanks fireballs."""
    ...
    def _route_entry(self, snap: ZeldaSnapshot) -> FrameAction:
        ...
        if abs(dx) <= 2 and abs(dy) <= 2:
            ...
            return FrameAction(nes_idle_action(), "boss_entry_waypoint_idle")
        if abs(dx) > 2:
            direction = "RIGHT" if dx > 0 else "LEFT"
        else:
            direction = "DOWN" if dy > 0 else "UP"
    ...
    def step(self, snap: ZeldaSnapshot) -> FrameAction:
        ...
        if snap.screen == ROOM_AQUAMENTUS and snap.mode == PLAY_MODE:
            ...
            if (
                self.boss_seen
                and not bosses
                and self.phase not in (AquamentusPhase.COLLECT_HEART, AquamentusPhase.DONE)
            ):
                self._set_phase(AquamentusPhase.COLLECT_HEART, "aquamentus_defeated")
            if self.phase is AquamentusPhase.COLLECT_HEART and self._heart_collected(snap):
                self.success = True
                self._set_phase(AquamentusPhase.DONE, "heart_container_collected")
                return FrameAction(nes_idle_action(), "done")
        ...
        if self.phase is AquamentusPhase.WAIT_BOSS:
            if self.boss_seen:
                ...
                return FrameAction(nes_action("UP"), "preposition_while_boss_spawns")
        ...
        if not self.tank_hits:
            ...
            if threatening:
                ...
                return FrameAction(nes_action(self.dodge_direction), "dodge_fireball")
    ...
    def step(self, snap: ZeldaSnapshot) -> FrameAction:
        ...
        if self.phase is TriforcePhase.COLLECT:
            ...
            if abs(dx) <= 3 and abs(dy) <= 3:
                self.waypoint_index += 1
                return FrameAction(nes_idle_action(), "triforce_waypoint_idle")
            if abs(dx) > 3:
                direction = "RIGHT" if dx > 0 else "LEFT"
            else:
                direction = "DOWN" if dy > 0 else "UP"
    ...
    return (
        ...
        (
            ...
            Level1AquamentusController(
                entry_delay_frames=boss_entry_delay, tank_hits=survival
            ),
            AQUAMENTUS_MAX_FRAMES,
        ),
        ("triforce_shard_1", Level1TriforceController(), TRIFORCE_MAX_FRAMES),
    )

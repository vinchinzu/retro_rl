"""From Level5Entered64 east mouth: north-wall x-cross then down to center stairs."""
...
from zelda_i.scripts._probe_l5_whistle_path import step, walk_axis, shot
    ...
    try:
        ...
        for direction in ("DOWN", "UP"):
            ...
            if stair_transition_modes(snap.mode) or snap.screen != 0x64:
                break
        snap = read_snapshot(env.get_ram())
        png = RECORDINGS_DIR / f"l5_64_nw_{name}.png"
        obs, *_ = env.step(nes_idle_action())
...
def main():
    configure_headless()
    paths = []
    for ywall in (93, 97, 101):
        for x in (80, 96, 104, 112, 120, 128, 136, 144):
            paths.append((f"y{ywall}_x{x}", (("y", ywall), ("x", x), ("y", 141))))
    paths += [
        ("y93_x120_y125", (("y", 93), ("x", 120), ("y", 125), ("x", 120), ("y", 141))),
        ("y93_x112_y125", (("y", 93), ("x", 112), ("y", 125))),
        ("y93_x128_y125", (("y", 93), ("x", 128), ("y", 125))),
        ("south189_x32", (("y", 189), ("x", 32), ("y", 141), ("x", 120))),
        ("y189_x80", (("y", 189), ("x", 80), ("y", 141))),
        ("y189_x120", (("y", 189), ("x", 120), ("y", 141))),
    ]
    results = [try_path(n, s) for n, s in paths]
    hits = [r for r in results if r["took"] or r["end"].get("stair") or (r["end"]["xy"][0] < 140 and r["end"]["xy"][1] < 160)]
    write_json_report(RECORDINGS_DIR / "l5_64_northwall.json", {"results": results, "hits": hits})

"""From Level5Whistle64: south-gap EAST 0x65, then live graph to 0x24, Digdogger, TF."""
...
def main() -> dict:
    ...
    try:
        idle(env, assist, total, 12)
        snap = read_snapshot(env.get_ram())
        print("START", f"0x{snap.screen:02x}", [snap.link_x, snap.link_y], "whistle", int(read_u8(env.get_ram(), ADDR_WHISTLE)), "doors", int(snap.cur_opened_doors), flush=True)
        if snap.screen != 0x64 or int(read_u8(env.get_ram(), ADDR_WHISTLE)) < 1:
            return {"ok": False, "reason": "not_64_with_whistle"}
        ...
        if not hop["ok"]:
            rec = {"ok": False, "failed_room": "0x64", "hops": hops, "now": w.dump_live(read_snapshot(env.get_ram()), env.get_ram()), "pokes": False, "status_claim": None}
            w.write_dump("l5_whistle_to_24", rec)
            w.shot(env, assist, total, "l5_64_east_fail")
            return rec
        ...
        rec = {
            ...
            "hops": hops,
            "checkpoints": ["Level5WhistleFloor", "Level5Whistle64"] + (["Level5Triforce"] if boss.get("ok") else []),
            "pokes": False,
            "status_claim": None,
        }

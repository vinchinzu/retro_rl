"""Level5Whistle66: kill 3 Gibdo to open north shutter, then graph to 0x24 / Digdogger / TF."""
...
DOOR_PATHS = {
    ...
    "UP": (
        ...
        (("y", 189), ("x", 120), ("y", 93)),
        (("x", 120), ("y", 141), ("y", 93)),
        (("x", 160), ("y", 141), ("x", 120), ("y", 93)),
        (("x", 80), ("y", 141), ("x", 120), ("y", 93)),
        (("x", 80), ("y", 93), ("x", 120)),
        (("y", 93), ("x", 120)),
    ),
    "DOWN": (
...
def main() -> dict:
    ...
    env, assist, _ = w.open_env("Level5Whistle66")
    total = [1]
    hops = []
    checkpoints = ["Level5WhistleFloor", "Level5Whistle64", "Level5Whistle65", "Level5Whistle66"]
    try:
        idle(env, assist, total, 12)
        snap = read_snapshot(env.get_ram())
        n = len([o for o in snap.objects if 1 <= o.slot <= 12 and o.type_id == GIBDO and o.hp > 0])
        print("START66", [snap.link_x, snap.link_y], "gibdo", n, "doors", int(snap.cur_opened_doors), "whistle", int(read_u8(env.get_ram(), ADDR_WHISTLE)), flush=True)
        if n:
            fight = w.fight_type(env, assist, total, 0x66, GIBDO, expected=n)
            idle(env, assist, total, 20)
            hops.append({"hop": "clear_66_gibdo", "ok": fight.get("ok"), "end_n": fight.get("end_n"), "frames": fight.get("frames")})
            print("FIGHT66", hops[-1], flush=True)
            snap = read_snapshot(env.get_ram())
            if snap.screen != 0x66:
                rec = {"ok": False, "failed_room": f"0x{snap.screen:02x}", "reason": "left_66_during_fight", "hops": hops, "pokes": False, "status_claim": None}
                w.write_dump("l5_whistle_to_24", rec)
                return rec
            live = [o for o in snap.objects if 1 <= o.slot <= 12 and o.type_id == GIBDO and o.hp > 0]
            if live:
                rec = {"ok": False, "failed_room": "0x66", "reason": "gibdo_remain", "n": len(live), "hops": hops, "pokes": False, "status_claim": None}
                w.write_dump("l5_whistle_to_24", rec)
                return rec
        idle(env, assist, total, 30)
        snap = read_snapshot(env.get_ram())
        print("POST66", "doors", int(snap.cur_opened_doors), "mask", int(snap.open_doorway_mask), [snap.link_x, snap.link_y], flush=True)
        ...
        if not north["ok"]:
            rec = {"ok": False, "failed_room": "0x66", "reason": "north_not_0x56_after_clear", "hops": hops, "now": w.dump_live(read_snapshot(env.get_ram()), env.get_ram()), "pokes": False, "status_claim": None}
            w.write_dump("l5_whistle_to_24", rec)
        ...
        w.dump_and_save_room(env, assist, total, "l5_27_whistle", "Level5Whistle27", "Level5Whistle66", "0x37 UP")
        west27 = walk_west_from_27(env, assist, total)
        wait_play(env, assist, total, 0x26, max_f=240)
        snap = read_snapshot(env.get_ram())
        if snap.screen != 0x26:
            left = take_door(env, assist, total, "LEFT", expect=0x26)
            hops.append({"hop": "0x27_west", "dest": f"0x{left['dest']:02x}", "ok": left["ok"], "via": "take_door"})
        else:
            hops.append({"hop": "0x27_west", "dest": "0x26", "ok": True, "via": "walk_west_from_27", "raw_success": west27.get("success")})
        print("27WEST", hops[-1], flush=True)
        if read_snapshot(env.get_ram()).screen != 0x26:
            rec = {"ok": False, "failed_room": "0x27", "hops": hops, "now": w.dump_live(read_snapshot(env.get_ram()), env.get_ram()), "pokes": False, "status_claim": None}
            w.write_dump("l5_whistle_to_24", rec)
            return rec
        west26 = walk_west_from_26(env, assist, total)
        wait_play(env, assist, total, 0x25, max_f=240)
        snap = read_snapshot(env.get_ram())
        if snap.screen != 0x25:
            left = take_door(env, assist, total, "LEFT", expect=0x25)
            hops.append({"hop": "0x26_west", "dest": f"0x{left['dest']:02x}", "ok": left["ok"], "via": "take_door"})
        else:
            hops.append({"hop": "0x26_west", "dest": "0x25", "ok": True, "via": "walk_west_from_26", "raw_success": west26.get("success")})
        print("26WEST", hops[-1], flush=True)
        if read_snapshot(env.get_ram()).screen != 0x25:
            rec = {"ok": False, "failed_room": "0x26", "hops": hops, "now": w.dump_live(read_snapshot(env.get_ram()), env.get_ram()), "pokes": False, "status_claim": None}
            w.write_dump("l5_whistle_to_24", rec)
            return rec
...
if __name__ == "__main__":
    r = main()
    print("OK", r.get("ok"), "TF", r.get("tf_l5_bit"), "FAILED", r.get("failed_room"), "HOPS", r.get("hops"))

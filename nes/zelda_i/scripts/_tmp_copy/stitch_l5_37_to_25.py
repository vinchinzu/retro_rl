...
def main():
    ...
    n = [0]
    env = make_env(GAME, "Level5Cleared37", GAME_DIR, render_mode="rgb_array", record=str(movie))
    seams = []
    try:
        obs, _ = reset_obs(env)
        ...
        obs, c27 = run_spec(env, ROOM_27_SPEC, assist, n)
        ok27 = level5_room_27_cleared(env.get_ram())
        seams.append({"name": "Level5Cleared27", "ok": ok27, "ctl": c27.report(), **pin(env)})
        print("27", ok27, pin(env), flush=True)
        if not ok27: raise SystemExit("fail_27")
    ...
    report = {"ok": True, "segment": TAG, "continuous_emulator_session": True, "track": "assisted",
              "status_claim": False, "start_state": "Level5Cleared37", "end_claim": "Level5Cleared25",

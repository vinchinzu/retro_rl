...
Segments (rr-5lu children)::
    ...
    chain_to_key          entry→clear→bomb→key (assisted path proof)
...
Examples::
    ...
    uv run python nes/zelda_i/scripts/run_level4_rooms.py --segment clear_61 \\
        --infinite-life --trials 2 --save-state
    uv run python nes/zelda_i/scripts/run_level4_rooms.py --segment chain_to_key \\
        --infinite-life --trials 2 --save-state
"""
...
SEGMENTS = ("entry_up", "clear_61", "bomb_61", "key_51", "chain_to_key")
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_json_report(
        out,
        {
            ...
            "live_graph": {
                "0x71": {"UP": "0x61"},
                "0x61": {"BOMB_UP": "0x51", "enemies": "3x0x12→0x1c"},
                "0x51": {"LEFT": "0x50", "enemies": "8x0x1b", "key": "0x19"},
            },
            "reports": reports,
        },
    )

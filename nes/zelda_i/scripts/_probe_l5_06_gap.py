"""0x06 diamond GAPS (y=109 / y=173) onto center stairs."""
...
def dump(env):
    ...
    return {"sc": f"0x{s.screen:02x}", "mode": s.mode, "xy": [s.link_x, s.link_y], "tile": int(s.colliding_tile), "whistle": int(read_u8(env.get_ram(), ADDR_WHISTLE))}
...
def main():
    ...
    try:
        ...
        paths = [
            (("x", 96), ("y", 109), ("x", 120), ("y", 141)),
            (("x", 96), ("y", 173), ("x", 120), ("y", 141)),
            (("x", 80), ("y", 109), ("x", 120), ("y", 141)),
            (("x", 80), ("y", 173), ("x", 120), ("y", 141)),
            (("x", 96), ("y", 149), ("x", 120), ("y", 141)),
            (("x", 96), ("y", 125), ("x", 120), ("y", 141)),
            (("x", 104), ("y", 117), ("x", 120), ("y", 141)),
            (("x", 104), ("y", 165), ("x", 120), ("y", 141)),
        ]
        for i, steps in enumerate(paths):
            if in_cellar(env):
                break
            print("PATH", i, steps, flush=True)
            for axis, tgt in steps:
                ok = walk_axis(env, assist, total, axis, tgt, max_f=400)
                rec = {"p": i, "s": f"{axis}{tgt}", "ok": ok, **dump(env)}
                log.append(rec)
            ...
            if in_cellar(env):
                break
            # small steps toward center
            for _ in range(40):
                s = read_snapshot(env.get_ram())
                if in_cellar(env):
                    break
                if abs(s.link_x - 120) <= 1 and abs(s.link_y - 141) <= 1:
                ...
                total[0] += 1
                assist.apply_env(env, frame=total[0])
            rec = {"p": i, "s": "center_nudge", **dump(env)}
            log.append(rec)
        ...
        save_rgb_png(env.step(nes_idle_action())[0], RECORDINGS_DIR / "l5_06_gap.png")
            ...
            if dest["sc"] == "0x64" and dest["mode"] == 5:
                ...
                write_state_provenance(
                    ...
                    request={
                        ...
                        "via": "0x06 diamond gap stairs, 0x07 other mouth",
        ...
        write_json_report(RECORDINGS_DIR / "l5_06_gap.json", {"log": log, "final": dest, "pokes": False})

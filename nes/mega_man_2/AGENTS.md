...
Goblin-top solid window still unfound overnight. Then natural-entry (M4).
...
- `AirScreen3` / `AirScreen4` are mid-air clear snaps — use `AirFanPlatform` for
  grounded post-s3 iteration (solid **prog 937–984**). Pink head = Goblin.
...
- Do not treat left-ledge hops or same-platform walk as Goblin-head / past-island
  checkpoints; prune false `AirGoblinHead*` / `AirPastFan*` probe states.

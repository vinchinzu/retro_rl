...
| `screen8` | HeatScreen8 | LEFT10 → idle1 → A+LEFT14 → coast → Yoku sx~168 sy~100 |
...
4. **Heat multi-level Yoku → Sniper → boss door** — chain past first block; no boss_hp yet
5. **Heat boss clear + Item-1 unlock pin** — post-Heat `items\|$01` + Atomic Fire
   `weapons\|$01`
6. **Stage select → Air with Item-1** — weapons/items persist after Heat clear
7. **Air Fan → Item-1 platforms past s4** — deploy Item-1 (weapon menu + B) at
   prog ~984; clear camera ≥5 Clean
8. Optional: FCEUX human cloud-stand RAM pin (parallel residual; see below)
...
| Recipe | LEFT10 → idle1 → A+LEFT14 → LEFT coast (`start=screen8`) |
...
**Next tip:** chain multi-level Yoku (approach upper blocks from side/above, not
below) → Sniper → boss door → boss clear → Item-1. Do not tip rr-810 until Item-1.
...
| **rr-k1ea** | HeatScreen8 Sniper/Yoku → boss door | PARTIAL — first Yoku land dual-green; multi-level residual |

...
# Abort a start if x has not advanced this many frames and we are still
# short of the flag zone. Landed 2-2 ±80 sat at x≈700 until max_play.
STALL_FRAMES = 180
STALL_X_MAX = 2400
...
def reach_warpless_control(
    ...
    *,
    ...
    ``from_pred=True`` pins at the predecessor leave (drop-in) instead of
    idling to this leg's control. Returns an open env when ``ok``; caller
    must close it.
    """
...
def search_warpless(
    ...
    *,
    ...
    """Grid-search FM2 starts (+ optional lead idle) until leave / death."""
    leg = get_leg(stage_id)
    reached = reach_warpless_control(stage_id, from_pred=from_pred)
    ...
    center = fm2_hint(stage_id) if from_pred else fm2_hint(stage_id) + wait
    ...
    leads = range(0, max(0, int(lead_max)) + 1)
    pin = get_state(env)
    ...
    print(
        ...
        f"search {lo}..{hi} step={step} lead=0..{lead_max} "
        f"from_pred={from_pred} {reached['snap']}",
        flush=True,
    )
    try:
        for lead in leads:
            for si in range(lo, hi + 1, max(1, step)):
                ...
                tr["start_idx"] = si
                tr["lead_idle"] = lead
                tr["leave_to"] = leg.leave_id if tr["leave_frame"] is not None else None
                trials.append(tr)
                if _better_trial(tr, best, center):
                    best = {**tr, "_score": _trial_score(tr, center)}
                if (
                    tr["leave_frame"] is not None
                    or tr["warped"]
                    or (tr["max_x"] or 0) > 400
                    or si in (lo, hi)
                ):
                    print(
                        f"  {tag} lead={lead} si={si} max_x={tr['max_x']} "
                        f"leave={tr['leave_frame']} death={tr['death']}"
                        + (f" stall={tr['stall']}" if tr.get("stall") else "")
                        + (f" warped={tr['warped']}" if tr["warped"] else ""),
                        flush=True,
                    )
    finally:
        env.close()
    ...
    *,
    ...
    for i in range(min(len(body), cap)):
        ...
        if 0 < px < 20_000 and px > max_x:
            ...
            if (
                stall_frames > 0
                and stalled >= stall_frames
                and max_x < STALL_X_MAX
            ):
                stall_at = i + 1
                break
...
def _trial_score(tr: dict[str, Any], center: int) -> tuple[int, int, int, int, int]:
    """Rank: clear, shorter leave, fewer leads; misses rank by max_x then center."""
    leave = tr.get("leave_frame")
    clear = 1 if leave is not None and not tr.get("warped") else 0
    lead_rank = -int(tr.get("lead_idle") or 0)
    dist = -abs(int(tr["start_idx"]) - center)
    max_x = int(tr.get("max_x") or 0)
    if clear:
        return (1, -int(leave or 0), lead_rank, dist, max_x)
    return (0, max_x, lead_rank, dist, 0)


def _better_trial(tr: dict[str, Any], best: dict[str, Any] | None, center: int) -> bool:
    if best is None:
        return True
    return _trial_score(tr, center) > tuple(best.get("_score") or (0, 0, 0, 0, 0))
...
def export_warpless_slice(
    ...
    *,
    ...
    ``lead_idle`` is extra NULLs after control, stored in metadata (not
    baked into the FM2 body) so play/verify can apply them.
    """
    leg = get_leg(stage_id)
...
def verify_warpless_slice(
    ...
    *,
    ...
    lead = 0 if lead_idle is None else int(lead_idle)
    seed_data: dict[str, Any] | None = None
    if leg.seed_path.is_file():
        seed_data = load_nes9_rle_seed(leg.seed_path)
        if si <= 0:
            si = int(seed_data.get("fm2_start_index") or 0)
        if n <= 0:
            n = int(seed_data.get("num_frames") or 0)
        if lead_idle is None:
            lead = int(seed_data.get("lead_idle") or 0)
    if si <= 0 or n <= 0:
        return {
            "ok": False,
